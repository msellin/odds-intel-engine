...
|---|---|---|
| Admin visual direction | `dev/active/admin-shell-visual-direction.md` (+ maybe `dev/active/admin-refs/*.png`) | spec, read-only work |
| Admin information-architecture audit | `dev/active/admin-information-architecture.md` | spec, read-only work |
| Shared admin layout (structural refactor) | odds-intel-web: `src/app/(app)/admin/layout.tsx`, `src/components/admin/*`, edits to every admin page's outer wrapper, removal of `src/app/(app)/admin/bots/admin-shell.tsx`; engine smoke test `ADMIN-SHARED-SHELL` | CODE — review before committing |
