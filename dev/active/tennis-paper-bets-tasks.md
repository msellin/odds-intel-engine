...
- [⬜] **1.3** New `scripts/tennis/odds_api_scanner.py` — replaces `value_scanner.py`. Lists active tennis sport keys, fetches `/odds` per sport, computes edge vs Pinnacle, writes to `tennis_value_bets`
- [⬜] **1.4** `scripts/tennis/settle_value_bets.py` — calls `/sports/{key}/scores?daysFrom=2` per active sport, settles rows with `completed: true`
...
- [⬜] **1.6** Scheduler: replace `job_tennis_scanner` with `job_tennis_odds_api_scanner` (2× daily). Add `job_tennis_settlement` (02:00 + 14:00 UTC) + `job_tennis_closing_odds` (hourly 06-22 UTC, cheaper than per-fixture for MVP)
- [⬜] **1.7** Delete `scripts/tennis/value_scanner.py` once the replacement is live (OddsPapi client stays for soccer CLV backfill)
- [⬜] **1.8** Smoke `TENNIS-ODDS-API-SCANNER` + `TENNIS-SETTLEMENT` + `TENNIS-CLOSING-ODDS`
