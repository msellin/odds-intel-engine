...
| **COOLBET-AUTO-COOKIE-REFRESH** | P2 | 1-2h | Eliminate manual cookie pastes | Use `undetected-chromedriver` to launch real-Chrome browser, navigate to coolbet.com, let Imperva's challenge JS set fresh cookies, scrape them via `driver.get_cookies()`, write to `.env` via `python-dotenv set_key`. No login needed — Imperva sets cookies on first visit. Wire into daemon to auto-trigger on keepalive 403, or scheduled job every ~12-24h. Caveats: first run probably needs `headless=False`; CAPTCHA risk if Imperva escalates (rare); breaks on Chrome version updates if undetected-chromedriver lags. Worth shipping when manual refresh starts being annoying (~weekly in steady state). |
