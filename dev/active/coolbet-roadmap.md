...
| **COOLBET-AUTO-COOKIE-REFRESH** | 2026-05-20 | Eliminates the manual JWT paste. `scripts/coolbet_browser_setup.py` runs once at the laptop (headful Chrome via `undetected-chromedriver`, user does Smart-ID, profile saved to `~/.coolbet-daemon/chrome-profile/`, JWT location persisted to `jwt-location.json`). `scripts/coolbet_refresh_jwt.py` then runs headless every 25 min from the daemon (subprocess on venv python), extracts a fresh JWT from the saved profile, writes to `.env`, and the daemon hot-swaps via `CoolbetSession.reload_manual_jwt()`. Telegram `/relogin` triggers the same path. Session-expiry alert via Telegram with the recovery command. |
