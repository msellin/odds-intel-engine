...
| **COOLBET-INPLAY-SNAPSHOTS (Mode A capture)** | 2026-05-20 | Measures slippage between inplay-bot decisions and Coolbet live markets. Migration 115 adds `coolbet_inplay_snapshots` + `AFTER INSERT` trigger on `simulated_bets` that `NOTIFY inplay_bet_fired` when `xg_source IS NOT NULL`. Daemon runs a dedicated psycopg2 `LISTEN` thread; each NOTIFY → 1 Coolbet GET (`fetch_match_markets(live=True)` + `fetch_odds_for_markets`) → 1 snapshot row. **Zero polling on either side** — purely event-driven, ~1 Coolbet call per inplay signal (~30-50/day expected → trivial Imperva load). Three modes shipped (capture/paper/execute), default capture-only. `--inplay-mode` CLI flag + `/inplay_mode` Telegram command + `_CTRL` runtime override. Mode B writes `real_bets` row with `notes='inplay paper'` (no POST); Mode C additionally POSTs `/s/bets/bets` (REAL MONEY, gated by `--max-stake-per-bet`). 20% odds-drop tolerance flags `odds_drop_too_large` outcome before any side effects fire. |
