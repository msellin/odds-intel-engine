...
> **CHANGED 2026-09-26 (#162 W7.3 / audit C §7):** deleted as dead code — web `getModelV2Stats` (and its /performance call), `botRecentRoi`, `fetchUpcomingPicks`, `dropVipUnsettled`; engine `workers/jobs/weekly_digest.py`, `settlement._build_upcoming_model_summary`, the unused footer-CLV read in `daily_pipeline_v2`. Rows below that name them describe the pre-deletion state.
...
| c6 | `publish_picks_forward_test.claim` / `attach_message_id` plus `settle_picks_forward_test` / `void_bets_on_dead_matches` | the pre-registered test | dual-write with the immutability trigger live on `picks`; reconcile field by field daily |
