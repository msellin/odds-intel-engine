...
| `bot_v10_ou` | O/U 2.5 | model | — | — | paper | ⚑ **RETIREMENT RECOMMENDED 2026-09-23 ([[#077]])** — four independent measurements now return α = 0 for O/U (XGBoost head, double-Poisson, the full residual harness, and a shots+corners rating), no odds floor helps ([[#073]]), and it has published nothing since 2026-09-13. Owner's call; it is harmless where it stands. The O/U 2.5 half. De-vigged Pinnacle CLV **−3.85%** (n=181, 95% CI [−5.01, −2.69]) against ROI −0.5% — **`beta`, deliberately NOT `calibrated`**, because this page's own legend sells `calibrated` as proven and a CI entirely below zero cannot carry it. Survives gotcha 39: negative in **all 5 months** (−1.76% to −4.84%) and **all 7 model versions** (−0.97% to −6.12%), so it predates OU-CALIBRATOR-DOMAIN-MISMATCH (migration 335) — that bug made a bad half worse, it did not create it. Has published nothing since 2026-09-13. |
...
| `bot_consensus_b_v1` | 1x2 + O/U 2.5 | **sharp (consensus)** | **3%, ceiling 6%** (grade B) | cap 4.0 | published, not staked | **Grade B of the consensus arm — `beta`** ([[#095]], 2026-09-23). Same rule, same ledger row as the retired parent; a pick belongs here when every [[#094]] grade check passes: classified league (tier > 0), a second sharp panel book also sees an edge at the published price, edge ≤ 6%. Replay n=392, ROI +10.6% but **−3.4% in the holdout — not proven**. Ledger: `picks_forward_test WHERE arm='consensus_anchor' AND grade='B'`. **Why split in the views, not the ledger:** per-grade arms would defeat the `(match, market, selection, arm)` de-dupe when a leg's grade flips between the 30-min runs, publishing it twice. |
| `bot_consensus_c_v1` | 1x2 + O/U 2.5 | **sharp (consensus)** | 3%, ceiling 8% (grade C) | cap 4.0 | published, not staked | **Grade C of the consensus arm — `testing`** ([[#095]]). League tier 0, OR no second panel book sees an edge at the published price, OR edge > 6%. Replay n=285, **ROI −25.6%, stable in both halves (−26.0 / −24.7)**. Published and tracked in the open so it can be retired on its OWN record. Ledger: `picks_forward_test WHERE arm='consensus_anchor' AND grade='C'`. |
