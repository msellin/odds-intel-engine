...
| `bot_v10_1x2_newplus_v1` | 1x2 | model (combined) | — | — | paper | **"Match result — new model"** ([[#152]], 2026-09-25, owner). Twin of `bot_v10_1x2` on the NEW+ model (`r1x2_comb_v1`, used as is): EV ≥ 3% flat, odds 1.30–4.50, min_prob 0.30, `vip_exclude` (never a pick the VIP bot holds — so the EV 3–5% band). `bot_v10_1x2` is unchanged because its LIVE record is CLV +6.6% / +6.0% / +1.8% Jul/Aug/Sep (n 178) while the backtest that preferred this rule priced the old one at opening quotes (ANALYSIS_GOTCHAS #84). Compared live after 50–100 settled picks. `testing`, on /performance via `bots.show_on_performance` (migration 427). |
| `bot_high_roi_global_v2_newplus_v1` | 1x2 | model (combined) | — | — | paper | **"High-odds match result — new model"** ([[#152]], 2026-09-25, owner: "lets try it anyways"). Twin of `bot_high_roi_global_v2` with IDENTICAL rules (Spain/Australia/Iceland, home/away, odds 1.50–5.50, min_prob 0.28, same tier thresholds); only the probability changes to NEW+ (`r1x2_comb_v1`, used as is); `vip_exclude`. The original keeps its live record (CLV +6.9%); the backtest could not judge this niche and its NEW+ variants were negative, so this is a live experiment. `testing`, on /performance (migration 428). |
...
| `bot_combined_1x2_ev8_v1` | 1x2 | model (combined) | — | — | paper | **"1x2 NEW+ EV8"** (added 2026-09-24, [[#141]] B4). As EV5 with EV ≥ 8%. Backtest B2 CLV +3.1% (n=557; +2.7% at our own sweepers). `experimental`, `simulated_bets`, no placement path. |
