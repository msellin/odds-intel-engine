...
| Anchor ("fair value") | our **calibrated model** | **de-vigged Pinnacle** line — `devig.fair_prob` (Shin 1x2, power 2-way), via the ONE sharp engine (see below) |
...
**ONE SHARP ENGINE ([[#162]] W7.6, 2026-09-26).** Every sharp-anchored bot that writes picks
(`bot_trigger_*_sharp_v1`, the per-book `bot_{coolbet,unibet}_trigger_sharp_*`, the tight instrument,
O/U EARLY and O/U TWO-ANCHOR) is a `SharpRule` config of `workers/automation/sharp_engine.py`, and
`sharp_engine.evaluate` is the one decision. Shared by all: fair price `devig.fair_prob`; Pinnacle age
`anchor.anchor_line_too_old` (W8.3) unless a rule sets a stricter fixed cap (O/U EARLY 3 h); book quote
≤ `SHARP_BOOK_MAX_AGE_MIN` = 60 min (O/U EARLY: 3 h); edge ceiling `SHARP_EDGE_CEILING` = 8 pp (not on
the pre-registered tight instrument, nor O/U, whose EV cap is 15%); outlier cap `OUTLIER_MULT` × the
window's min odds; wrong-fixture guard `is_anchor_sane` vs Pinnacle's own quote — every gate on every
book BEFORE the best price is chosen. The producers keep only their loader and their write
(`pick_generator._generate_sharp`, `pick_trigger_matcher.match_and_emit`, `ou_sharp_outlier.run`).
Before this, three implementations had drifted: Shin on O/U, a 180-min book quote on the merged bots,
the ceiling checked on the winning book only, no ceiling at all on the per-book bots, and the per-book
bots priced off a fair price frozen at :05. The seven trigger/tight bots are **r2** from 2026-09-26
(split their series on `rule_version`); O/U EARLY / TWO-ANCHOR replayed **identical** over 2 days and
stay r1. Pinned by smoke `ONE-SHARP-ENGINE` (parity fixture per bot).
...
> **ONE SHARP ENGINE, r2 (#162 W7.6, 2026-09-26)** — see §1. The per-book sharp bots below are decided
> at match time by `sharp_engine.evaluate` on `pick_triggers.sharp_rule(strategy, book)` (they used to
> join the :05 Stage-A window), with the 8% edge ceiling and, on O/U, the power fair price; the merged
> `bot_trigger_*_sharp_v1` pair moved from the router's 180-min quote to 60 min and apply every gate to
> every book before taking the best price. `pick_triggers` still writes the sharp windows as a record.
...
> |---|---|---|
> | ~~`bot_coolbet_trigger_sharp_1x2_v1`~~ **RETIRED 2026-09-26 (#191, migration 480)** | −1.50% (n=62) | **+5.30%** (n=15) |
> | ~~`bot_unibet_trigger_sharp_1x2_v1`~~ **RETIRED 2026-09-26 (#191, migration 480)** | −5.04% (n=42) | +0.37% (n=25) |
> | ~~`bot_coolbet_trigger_sharp_ou_v1`~~ **RETIRED 2026-09-26 (#191, migration 480)** | −5.21% (n=11) | **+7.35%** (n=8) |
> | ~~`bot_unibet_trigger_sharp_ou_v1`~~ **RETIRED 2026-09-26 (#191, migration 480)** | −3.90% (n=10) | +2.62% (n=10) |
...
| Bot | Market | Anchor | Edge floor | Odds floor | Money | What it does |
|---|---|---|---|---|---|---|
| ~~`bot_coolbet_trigger_sharp_1x2_v1`~~ **RETIRED 2026-09-26 (#191, migration 480)** | 1x2 | sharp | 3%–8% (ceiling from r2) | 1.01 | paper | **r2 (W7.6)** Sharp twin: fires when Coolbet's 1x2 price beats the de-vigged Pinnacle line by ≥3% (no odds floor — experimental). Paper. Head-to-head vs the model twin. |
...
| ~~`bot_coolbet_trigger_sharp_ou_v1`~~ **RETIRED 2026-09-26 (#191, migration 480)** | O/U 2.5 | sharp | 3%–8% (ceiling from r2) | 1.01 | paper | **r2 (W7.6: power fair price)** Sharp twin: fires when Coolbet's O/U 2.5 price beats the de-vigged Pinnacle line by ≥3% (no odds floor — experimental). Paper. Head-to-head vs the model twin. |
| ~~`bot_unibet_trigger_sharp_1x2_v1`~~ **RETIRED 2026-09-26 (#191, migration 480)** | 1x2 | sharp | 3%–8% (ceiling from r2) | 1.01 | paper | **r2 (W7.6)** Unibet 1x2 sharp trigger. **Where the DRAW edge the model can't see should surface** (soft-book mispricing vs de-vig Pinnacle, §57). Paper. |
...
| `bot_ah_sharp_v1` | asian handicap (every line: whole / half / quarter) | sharp (Pinnacle, same line) | EV 3–15% | — | paper | **"Asian handicap — sharp line"** ([[#187]], 2026-09-26; the third market, 👥 PICKS, global books). An API-Football book's AH price beats Pinnacle's power-de-vigged fair price **on the same line in the same fetch** by EV 3–15%, ≥ 45 min before kickoff; the book's own neighbouring lines must be consistent with the price (ladder guard — Betano's quarter ladder is broken on 9.4% of same-fetch triples); one pick per match, ever. Pre-registered (`dev/active/ah-market-bot-prereg.md`, 2 dated amendments): CLV vs an **independent close** (other books' consensus, Pinnacle and the pick's own book excluded — §85) **+5.4% discovery** (Jul–Aug, n 223) / **+3.0% holdout** (1–25 Sep, n 185, p 0.0001, Holm); ROI +1.2% [−8.9, +11.1] = unproven. Direct-feed books excluded (Epicbet −8.1% in the holdout, absent from discovery) — `AF_BOOKS` allow-list frozen in the job. Public price = the price the pick was taken at (never a refused higher quote). Quarter lines settle half win / half loss (`simulated_bets.settle_fraction`, migration 476). Job `workers/jobs/ah_sharp_outlier.py` :18/:48, a `SharpRule` of the one sharp engine (rung key `asian_handicap:<home line>`). `experimental`, `simulated_bets`, no placement path. **Read the backtest correctly:** retention keeps only open/close/latest rows after ~1 day (ANALYSIS_GOTCHAS §59), so Jul–Sep mostly tested OPENING quotes vs Pinnacle's opening price; live uses the latest quotes every 30 min — the forward record decides. Measured live (30 h): opportunities last a median 150 min early / 90 min late, so faster polling was not built. |
| `bot_ou_sharp_early_v1` | ou (1.5/2.5/3.5) | sharp (Pinnacle) | — | — | paper | **⭐ VIP bot #2 (O/U, owner 2026-09-25; migration 424)** — live picks only to Pro/Elite DMs + the private channel, EV8/EV5 labels, public once settled. **"O/U EARLY"** ([[#149]], 2026-09-25; `dev/active/market2-model-plan.md` rounds O1–O3). A soft book's O/U quote beats Pinnacle's power-de-vigged fair price by EV 5–15% (the 15% cap guards against misposted lines) while ≥ 12 h before kickoff; one pick per (match, line), best EV; every publishable book. Why not a model: O1's combined O/U model lost to Pinnacle on Pinnacle-priced rows. Backtest O3 T3: CLV +7.5% / +6.9%, ROI +10.4% / +10.8% (Aug n=383 / Sep n=459). Job `workers/jobs/ou_sharp_outlier.py` :14/:44 — a `SharpRule` config of the one sharp engine since #162 W7.6 (replayed identical, stays r1). `experimental`, `simulated_bets`, no placement path. |
...
| `bot_v10_ou_comb_v1` | ou (1.5/2.5/3.5) | model (combined O/U) | EV ≥ 3% | 1.30–3.00 | paper | **"Goals over/under — new model"** ([[#152]], owner 2026-09-25: *"unretire bot_v10_ou — it gets the new ou model"*; migration 443). `bot_v10_ou`'s return on the combined O/U model `ou_comb_v1` (`workers/model/combined_ou.py`, table `ou_model_predictions`; served p = Pinnacle where priced, else combined — **at decision time** a ≤ 3 h Pinnacle quote on the line overrides the stored p and the combined rows are refreshed immediately before the bots read them ([[#176]]); used as is — no `calibrate_prob`, no data-tier bump): EV = p×odds−1 ≥ 3% flat across tiers, O/U 1.5/2.5/3.5 over and under, odds 1.30–3.00, min_prob 0.30, **one pick per match** (best EV). Picks O/U EARLY holds or would take (EV 5–15% vs Pinnacle, ≥ 12 h out) are recorded and **held back until kickoff** by `store_bet`'s VIP guard ([[#164]]). **TESTING**: sent to /picks (every pick) + public Telegram at EV ≥ 5% only ([[#174]]), own record, not in the headline (`show_on_picks`, `show_on_performance`). **Why a twin, not the old row re-activated:** `bot_ledger` counts every `simulated_bets` row of a bot, and `bot_v10_ou` holds 252 settled old-ensemble picks (sharp CLV −3.9%, n 138, 95% upper bound −2.65%) — re-activating it would raise the [[#155]] review flag on day one and sell the old model's record as this rule's (ANALYSIS_GOTCHAS #84). Backtest (08-31..09-24 at open, #152 step-3 machinery extended to 1.5): **61 picks (~2.4/day), sharp CLV +2.0% [+1.0, +3.0]**, ~7% in VIP's range; same window the model was chosen on, so the forward record decides. |
...
|---|---|---|
| `anchor_sanity.is_anchor_sane` (ratio > 1.5625× vs Pinnacle) | READ: `best_price_router._latest_book_odds` (model bots) **and** the sharp engine's gate stack (every sharp bot, since #162 W7.6) | 20 of the 25. Weak on O/U, where prices are compressed into ~1.2–3.0 and a wrong fixture rarely trips a ratio test. **No Pinnacle line → falls back to the median raw price of ≥4 other books (#113, 2026-09-23; covered 418 of 569 placeable quotes on non-Pinnacle fixtures at wiring); fails open only below that quorum.** |
| `sharp_engine.SHARP_EDGE_CEILING` (8%; `BotConfig.edge_ceiling` on `sharp_devig`) | the sharp engine — merged AND (since #162 W7.6) per-book sharp bots; not the pre-registered tight instrument | all 31, including every O/U one. Works in edge space, so market compression does not blunt it. |
| `anchor_sanity.OUTLIER_MULT` (`max_odds = min_odds × 1.6`) | the sharp engine (every pp-unit sharp bot) | 14 of the 25. The generator used to be a *clone* of the sharp path that had silently dropped this half of the window; since #162 W7.6 there is one path. |
...
> **#162 W0.2 (2026-09-25, migration 436):** the run-level gate also requires the money-gate contract (`coolbet_session_state.money_gate_contract` ≥ 1 and = code `GATE_CONTRACT`); it is 0 until #162 W4 unifies the placement checks, and the DB refuses a real-money switch ON / arming until then. **CLOSED 2026-09-25 (migration 459): the contract is 1 in the DB and in code — the lock no longer refuses; turning real money on is the owner's (COOLBET_OWN_BETTING 'Turning real money on').**
...
owner decision, migration 462):** BETA and CALIBRATED were merged into ONE status, **ACTIVE** — verified that
day, they differed in nothing a reader saw or received (same /picks, /performance listing, headline totals and
Telegram); only a badge colour, a legacy `pro_value_bets_30d` stat, the operator's Coolbet summary and the
signaler's row order told them apart. The owner's reason: readers must clearly see which bots count toward the
totals — /performance marks ACTIVE "counts in the totals above" and TESTING "own record only — not in totals".
Relabelled: `bot_v10_1x2` (was `calibrated`) and `bot_high_roi_global_v2` (was `beta`) → `active`.
...
now OPENS with a short, same-shape status line — `🟢 ACTIVE · Sharp` (bold) or `⚠️ TESTING · Model` (italic): the status plus the METHOD tag from `bot_config.anchor` (Sharp / Consensus / Model; the market is already on the pick row). The pick row's marker follows the status — `🎯 Pick:` for ACTIVE, a plain `▫️ Pick:` for TESTING — so the only green in the channel is ACTIVE (owner 2026-09-26: "it's all very green"; the long bot names on every pick were noise; Telegram has no colour or font size, so the difference is emoji + bold/italic weight). The channel description explains ACTIVE / TESTING and the three method words. Stamped in ONE place, `pick_sender.send_pick` (CHANNEL_PUBLIC), via `bot_status.style_public_pick`, so no caller can post an unlabelled pick. The consensus
...
- Headline totals: `HEADLINE_BOT_SQL` (settlement `write_dashboard_cache`) and web
  `HEADLINE_MATURITY_LABELS` = ACTIVE, VIP excluded.
- /performance: web `PUBLIC_MATURITY_LABELS` = TESTING + ACTIVE (lib/bot-status.ts).
...
- **TESTING → ACTIVE:** after **50 settled picks** (owner 2026-09-26 — the same n as the review flag, so a TESTING bot is either promotable or flagged by n = 50)
  ...
  There is no further promotion: the old BETA → CALIBRATED rule was deleted with the merge ([[#175]]).
...
> **RETIRED 2026-09-26 ([[#175]]):** `beta` and `calibrated` no longer exist — both were merged into
> `active` (migration 462; the CHECK allows only experimental / testing / active / retired), and the
> `beta` → `calibrated` gate below is DELETED — TESTING → ACTIVE (50 settled, sharp CLV > 0, "Promotion
> and review" above) is the only promotion. Everything below is the #069 history, kept for its reasoning.
>
