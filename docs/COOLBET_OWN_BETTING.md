...
edge floor, a 2.80 odds floor, a live-edge floor, blast-radius limits, and not
...
| 6 | **Odds floor** (REALMONEY-ODDS-BAND-MISMATCH) | `COOLBET_MIN_ODDS` | Coolbet price ≥ **2.80** | CLV measured NEGATIVE below 2.80. **This is a PRE-MATCH gate — not inplay** (the 2.80 also appears in the inplay eval, which caused the mix-up) |
...
| `COOLBET_MIN_ODDS` | pre-match odds floor | 2.80 |
...
- **Edge × odds interaction is NOT a validated matrix.** Gates 3 (edge floor) and
  6 (2.80 odds floor) are two INDEPENDENT 1D cuts, set separately. The 104k
  fixture-level matrix (2026-09-08) shows they interact (long odds bad at low
  edge, great at high edge; sweet spot high-edge × mid-odds). A 2D gate would
  likely beat two 1D floors — see the matrix work / any EDGE-ODDS-MATRIX task.
