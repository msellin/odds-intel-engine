...
| **1H / 2H** | 🟡 **DATA READY — model+edge test next** | NEW half-goals model | **HT/2H SCORE STORED ✅** (2026-09-08): `matches.ht_score_*` + `h2_score_*` (2H=FT−HT), from AF `/fixtures` `score.halftime`+`fulltime`, zero extra calls; forward-wired + daily sweep | ✅ (`1x2_1h`, `over_under_1h_*`, `team_total_1h_*`); **no native 2H line** in our feed | **build a 1H model + run the discrimination test** (BTTS/AH method) — data now exists. Descriptive: 55.3% of goals in 2H, 2H>1H in 44% vs 30% (robust, all tiers) — but it's a KNOWN, priced effect; flat-back 1H 1x2 = only vig (−9→−13%). Edge needs the model. NB HT/FT itself has NO Pinnacle anchor. |
