...
|---|---|---|---|---|
| Model-anchored 1x2 | **DEAD — re-confirmed on a FAIR test 2026-09-16** | residual α = **0.0000**, n=7,775; market AUC 0.6997 vs model 0.6632; residual AUC 0.4189 (**below** 0.5). The original run fed the three `pinnacle_implied_*` features as 0.0 (they live in `match_signals`, not mfv) — supplying them lifted model AUC 0.6437→0.6632 and did **not** move α | the lift shows the model CAN use market features when given them — that is evidence about the feature pipeline, not the model family | `residual_test.py`, `c8b729af` |
