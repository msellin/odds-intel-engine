...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_predictions_match_market
  ON predictions (match_id, market);

-- matches: used by getModelAccuracy() with WHERE status = 'finished' AND result IN (...)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_matches_status_result
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_match_signals_match_captured
  ON match_signals (match_id, captured_at DESC);

-- odds_snapshots: composite for RPC queries filtering by match + market + timestamp
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_odds_snapshots_match_market_ts
