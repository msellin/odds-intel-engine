...
Data tier system:
- **Tier A**: team in targets_poisson_history.csv — Poisson + XGBoost available. European leagues + Romania (RO1, since 2026-05-12); TIER-C-EXPAND (2026-05-19) added 14 more top divisions via `scripts/ingest_football_data_extras.py` — ARG/AUT/BRA/CHN/DNK/FIN/IRL/JPN/MEX/NOR/POL/RUS/SWE/USA (Switzerland skipped — football-data publishes Chinese data under `CHE`). Tier A team count: 541 → 864.
...
| Odds volatility (std of implied prob, 24h) | `odds_volatility` | Morning pipeline | ✅ Running — ⚠️ **NOT the model's `odds_volatility_{sel}_at_t6h`** (MFV, per selection, at T−6h, written by the 22:30 backfill): measured r=0.138 over 20 days. Same caveat for `sharp_consensus_*` (r=0.615 vs its `_at_t6h`); `pinnacle_line_move_*` agrees (r=0.955). No model reads the `match_signals` copies ([[#085]], 2026-09-23). |
...
| Pinnacle open→close drift home | `pinnacle_drift_home` | DRIFT-FEATURE backfill 2026-06-04 + future MFV builder | ✅ **WRITER FIXED 2026-09-11 (DRIFT-FEATURE-WRITER). Coverage 0% → 46.5%** over the last 60 days (10,418 of 22,413 rows), newest value today. Two bugs were behind the three-month silence: the script was never registered in the scheduler (now `pinnacle_drift_refresh`, daily 02:30 UTC on a 3-day rolling window), and its query joined ALL opening rows to ALL closing rows so the fan-out dropped 84% of what it could compute — 1,861 matches where 11,827 were available. ⛔ **NOT a training feature, permanently.** It is `1/closing − 1/opening`, so it needs the closing price and cannot exist at prediction time; the inference path never supplies it. Training on it would fit three always-missing features and bias every other coefficient. The backtested +8.76pp spread is a POST-HOC result, not a pre-kickoff signal. Use it for research / CLV analysis / the delayed-placement design; the pre-kickoff cousins are `pinnacle_line_move_*_at_t6h` and `sharp_consensus_*_at_t6h`. Guarded by smoke DRIFT-NOT-IN-RETRAIN. Column added migration 179; backfill via `scripts/backfill_pinnacle_drift.py`. Backtest: +8.76pp WR spread top vs bottom quintile (8,850 matches, CSV-FULL-EXTRACT). Distinct from `pinnacle_line_move_home_at_t6h` (which uses ≤T−6h cutoff) — this column uses open→close which captures CSV-historical period that lacks intermediate snapshots. |
