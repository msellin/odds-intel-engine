...
| Pinnacle open→close drift home | `pinnacle_drift_home` | DRIFT-FEATURE backfill 2026-06-04 + future MFV builder | ⚠️ **STALE WRITER — flagged 2026-09-11 (DATA-TASK-AUDIT). The column exists but nothing populates it any more: last non-null value is dated 2026-06-06, and it is non-null on 0 of 22,401 `match_feature_vectors` rows built in the last 60 days (10,479 of 87,932 ever).** The "future MFV builder" in the Source column was never built, and `scripts/backfill_pinnacle_drift.py` is not registered in `workers/scheduler.py`, so this signal is effectively absent from every model trained since June. See `PRIORITY_QUEUE.md` → DRIFT-FEATURE-REENABLE. Column added migration 179; backfill via `scripts/backfill_pinnacle_drift.py`. Backtest: +8.76pp WR spread top vs bottom quintile (8,850 matches, CSV-FULL-EXTRACT). Distinct from `pinnacle_line_move_home_at_t6h` (which uses ≤T−6h cutoff) — this column uses open→close which captures CSV-historical period that lacks intermediate snapshots. |
