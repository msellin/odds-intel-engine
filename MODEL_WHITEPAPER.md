...
**Model-version segmentation (important):** Every `simulated_bets` row carries a `model_version` column. Platt fitting and all calibration checks must be run on a single model version at a time. Mixing versions produces a blended curve that is not valid for any individual version — probabilities from `v9a_202425` were generated by a different ensemble configuration than `v12_post0e` or later. Filter: `WHERE model_version = 'vX'`. Minimum sample size applies per version, not in aggregate.

Current status (2026-05-11): `v9a_202425` has ~600+ settled bets — sufficient for calibration as a baseline. **Only calibrate the current production model version** — short-lived intermediate versions (e.g. `v12_post0e` with 36 bets before being superseded by `v14`) will never reach the 300+ threshold and should be skipped. Their bets remain in the DB and are valid for CLV analysis, but not for Platt refit.
