...
**Active production version (2026-07-31):** `MODEL_VERSION=v20260712` (main head — governs 1X2 + BTTS + AH via the joint-goal matrix). `MODEL_VERSION_OU=v20260719` and `MODEL_VERSION_OU_T1=v20260719` as env overrides. Main remains v20260712 because v20260719 regresses on 1X2_draw (+1.8% p=0.012) vs v20260712 — but v20260719 wins the OU-override slot outright over the prior override v20260621 (OU log-loss −2.3% p=0.997, 6 BETTER / 0 WORSE / 5 TIE overall). See `dev/active/rigorous-eval-v20260719-vs-v20260621-ou.md` and `dev/active/rigorous-eval-v20260719-vs-v20260712.md`.

**2026-07-31 re-eval + OU override flip (RIGOROUS-OOS-RE-EVAL-2026-07-31 + OU-OVERRIDE-FLIP-2026-07-31).** Three OOS evals run post-vacation via `scripts/rigorous_eval.py` (bootstrap-n=1000). (1) **v20260712 vs v20260705** on the full 20-day OOS window 2026-07-12 → -07-31 (n=3,171, 3× the 07-18 sample) reconfirms the 07-18 promotion: 5 BETTER / 2 WORSE / 4 TIE, 1X2 gains widened (home −4.3% p=1.000, draw −4.4% p=1.000), the "borderline OU regression" from 07-18 did NOT replicate (OU Δ 0.0% p=0.523 — TIE). Pinnacle-close 1X2 gap tightened to +2.7% / +2.3% (was +5.9% / +5.2%). Report: `dev/active/rigorous-eval-2026-07-31.md`. (2) **v20260719 vs v20260712** (candidate-as-main test, n=1,317, 11-day OOS): 2 BETTER / 1 WORSE / 8 TIE. 1X2_draw REGRESSION +1.8% p=0.012 blocks promotion as main. OU BETTER −1.5% p=0.962 — motivates OU-slot test. Report: `dev/active/rigorous-eval-v20260719-vs-v20260712.md`. (3) **v20260719 vs v20260621** (OU-override head-to-head, n=1,328): v20260719 wins 6 markets significantly (OU25 −2.3% p=0.997, 1X2_home −2.6% p=0.999, 1X2_draw −4.2% p=1.000, AH_+0.5, AH_-1.5) with 0 losses / 5 ties. Report: `dev/active/rigorous-eval-v20260719-vs-v20260621-ou.md`. **Ship**: `MODEL_VERSION_OU` and `MODEL_VERSION_OU_T1` flipped `v20260621` → `v20260719` on VPS via env swap + `systemctl restart oddsintel-scheduler.service`. Main unchanged. Meta unchanged (blocked by META-EVAL-PIPELINE-BROKEN). Rollback: one sed back to `v20260621` + one restart.
