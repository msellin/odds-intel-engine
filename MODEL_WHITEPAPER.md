...
**Active production version (2026-07-31):** `MODEL_VERSION=v20260712` (main head — governs 1X2 + BTTS + AH via the joint-goal matrix). `MODEL_VERSION_OU=v20260719` and `MODEL_VERSION_OU_T1=v20260719` as env overrides. Main remains v20260712 because v20260719 regresses on 1X2_draw (+1.8% p=0.012) vs v20260712 — but v20260719 wins the OU-override slot outright over the prior override v20260621 (OU log-loss −2.3% p=0.997, 6 BETTER / 0 WORSE / 5 TIE overall). See `dev/archive/rigorous-eval-v20260719-vs-v20260621-ou.md` and `dev/archive/rigorous-eval-v20260719-vs-v20260712.md`.

**2026-07-31 re-eval + OU override flip (RIGOROUS-OOS-RE-EVAL-2026-07-31 + OU-OVERRIDE-FLIP-2026-07-31).** Three OOS evals run post-vacation via `scripts/rigorous_eval.py` (bootstrap-n=1000). (1) **v20260712 vs v20260705** on the full 20-day OOS window 2026-07-12 → -07-31 (n=3,171, 3× the 07-18 sample) reconfirms the 07-18 promotion: 5 BETTER / 2 WORSE / 4 TIE, 1X2 gains widened (home −4.3% p=1.000, draw −4.4% p=1.000), the "borderline OU regression" from 07-18 did NOT replicate (OU Δ 0.0% p=0.523 — TIE). Pinnacle-close 1X2 gap tightened to +2.7% / +2.3% (was +5.9% / +5.2%). Report: `dev/archive/rigorous-eval-2026-07-31.md`. (2) **v20260719 vs v20260712** (candidate-as-main test, n=1,317, 11-day OOS): 2 BETTER / 1 WORSE / 8 TIE. 1X2_draw REGRESSION +1.8% p=0.012 blocks promotion as main. OU BETTER −1.5% p=0.962 — motivates OU-slot test. Report: `dev/archive/rigorous-eval-v20260719-vs-v20260712.md`. (3) **v20260719 vs v20260621** (OU-override head-to-head, n=1,328): v20260719 wins 6 markets significantly (OU25 −2.3% p=0.997, 1X2_home −2.6% p=0.999, 1X2_draw −4.2% p=1.000, AH_+0.5, AH_-1.5) with 0 losses / 5 ties. Report: `dev/archive/rigorous-eval-v20260719-vs-v20260621-ou.md`. **Ship**: `MODEL_VERSION_OU` and `MODEL_VERSION_OU_T1` flipped `v20260621` → `v20260719` on VPS via env swap + `systemctl restart oddsintel-scheduler.service`. Main unchanged. Meta unchanged (blocked by META-EVAL-PIPELINE-BROKEN). Rollback: one sed back to `v20260621` + one restart.

**Promotion rationale (MODEL-VERSION-FLIP-2026-07-18).** Sunday 2026-07-12 weekly_retrain landed `v20260712`. Offline eval via `scripts/weekly_eval_and_compare.py` on 3,130 settled matches over 2026-07-04 → -07-18 holdout: **1x2_home −7.4% log-loss, 1x2_draw −6.6%, 1x2_away −2.8%, over/under +1.8% (worse), BTTS tie, ah_home_-0.5 −1.0%, ah_home_+0.5 −2.0%, ah_home_-1.5 −2.2%, ah_home_+1.5 −2.5%.** Headline **7 BETTER / 2 WORSE / 2 TIE.** Caveat: first week (2026-07-04 → -07-11) is *in-sample* for v20260712 (trained on data cut ≈ 2026-07-11); only 2026-07-12 → -07-18 (~6 days) is strictly OOS, so headline 1X2 improvement is inflated. Real OOS lift likely 3-5% on 1X2 not 7. Still directionally consistent across 1X2 + AH (correlated markets), which is the strongest available signal without waiting 3 more weeks. Deployed via `sed` on `/opt/odds-intel-engine/.env` + `systemctl restart oddsintel-scheduler.service` (env-override pattern from MODEL-VERSION-FLIP-2026-07-06). Backup at `.env.bak-2026-07-18-model-flip` on VPS. Rollback = one sed + one restart. **Post-vacation task:** re-run `weekly_eval_and_compare.py v20260712 v20260705 --since 2026-07-12` on strictly-OOS data (should have 3+ weeks by then) to confirm the flip stuck.

**Historical.** `v20260705` since 2026-07-06 (main), `v20260621` since 2026-07-06 (OU override). Before that: `v20260524_market` since 2026-05-24. **Candidate queued for Sunday 2026-06-07 cron retrain (historical, superseded):** `v_20260525_signals` — trained 2026-05-25 on the extended FEATURE_COLS (42 columns, adds 10 over the v14-era 32 — form_momentum, injury_severity_score, league_draw_rate_ytd, season_progress, line_velocity, xg_overperf, league_clv_efficiency, team_avg_player_rating). Offline eval vs production on 2,522 matches in 2026-05-20..05-24: **beats production on 4/5 markets** (1X2 −6.5% to −7.8% log-loss, BTTS −2.9%, OU 2.5 tied). Calibration regresses slightly on draw + over_25 — likely to be fixed by the standard Sunday 2026-06-07 weekly_retrain with a clean week of post-signal data. **Not deployed mid-Phase-3.5** (lock until 2026-06-07). Decision rule on 2026-06-07: run `offline_eval v20260607 v_20260525_signals v20260524_market` three-way and deploy market-by-market via per-market `MODEL_VERSION_*` env overrides if calibration is mixed. Full report: `dev/archive/model-comparison-20260525-signals-vs-market.md`.
...
**Per-tier OU routing (OU-CLV-OPTION-B-RE-EVAL, 2026-06-12).** A follow-up to the WEEKLY-RETRAIN-OU-FEATURES residual: the 2026-06-07 mitigation pinned OU globally to `v14_recreate_2026_05_11` via the `MODEL_VERSION_OU` env override to neutralise the TIER-C-EXPAND-driven log-loss regression. But OU25-DEDICATED-MODEL-INVESTIGATE (2026-06-08) noticed the universe override under-performed `v20260524_market` and `v20260607` on the AF-era mature-league cohort. Per-tier eval (`scripts/diag_ou_eval_by_tier.py`) on n=219 AF-era holdout (train ≤ 2025-10-01, test through 2026-06-12) confirmed: on T1 (n=180), `v20260607` ROI@+5% is **+45.23%** (log-loss 0.6420) vs the override's **−4.09%** (log-loss 0.7778) — Δ +49pp ROI / −17% log-loss in favour of the global. T2 (n=2) and T3+ (n=4) holdout are too thin to falsify the override on tier-C, so the safe per-tier policy is to promote the global only where evidence exists. `_resolve_version("ou", tier=tier)` in `workers/model/xgboost_ensemble.py` now reads `MODEL_VERSION_OU_T{1|2|3}` (tier ≥ 3 buckets to `_T3`) before the existing `MODEL_VERSION_OU` market-level override, before the global `MODEL_VERSION`. Production envs after this change: `MODEL_VERSION_OU_T1=v20260607` (T1 routes back to global), `MODEL_VERSION_OU=v14_recreate_2026_05_11` (T2/T3+ continue to use the universe override). OU-EVAL-BY-TIER (2026-06-15) is the next data point — once T2/T3+ holdout grows enough to falsify the override there, we revisit.
...
**OU TIER-C exclusion at training (OU-LONGTERM-EXCLUDE-TIERC, 2026-06-25).** The residual 4.4% OU log-loss gap traced to TIER-C-EXPAND in the entry above is now mitigated at the data layer rather than via version pinning. `workers/model/train.py` carries an `OU_EXCLUDED_LEAGUE_COUNTRIES` frozen set (Argentina, Austria, Brazil, China, Denmark, Finland, Ireland, Japan, Mexico, Norway, Poland, Russia, Sweden, USA — the 14 countries onboarded 2026-05-19) and `train_over25_model(...)` filters those rows from the OU 2.5 target before XGBoost fit by default (`exclude_tier_c_countries=True`). The 1X2 / BTTS / goals heads still train on the full corpus — only the `over_under` head is filtered, because the per-tier diag showed those leagues have a different OU calibration than the mature-league cohort the model serves bets on. CLI opt-out is `--ou-include-tier-c` for forensic comparisons. The intent is to retire the `MODEL_VERSION_OU=v14_recreate_2026_05_11` env override once a fresh Sunday bundle trained with this filter demonstrates OU log-loss parity with v14 on a held-out AF-era sample. Smoke-pinned at `OU-LONGTERM-EXCLUDE-TIERC` in `scripts/smoke_test.py` (constant, country list, default-true kwarg, JOIN, CLI flag).
...
> **⚠️ CONTRADICTED 2026-09-16 — and a correction to the correction.** A Poisson/Dixon-Coles wrapper DOES exist (`workers/model/ou25_dedicated.py`, with `joint_probability.py` supplying the τ patch), so the first version of this banner was too strong in saying we had no such model. What is wrong is narrower and still fatal to the paragraph below: that wrapper takes its lambdas from the XGBoost goals regressors trained on the SAME shared feature set, so it inherits every problem the feature set has. A true Dixon-Coles — team attack/defence fitted by maximum likelihood on scores alone — was built and measured on 2026-09-16 and returned **alpha = 0.0000** (AUC 0.5708 vs Pinnacle 0.6043, n = 5,204). Goal-line markets therefore must NOT be given a higher model weight than 1x2 on the strength of this section. Also note `DIXON_COLES_RHO = -0.18` is hardcoded for every league in `ou25_dedicated.py`, while a per-league fit puts the median at **-0.048**; that discrepancy is now RESOLVED and the answer is sharper than a wrong constant: rho is **INERT**. The four cells tau corrects are all under 2.5 goals and their mass changes cancel exactly, so with the renormalisation that function performs, rho cannot move P(over 2.5) at all — measured spread 0.00e+00 across rho from -0.30 to +0.20. **For its only output, that module reduces to independent Poisson.** Smoke `OU25-DC-RHO-IS-INERT`.**
>
> **⚠️ ORIGINAL NOTE — OU-CLEAN-ALPHA-MEASURED.** The premise of the table below ("designed for goal totals, so we trust it more") has now been measured against outcomes and does not hold. `scripts/residual_test_ou.py`: residual **α = 0.0000** on all three arms, n=7,273 out-of-sample; the O/U head's AUC is **0.5788 against the market's 0.6007**, and the residual AUC is **0.4443 — below 0.5**, so where the model disagrees with Pinnacle, Pinnacle is right more often. Giving goal-line markets a HIGHER model weight than 1x2 is therefore backwards on the current head. Two caveats keep this a flag rather than a rewrite: (a) the shipped O/U head is an XGBoost classifier sharing the 1x2 feature set, **not** the Poisson/Dixon-Coles component this paragraph describes, so the sentence was already describing a different model than the one in production; (b) these alphas are loaded from `model_calibration` at startup and the live values may already differ from the defaults tabled here. **Do not raise goal-line alpha on the strength of this section.** See `docs/OWN_STRATEGY_AUDIT_2026_09_15.md` §3.
...
**Anchor price-sanity gate on 1X2 and on the BOT pricing paths (ANCHOR-PRICE-SANITY, implemented 2026-09-20).** The `OU-PINNACLE-CAP` above is the same idea, and its scope was the gap: it caps non-Pinnacle prices **only in O/U aggregation inside `_load_today_from_db`**. 1X2 had no equivalent, and neither of the two *bot* pricing engines (`best_price_router._latest_book_odds`, `workers/jobs/pick_trigger_matcher`) consulted Pinnacle at all. The consequence was `bot_trigger_1x2_sharp_v1` publishing **+549.9% ROI / €1,319.80** on 25 picks that were every one priced off a quote belonging to a different fixture — Beitar Jerusalem stored at 18.00 in a match Pinnacle priced 1.67, and settled "won". `workers/automation/anchor_sanity.py` now drops any book quote more than **1.5625×** from Pinnacle's price for the same `(match, market, selection)`, in both engines, both directions (an inverted triple makes one leg far too long AND its partner far too short). Threshold measured, not guessed: over 30 days and 7,457 paired fixtures the max-over-selections relative implied-probability deviation of Coolbet/Epicbet/Unibet-Site from Pinnacle runs p50 0.071, p90 0.196, p95 0.278 — 1.5625× is a 36% deviation, just past p97 and roughly 5× the largest overlay ever observed against Pinnacle. Fails **open** with no anchor, deliberately: refusing every fixture Pinnacle does not price would delete most obscure-league coverage and trade a known fault for an invisible one.

**Edge CEILING on anchor-derived bots (implemented 2026-09-20).** A structural point that applies to every gate in this system: `edge = p − 1/odds`, and every floor we own is a **lower** bound, while a bad price only ever **inflates** the edge. A price fault therefore clears every floor and can never trip one — and a bot whose fair value is a de-vigged sharp line is not merely exposed to this, it is a *search procedure for it*, since its edge definition is "this book disagrees hugely with Pinnacle". Contamination rates make the point: under 3% across all model-anchored cohorts, **80% for the sharp 1x2 bot**. `BotConfig.edge_ceiling` now refuses a `sharp_devig` pick above **8%**, against a documented observed maximum overlay of +6.6%. It is not set on model-anchored bots, where a 20% edge is ordinary (the registry floor alone is 13%). The ceiling is necessary *in addition to* the ratio guard because a ratio test is market-shaped: 1x2 prices span 1.02–101 and a wrong fixture usually blows past 1.5625×, but O/U prices are compressed into ~1.2–3.0, where all six contaminated O/U picks passed the ratio test and only the ceiling caught them.

**One sharp engine (#162 W7.6, 2026-09-26).** The sharp-anchored bots (fair value = de-vigged Pinnacle, no model) are all configs of `workers/automation/sharp_engine.py`: one fair price (`devig.fair_prob` — Shin for 1X2, power for 2-way), one Pinnacle age rule (W8.3, or a stricter fixed cap), one 60-min book-quote cap, one 8-point edge ceiling, the OUTLIER_MULT window cap and the anchor ratio guard, applied to every book before the best price is chosen. It replaced three drifted copies (Shin on O/U; a 180-min quote; a ceiling checked on the winning book only or not at all; a fair price frozen 10-40 min before matching). The seven trigger bots are rule_version r2 from that date; the O/U EARLY / TWO-ANCHOR rules replayed identical and are unchanged.
...
8. **Calibrated AH systematically loses (retired 2026-06-24):** `bot_ah_home_fav` accumulated 132 settled bets at -13.63% ROI (-€94 on €689 staked) from 2026-05-24 → 2026-06-24, then was retired (`is_active=FALSE`, `maturity_label='retired'`). Trajectory shows one good first week (+8.23% on n=30) followed by five losing weeks (-13% to -30% weekly) — not variance, real degradation. Hypothesis under investigation: handicap-line calibration isn't separating "home -1.0" / "home -1.5" / "home -2.0" cleanly; line-specific Platt fits may be needed. Bot continues to produce shadow_bets every 30 min (SHADOW-RETIRED-OK pattern); reactivation gated on ≥30 shadow bets at ≥3% ROI sustained.

9. **T-24h entry has higher edge than current cadence — coverage limited (day-ahead backtest 2026-06-24):** For the 15% of picks where odds_snapshots contains a T-24h snap, ROI is **+19.76%** vs production baseline **+9.80%**. The mid-horizons (T-12h, T-6h, T-2h) all underperform baseline. Implication: the model identifies mispriced lines best when entered ~24h pre-kickoff, before sharp money has narrowed the spread. **Constraint**: only 15% coverage today because `job_odds_tomorrow` runs once at 22:00 UTC. T24H-COVERAGE (2026-06-24) adds 3 new fetches at 04/10/16 UTC; target coverage 60-70%. If the +19.76% holds at expanded coverage, a "day-ahead" product tier becomes credible. Re-run the day-ahead backtest 4 weeks after the cron change. Reference: `dev/active/day_ahead_backtest_results.json`, `scripts/backtest_day_ahead_picks.py`.
