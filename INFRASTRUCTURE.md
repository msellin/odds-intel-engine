...
| **Mac launchd (Coolbet HTTP jobs)** | All jobs that hit Coolbet HTTP — cannot run on VPS (Imperva 403's the Linux Chrome fingerprint + Hetzner IP) | Free (operator's Mac) | Three LaunchAgents in `local/launchd/`: `com.oddsintel.coolbet-odds-snapshot` (:03/:33 bulk odds → `odds_snapshots`), `com.oddsintel.coolbet-ui-placer` (hourly, real money), `com.oddsintel.coolbet-feed-watchdog` (:20/:50, cookie refresh + session-keep), plus `com.oddsintel.near-kickoff-capture` (every 5 min, NEAR-KICKOFF-CAPTURE 2026-09-11 — direct-book closing prices for Coolbet/Unibet-Site/Epicbet by stored event id; Coolbet via `COOLBET_NO_FS` so it never shares the FlareSolverr session with the sweep or the real-money placer). (`coolbet-mac-daemon` RETIRED 2026-09-10; `cs2-coolbet-scanner` removed 2026-09-07.) Both scanners moved off VPS 2026-07-03 after silent 7-day 403 outage. `oi_local_flaresolverr` Docker on `localhost:8191` handles the CF/Imperva challenge with real-Mac fingerprint. Guardrail: `test_coolbet_scrapers_moved_to_mac` smoke test. |
