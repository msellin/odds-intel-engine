...
| **Mac LaunchDaemon `residential-egress`** | **RESIDENTIAL-EGRESS 2026-09-22** — WireGuard tunnel (VPS `10.8.0.1` ← Mac `10.8.0.2`, UDP 51820) plus `microsocks` on `10.8.0.2:1080`. Gives the VPS an OPTIONAL path out through the Estonian residential line. **Measured through it from the VPS: Epicbet `CF-CHALLENGE`→`OK`, Pinnacle-guest `CF-WAF-RULE`→`OK`** — so those become collectable server-side with plain requests. Note the reverse exists too: **BetVictor is OK from the datacenter and 403 from the residential line**, which is why selection is per-REQUEST via the proxy and there is deliberately **no `ip rule`/fwmark/NAT** on the VPS. | Free (operator's Mac + €0 VPS) | `scripts/ops/residential_egress_keepalive.sh`, `local/launchd/com.oddsintel.residential-egress.plist`. VPS: `/etc/wireguard/wg0.conf`, `ufw allow 51820/udp`. Plan: `dev/active/residential-egress-plan.md`. |
