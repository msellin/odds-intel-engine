...
BOTS: list[BotSpec] = [
    ...
    BotSpec("bot_high_roi_global_v2_newplus_v1", FAM_INTERNAL, "1x2", ANCHOR_MODEL,
            None, None, False,
            "High-odds match result — NEW MODEL (#152): twin of bot_high_roi_global_v2 with identical rules (Spain/Australia/Iceland, home/away, odds 1.50-5.50, same tier thresholds); only the probability is NEW+ (r1x2_comb_v1). Never a VIP-held pick. Live experiment — the original keeps its record; compare live. testing, on /performance, paper."),
    BotSpec("bot_v10_1x2_newplus_v1", FAM_INTERNAL, "1x2", ANCHOR_MODEL,
            None, None, False,
            "Match result — NEW MODEL (#152): twin of bot_v10_1x2 on the NEW+ model (r1x2_comb_v1), EV >= 3% flat, odds 1.30-4.50, never a VIP-held pick (so the EV 3-5% band). bot_v10_1x2 is unchanged (live CLV ~+4.7% Jul-Sep); the two are compared live after 50-100 settled picks. testing, shown on /performance, paper."),
