...
BOTS: list[BotSpec] = [
    ...
    # [[#191]] RETIRED 2026-09-26 (migration 480, owner "yes go with all 3" after the #191 audit): the per-book and
    # book-agnostic SHARP triggers bot_coolbet_trigger_sharp_1x2_v1 / bot_unibet_trigger_sharp_1x2_v1 /
    # bot_trigger_1x2_sharp_v1 (folded into bot_own_1x2_v1), bot_unibet_trigger_sharp_ou_v1 / bot_trigger_ou_sharp_v1
    # (folded into a future OWN O/U · SHARP bot), bot_coolbet_trigger_sharp_ou_v1 (100% duplicate) and
    # bot_unified_gate_1x2_paper_v1 (independent CLV −14.1%, n 241). All Pinnacle-only, no #182 guards.
    ...
    BotSpec("bot_ah_sharp_v1", FAM_INTERNAL, "asian handicap", ANCHOR_SHARP,
            None, None, False,
            "Asian handicap — sharp line (#187, 2026-09-26): an API-Football book's AH price beats Pinnacle's power-de-vigged fair price on the SAME line (whole / half / quarter) by EV 3-15%, same fetch, >= 45 min before kickoff, the book's own ladder consistent; one pick per match. Pre-registered (dev/active/ah-market-bot-prereg.md): CLV vs an independent close (other books, Pinnacle excluded) +5.4% discovery (n 223) / +3.0% holdout (n 185, p 0.0001); ROI unproven (+1.2% [-8.9, +11.1]). Quarter lines settle half win / half loss (migration 476). Job workers/jobs/ah_sharp_outlier.py. EXPERIMENTAL, paper, 👥 PICKS only (global books), no placement path."),
