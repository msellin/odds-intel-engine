...
def store_live_snapshot(match_id: str, snapshot: dict):
    ...
    optional_fields = [
        ...
        # INPLAY-BTTS-AH-BOTS (2026-05-24): added by migration 126.
        "live_btts_yes", "live_btts_no",
        "live_ah_main_line", "live_ah_home_odds", "live_ah_away_odds",
...
def bulk_store_shadow_bets(rows: list[dict], shadow_run_id: str, shadow_cohort: str) -> int:
    ...
    skips duplicates — including from rolling-restart double-fires where
...
def store_real_bet(
    *,
    ...
    (UI placer balance-delta / reconciled manual bet); FALSE = paper (historically
    the paper Mac daemon's record=True/execute=False — that daemon is deleted,
    #162 W4.6); None = caller didn't say (legacy). Callers on the real path must
    pass True.
