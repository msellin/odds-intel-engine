...
from workers.api_clients.supabase_client import (
    ...
    store_prediction, store_bet, store_team_season_stats, store_match_injuries,
...
def _load_today_from_db(today_str: str) -> tuple[list[dict], list[dict], dict[str, dict], dict[str, dict]]:
    ...
    odds_raw = execute_query(
        ...
        -- is what "best available" has to mean. (The retired pin-1x2
        -- line-shop pass used the same pattern; deleted in #162 W7.2.)
    ...
    for row in odds_raw:
        ...
        # allow-list, and they are different call sites: the placer re-derives at its own book's
        # live price before staking regardless. (The retired no-pin / sweep
        # shadow passes that also tested it were deleted in #162 W7.2.)
        if not is_publishable_book(bookmaker):
            continue
...
def run_morning(skip_fetch: bool = False, cohort: str | None = None,
            ...
            for mkt, selection, odds, raw_mp, cal_prob, ip, edge, kelly, alignment, odds_mv, stake, os_market, os_selection in bet_candidates:
                ...
                try:
                    ...
                    if bet_id:
                        ...
                        if config.get("one_per_match"):
                            _one_done = True
                        # #162 W7.2: the stats_only prediction_snapshots write is gone —
                        # the table does not exist (to_regclass = NULL) and nothing reads it.
    ...
    if shadow_mode:
        ...
        # #162 W7.2 (2026-09-26): the no-pin / sweep / pin-OU / pin-1x2 / Coolbet
        # line-shop shadow passes that used to run here are DELETED. Every bot
        # they wrote for was retired (2026-08-21..09-08), so each call returned 0.
    ...
    # #162 W7.2 (2026-09-26): the morning-cohort calls to the no-pin / sweep
    # shadow passes are deleted with the passes (every bot they served was
    # retired; see the shadow_mode block above).
...
def _get_bot_id_by_name(name: str) -> str | None:
    """Local helper — look up an ACTIVE bot's UUID by name (None when retired).
    Imported by workers/jobs/ou_sharp_outlier.py; kept when the shadow passes
    that first used it were deleted (#162 W7.2).
