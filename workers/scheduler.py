...
def settlement_pipeline():
    ...
    if is_refit_day:
        steps.append(("settlement_platt", "Platt recalibration", lambda: __import__('scripts.fit_platt', fromlist=['fit_and_store']).fit_and_store(model_version=__import__('workers.model.xgboost_ensemble', fromlist=['_resolve_version'])._resolve_version("1x2"))))  # #147: the 1X2 HEAD's version, not the global MODEL_VERSION
