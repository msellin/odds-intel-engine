...
| WC 2026 odds via Odds API | ✅ **ODDS-API-WC-DAILY-CRON 2026-06-06** — Daily 06:30 UTC Railway cron sweeps `soccer_fifa_world_cup` (3 credits/call). Gate auto-skips outside 2026-06-11 → 2026-07-19 window. Manual refresh 2026-06-09 inserted 6,235 rows / 22 books incl. Pinnacle/Bet365/Unibet (478/500 credits remaining). |
...
| Active production model | ✅ **`v20260607`** (since 2026-06-07) — 58,019 training rows (+1,752 vs v20260531 incl. June 6 CLV backfill). Beats v20260524_market: 1X2 −16.4% log_loss, AH −4.2%, BTTS −3.0%; OU +2.8% (TIER-C-EXPAND data composition drag — ongoing). Promoted 2026-06-07 via `scripts/promote_model.py`. **Railway env flip required: `MODEL_VERSION=v20260607`.** Performance page shows green "Model updated · Jun 7" callout for 7 days. **OU pin still pending** — ROADMAP target is `MODEL_VERSION_OU=v14_recreate_2026_05_11` (log_loss 0.8141 vs current 0.8258 in production) after 12-24h `SHADOW_MODEL_VERSION` shadow validation; see `OU-CLV-OPTION-B-FLIP-MODEL-VERSION-OU` in PRIORITY_QUEUE. Full history in `docs/MODEL_HISTORY.md`. |
