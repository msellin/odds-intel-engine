...
| `/performance` | ✅ Settled track record + per-bot leaderboard. **Since [[#159]] (2026-09-25):** every row's settled / W-L / ROI / P&L / CLV comes from ONE engine view (`bot_performance`, migration 433 — the same view /admin/bots projects): flat €10 at the best price available when the pick was made on all books, CLV vs the sharp close; **every reader** can open a row's detail view (chart + every pick); rows are grouped by status (calibrated & beta · testing · VIP · in development < 5 settled), no ROI-based "underperforming" group. **Anonymous** = 10-bet ledger teaser; **logged-in** = full filterable history (league / market / bot filters, 6-way sort, 200-row cap). Gate changed from `isPro` to `!!user` on 2026-08-21 (PERF-SIGNUP-HISTORY). |
...
| `near_kickoff_capture` (VPS systemd timer `oddsintel-near-kickoff-epicbet` since 2026-09-23; was Mac launchd) | every 5 min | NEAR-KICKOFF-CAPTURE 2026-09-11 — direct-book closing prices (Coolbet / Unibet-Site / Epicbet) for fixtures in the next 15 min, fetched by stored event id (`book_event_map`). Feeds own-book real-bet CLV (DIRECT-BOOK-CLV, migration 332). |
...
| Sunday cron chain (03-06:30 UTC) | ✅ **2026-06-15** — 03:00 weekly_retrain → 04:00 meta_retrain → 05:00 meta_validate → 06:00 weekly_threshold_check → 06:30 weekly_bot_review. Threshold check (`THRESHOLD-CHECK-WEEKLY 2026-06-06`) emails gate-count snapshot. Bot maturity review (`BOT-MATURITY-REVIEW-WEEKLY 2026-06-15`) emails per-bot 30/60/90d hit/ROI/CLV + PROMOTE/REVIEW/HOLD verdict (CHANGED 2026-09-26, #162 W6.5: PROMOTE = paper CLV/ROI t ≥ +1.65; REVIEW = the view `bot_review_flag`, the same flag /admin/bots shows — the old real-ROI thresholds and the job's own DEMOTE rules are gone). Closes the manual-promotion loop that let bot_high_alignment auto-place real money in the 2026-06-13 incident. |
...
| Pre-match bots (active) | ✅ **24 active** as of 2026-08-28 (was listed as 16 from 2026-06-15 — that list had gone stale, `bot_ah_home_fav` / `bot_greek_turkish` / `bot_opt_away_*` / `bot_high_alignment` / `bot_ou_specialist` / `bot_acca_leg_shadow` are all retired). **Calibrated (4)**: bot_v10_all, bot_1x2_specialist, bot_dnb_specialist, inplay_l. **Active (1)**: inplay_o. **Beta (13)**: bot_btts_all, bot_conservative, bot_high_roi_global_v2, bot_opt_home_lower, bot_proven_leagues_v2, bot_summer_specialist, inplay_a/b/g/h/m/q, inplay_btts_dryspell_v1. **Experimental (6)**: bot_btts_v2, bot_coolbet_value_v1, bot_pin_1x2_home_v1, bot_sweep_btts_yes_v1, bot_sweep_ou25_v1, bot_sweep_ou35_v1. Real-money gate: `COOLBET_RECORD_ALLOWED_MATURITY=calibrated`. Public Telegram + signed-out `/picks`: calibrated only — note the Telegram gate is **group-level** since SIGNALER-MATURITY-SHADOWING (2026-08-28): a pick publishes when ANY bot backing that (match, market, selection) is calibrated, not when the highest-edge bot happens to be. Gating on the canonical row had silently withheld 8 calibrated-backed picks in 60d. Signed-in `/picks`: calibrated+beta+active. **Promotion gate rebuilt 2026-08-28 (BOT-GATE-REACHABLE)** — the old gate required real-money history that only calibrated bots could produce, so it emitted 0 PROMOTE / 0 DEMOTE in 10 weeks; there is now a paper-evidence path and the shadow-only bots are visible to the review at all. See MODEL_WHITEPAPER 8.1b for the full ladder + thresholds, BOT-MATURITY-REVIEW-WEEKLY for the audit cadence. |
...
| Calibration | ✅ Per-tier Platt (1X2 1-feature, OU 2.5 2-feature). BTTS Platt fitted 2026-05-27 (offline holdout, n=139). DC + AH Platt fitted 2026-05-28 from live simulated_bets (`scripts/fit_platt_live.py`): `double_chance_1x` (n=58), `double_chance_x2` (n=105), `asian_handicap` aggregate (n=128). `apply_platt()` has `_MARKET_ROOTS` fallback for granular AH keys. Blend weights optimized. Dynamic DC rho. Weekly refit. **In-play calibration infrastructure (INPLAY-CALIBRATION-COMPLETE 2026-06-15; DELETED with InplayBot 2026-09-26, #162 W7.2)**: central `apply_platt` call in `_build_inplay_bet_data` + parameterized `scripts/fit_platt_inplay.py --strategy NAME`; canonical key shape `{bot}_{market}_{selection}` shared between read and write paths via `workers.jobs.inplay_bot.inplay_market_key()`. Every in-play bet writes `calibrated_prob` (equals raw until a Platt row lands for the key — honest no-op). Strategy E's existing `"inplay_e_under_25"` key preserved via `trigger["market_key"]` override. |
...
| /admin (Overview) | ✅ Admin dashboard (#139, 2026-09-24): 6 KPI cards, **attention inbox** (failed/stuck jobs via view `pipeline_job_latest` — mig 417 —, red feeds, stale status job, bot issues on the /admin/bots rules, armed money + CAN STAKE, unconfirmed manual real bets, DQ findings; unreadable sources shown as such), interactive charts (picks by family incl. retired, weekly mc-CLV by family, flat P/L, real bets, verdict + feed donuts). Shared components `src/components/oi/`. CS2/LoL/Tennis admin pages deleted |
| /admin/bots, Pick queue, Feeds, Jobs, Activity | ✅ #139 admin redesign 2026-09-24 on the shared design system (top bar with ⌘K + attention bell; DataTables with search/filter/sort/CSV). Pick queue = /admin/shadow-bots (picks + Place only; safety strip + scoreboard removed — their jobs are on /admin/bots). Jobs = /admin/ops (reads pipeline_job_latest; Email & alerts / Pro-Elite / stale bots sections removed; odds coverage, live tracker, AF budget moved to Feeds). /admin/activity new (control_changes ∪ feed_actions). /admin/shadow-bots/[bot] → redirect to the /admin/bots sheet. Bots with no pick yet show as "no picks yet". |
| /admin/bots | ✅ Superadmin unified bot board (#139 phase 1, 2026-09-24): every bot from all three ledgers via `bot_scoreboard` (mig 410), grouped by family, each judged only on its admissible metric; config drawer from `bot_config`; Retired tab; **control panel (phase 3-A, mig 413): fleet pause/arm, per-bot real-money eligibility + Show on /picks, audit log, Mac placer heartbeat** |
| Coolbet real-money risk limits | ✅ **COOLBET-MATCH-EXPOSURE-GUARD 2026-09-01** — real-money rows are written by the **UI placer** (`scripts/place_coolbet_ui.py` + `coolbet_ui_placer.py`), not `coolbet_placer.py`, and it had **no per-match state at all**: `MAX_BETS_PER_DAY=20` / `MAX_STAKE_PER_DAY=200` were the only limits. On 2026-08-31 that produced 19 bets / EUR 190 / **-EUR 92.80**, including both sides of one 1X2 (Colwyn Bay, the model flipped mid-day and emitted a new UUID the `shadow_bet_id` dedup could not recognise) and three correlated bets on one 6-2 scoreline. Now: **max 2 bets per match, max 1 per market family** (`result` = 1X2/DC/DNB/AH · `totals` = the whole OU ladder · `btts`), plus `MAX_STAKE_PER_MATCH=20`. Exposure collapses both `real_bets` vocabularies (`over_under_25`/`over` and `o/u`/`over 2.5`), excludes combos (placeholder match_id), and is tracked in-memory across a pass because same-pass placements are seconds apart. Blocks are recorded as `coolbet_placement_attempts` rows (`stage='exposure_guard'`), never silently skipped. Whitepaper 8.3.7. |
| /admin/place | ⛔ RETIRED 2026-09-24 (#139 IA P8b) — replaced by the Pick queue (/admin/shadow-bots). Was: Coolbet auto-place candidate table. **PER-MARKET-EDGE-V2 2026-06-06**: badge gates per market (1x2 ≥10%, o/u ≥3%, AH ≥5%, BTTS ≥10%, DC retired) instead of a flat 5% floor. Mirrors the placer's per-market thresholds. |
| /admin/real-bets | ↪️ **MOVED 2026-09-26 (#162 W6.8)** — now the **Real money** tab of /admin/bots (`/admin/bots?section=money`, `admin/bots/money-view.tsx`); the old URL redirects there and left the sidebar. Content unchanged: **Real bets** (money ledger, #139 redesign 2026-09-24: KPI cards, P/L charts, CSV tables, caps with by-hand split, unconfirmed manual bets, promotions). Real-money placement log + ROI/CLV stats. **PER-MARKET-EDGE-V2 2026-06-06**: split into Era v1 (pre `MARKET_THRESHOLDS_V2_EPOCH = 2026-06-06T17:00:00Z`) vs Era v2 (post) so the threshold-change lift is measurable in isolation. Daily breakdown row highlighted at the epoch. |
