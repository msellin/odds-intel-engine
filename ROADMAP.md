...
| Active production model | ✅ **`v20260607`** (since 2026-06-07) — 58,019 training rows (+1,752 vs v20260531 incl. June 6 CLV backfill). Beats v20260524_market: 1X2 −16.4% log_loss, AH −4.2%, BTTS −3.0%; OU +2.8% (TIER-C-EXPAND data composition drag — ongoing). Promoted 2026-06-07 via `scripts/promote_model.py`. **Railway env**: `MODEL_VERSION=v20260607`. **OU per-tier routing** (OU-CLV-OPTION-B-RE-EVAL 2026-06-12): per-tier OU 2.5 holdout (n=219) showed v20260607 beats `v14_recreate_2026_05_11` on T1 by **+49pp ROI@+5%** (45.23% vs −4.09% on n=180); T2/T3+ holdout too thin to falsify. Routing shipped via `_resolve_version("ou", tier=tier)` reading `MODEL_VERSION_OU_T{1|2|3}` → `MODEL_VERSION_OU` → `MODEL_VERSION`. Railway envs: `MODEL_VERSION_OU_T1=v20260607` (promotes global on T1), `MODEL_VERSION_OU=v14_recreate_2026_05_11` (T2/T3+ default, unchanged from 2026-06-07). Full history in `docs/MODEL_HISTORY.md`. |
