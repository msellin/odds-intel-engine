...
| **Tonybet** (EE-licensed, Osaühing Tonybet; 20bet is the same platform and prices) | **Added 2026-09-23 (#101).** Anonymous REST `platform.tonybet.com/api/event/list` through the zone.ee Estonian exit; scheduler job `tonybet_odds_snapshot` :01/:31, next 48 h pre-match main board (~6 requests). Matched 188 of 252 DB fixtures on the first dry run — more than any other direct book. Stored in the shared vocabulary keyed on **Sportradar UOF ids** (`vendorMarketId`/`vendorOutcomeId`): `1x2`, `over_under_*` — **every FT and 1H/2H goals line since #130 (2026-09-24)**: .5 lines as before (`over_under_25`), whole and quarter lines under their own label (`over_under_20`, `over_under_225`; `handicap_line` carries the number; settlement refuses to grade a quarter label rather than guess), `asian_handicap` (lines where both sides price 1.25–4.0), `btts`, `double_chance`, `draw_no_bet`. **Every outcome carries Sportradar's margin-free probability** → `book_fair_probs` (migration 383, latest value per key = fair close). **History since 2026-09-26 ([[#154]] idea 3, migration 474):** the FIRST value in each checkpoint — open / ≤24 h / ≤3 h / ≤1 h before kick-off — also goes to `book_fair_probs_history` (≤4 rows per line), so the anchor test can align Tonybet with Pinnacle at the same instant. **In-play boards since #130 (2026-09-24):** the 120 s live-stats request also returns odds → `inplay_book_quotes` book='Tonybet' (all goals-total lines, 1x2, AH, BTTS; zero extra requests). **Raw archive:** every response gzipped to `/opt/oddsintel/raw/tonybet/YYYY/MM/DD/` (~60 MB/day) so later parsers can rebuild history. Placeable (`ACCESSIBLE_BOOKMAKERS`) since the 2026-09-23 site check (14/15 prices identical); part of the public forward test's "all books" universe since 2026-09-23 (owner's go-ahead). **Phase 1b (2026-09-23):** full market board (`main=0`, ~120–140 types) for our matched fixtures at ~24 h / 3 h / 30 min before kickoff and at the close (near-kickoff timer) → also `team_total_*`, `team_total_1h_*`, `1x2_1h`, `1x2_2h`, `over_under_1h/2h_*`, `btts_1h/2h`, `double_chance_1h`, `corners_*` (total, team, 1H, handicap) and **`bookings_*`** (Sportradar bookings — deliberately NOT filed as `cards_*`: counting rule unproven). Player/goalscorer, correct score, combos and interval markets stay in the raw archive only. **Phase 2:** live score/clock/status/corners/cards for EVERY live football event every 120 s → `book_live_stats`; FT/HT/2H results + final corners/cards every 2 h → `book_match_results` (migration 385; Tonybet purges results after ~1–2 days). Design: `dev/active/tonybet-sweeper-plan.md`, capability map `dev/active/tonybet-af-capability-map.md`. | ✅ Active (pre-match + live + results, VPS) |
| **Optibet** (EE-licensed, Optiwin OÜ; Enlabs in-house trading "betex") | **LIVE 2026-09-26 (#101) — scheduled :06/:36 UTC (`job_optibet_odds_snapshot`) + near-kickoff close; collection only.** Anonymous REST on the sportsbook's own backend `ensb-trading.optibet.ee/en/…` (the JSON the bet.optibet.ee iframe reads): `/groups` → `/events/group/{id,id,…}?gameTypes=1` (comma list of groups, 40 per call) → `/events/{id,id,…}?gameTypes=…` (comma list of matched events, 25 per call) — **~14–21 requests per 48 h sweep**. `workers/automation/optibet_feed.py` (`python3 -m workers.automation.optibet_feed [--dry-run]`). Stores, through the shared guarded writer + `book_event_map`: `1x2`, `double_chance`, `over_under_*` (every line), `asian_handicap` (2-way handicap, HOME line — verified on the site: "Albaania (-0.5) 1.55"), `btts`, `btts_1h/2h`, `draw_no_bet`, `1x2_1h/2h`, `double_chance_1h`, `over_under_1h/2h_*`, `team_total_*` / `team_total_1h_*`, `corners_ou_*` / `corners_1h_ou_*` (~49 rows per fixture). Market period comes from the English title (one type id covers 1st half / 2nd half / regular time). No cards/bookings pre-match. **Measured 2026-09-26 (from the Mac):** 463 pre-match events in 48 h, **373 matched** to our fixtures, 18,395 rows/sweep; 24/24 UEFA Nations League 1X2 triples identical to the rendered site, O/U, BTTS, DC, AH and 1H O/U spot-checks identical; favourite agrees with Tonybet 331/337, Pinnacle 269/282; median 1X2 price 3.0% shorter than Pinnacle, 1.7% shorter than Tonybet. 210 test rows written for 6 fixtures (2026-09-27 07:00–09:30 UTC). **The 2026-09-26 block, resolved the same day:** the 403 page ("contact support@optibet.ee") was answered to the bare Hetzner IP; through the zone.ee Estonian exit (217.146.76.113, `EPICBET_RESIDENTIAL_PROXY`) `/groups` answered 200 at 16:10, 18:10 and 20:45 UTC, so the feed was scheduled — and it has `auto_pause`, so repeated failures back off instead of keeping a block fresh. The Hetzner IP is never used for Optibet. **Collection only** — not in `ACCESSIBLE_BOOKMAKERS` (placeable needs the owner's account + a site-price check). Joined `board_guard.DIRECT_BOOKS`, `health_alerts.DIRECT_FEED_BOOKS`, the near-kickoff close (ONE batched request per 5-min run) and the /admin/feeds card. | 🟢 Live (collection only) |

> **Coolbet draw-no-bet (#132, 2026-09-24).** Coolbet's full-match `draw no bet` (market_type_id 70 — seen 1,335 times in `coolbet_market_inventory`, never stored before) is now written as `draw_no_bet` home/away, the vocabulary Bet365 / Betano / BetVictor / Unibet-Site / William Hill / Tonybet already use; `1st half draw no bet` (427) is deliberately not captured. Settled by `settlement._r_draw_no_bet` (draw = push).

> **Book side-tables — who reads them (#112 item 7, 2026-09-24).** `book_match_results` (Tonybet regular-time results) is READ by `workers/jobs/results_check.py` — the AF-vs-Tonybet cross-check and its tie-break. `book_live_stats` (Tonybet live score/corners/cards every 120 s) is read only by `tonybet_feed` itself (the last pre-whistle snapshot is the only durable record of final corners/cards); its consumer-to-be is #105 (settling corners/cards where AF has no stats). `book_fair_probs` (Tonybet's Sportradar fair probabilities) has **no reader** — an archive, kept because it is a free second opinion on fair price that #119/#121 may test as an anchor; nothing prices off it today.
| **The Odds API — Coolbet** (licensed fallback) | **Scheduled 2026-09-24 (#110 step 4) — every 2 h at :50, a no-op unless `coolbet_prematch` is paused.** `workers/automation/odds_api_fallback.py` reads Coolbet (`coolbet`, region `eu`) 1X2 + the MAIN goal total from the-odds-api.com with the key stored as `OA_KEY`, and stores as bookmaker **`Coolbet-OddsAPI`** — deliberately NOT in `ACCESSIBLE_BOOKMAKERS`, so it cannot price a pick or a stake until the owner compares it with our own Coolbet rows and promotes it. **Coverage is thin:** measured 2026-09-24, 7 Coolbet-priced events in the next 48 h across 6 live competitions (5 matched to our fixtures) vs hundreds from the own sweep; EPL 10 of 20 events. **Credits:** the key is on a ~500/month plan (233 left 2026-09-24); a free `/events` pre-check means only competitions with a kickoff in the horizon are paid for (2 credits each, ~12 per sweep that day), and a run stops at a 60-credit floor. A paid, licensed source used during an outage — not a way round a block. | ✅ Scheduled (emergency only) |
...
| Optibet | ~42 expected (scheduled 2026-09-26) | 90 (60 until scheduled) | **Bulk, comma lists**: 1 groups call + ~5 multi-group listings (40 groups each) + ~8–15 multi-event boards (25 events each) = ~14–21 per sweep, 2 sweeps/h; the near-kickoff close is ONE comma-list request per 5-min run (≤12/h). |
...
reserve (Tonybet 40%, Coolbet 20%) for the close, live stats, results and the health ping: Tonybet deep
boards fall back to the main board; the Coolbet sweep keeps a far-off fixture's last price (fixtures
< 3 h out are still fetched every pass). A deferral is not a refusal and is not booked as one. Tonybet's budget was raised 150 → 250 the same day
(bulk API, no known block threshold; 150 had been 2× a weekday peak, not a weekend one).
...
1. **Pinnacle vs Betfair Exchange anchor** (7,328 paired matches) — identical to 4 decimals (Brier 0.5886/0.5887, LogLoss 0.9862). **Keep Pinnacle anchor** (CAL-PIN-SHRINK). *Extended 2026-09-24 ([[#119]] step C, ANALYSIS_GOTCHAS §78): Shin-de-vigged, paired t-tests, O/U 2.5 added, blend + agree-filter — still no difference (EXC−PIN LL t=+0.83 1X2 / +1.27 O/U); the exchange close is a Pinnacle-grade peer, and a close-to-close sharp-trigger replay shows no anchor variant beating Pinnacle (6 tests, Holm p=1.000).*
