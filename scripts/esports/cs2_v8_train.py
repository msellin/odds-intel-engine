"""
Train the v8 stacking model on full history + save coefficients to DB.

v8 = v7 stacking features + team K/D diff (via roster aggregation OR direct
team stats from /stats/teams bulk page). Coverage went 10% → 40% after the
bulk-page integration; v8 lifts AUC by ~+0.5pp on full sample and ~+2pp on
the K/D-covered subset.

Coefficients live in cs2_model_coefficients (model_version='v8') so the
production scorer cs2_v8_predict.py can apply them on every scan.

Run:
    python3 scripts/esports/cs2_v8_train.py --since 2025-06-01
...
# Feature order is FROZEN — cs2_v8_predict.py reads these from DB and
...
def main():
    ...
    print(f"=== v8 training  {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M')} UTC ===")
    ...
    print(f"  coverage:  pistol_diff={pistol_n}/{len(rows)}  kd_diff={kd_n}/{len(rows)}")
    ...
    execute_write("""
        ...
        VALUES ('v8', 1.0, 0.0, %s, %s::jsonb, %s, %s, %s, %s, NOW(), 'cs2_v8_train.py')
    ...
    print(f"\n  saved → cs2_model_coefficients[model_version='v8']")
