...
- **Every scheduled job runs its work INSIDE `_run_job`** (#034, 2026-09-24): `job_<name>()` = `_run_job("<name>", _job_<name>_impl)`. That wrapper is what writes the `pipeline_runs` row (failed on an exception, with the message), pushes Uptime Kuma, and honours a feed pause. Never do the work first and then call `_run_job(name, lambda: None)` — 20 jobs did, which made a crash invisible (no row) and a success a no-op row; smoke `OBS-LOG-ALL-JOBS` forbids it.
