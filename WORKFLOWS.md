...
:14/:44  O/U Sharp Outlier     job_ou_sharp_outlier()    Every 30 min ([[#149]], 2026-09-25). Soft books' latest O/U 1.5/2.5/3.5 quotes (≤ 3 h old) vs Pinnacle's power-de-vigged fair price: EV 5–15% → `bot_ou_sharp_early_v1` (≥ 12 h before kickoff) and `bot_ou_sharp_2anchor_v1` (also beats the other books' consensus by ≥ 2%). One pick per (match, line) per bot. Paper → `simulated_bets`. `workers/jobs/ou_sharp_outlier.py`.
