...
Telegram public channel posting — every calibrated-maturity pre-match pick (1x2/OU/BTTS, no AH) auto-posts to `@oddsintelpicks` via `workers.notify.telegram.send_telegram_public()` from inside `coolbet_signaler.py` (PUBLIC-CHANNEL-POST hook, 2026-06-24). **SIGNALER-PUBLIC-ONLY + PUBLIC-CHANNEL-DECOUPLED (2026-09-11): this is now the ONLY sink, and it is independent of our own staking.** Two fixes: (a) the candidate query no longer excludes picks already in `real_bets` — it used to, which silently denied customers the picks we had backed with real money, i.e. our highest-conviction ones (8 publishable picks lost in 30d, edges 0.08-0.15; placing every pick on a given day would have left the channel EMPTY); (b) the public post is no longer nested inside the operator send's success branch, where an operator-side dedup-skip or missing creds dropped a customer pick with nothing logged. The owner's private per-pick prompt is OFF by default (`SIGNALER_OPERATOR_PROMPT`) — the placer is unattended now, so it duplicated every public pick; kept behind the flag for a possible paid invite-only channel. Beta/active/experimental picks have no sink while it is off and are deliberately left unmarked, so they can still publish if a calibrated bot joins their group before kickoff. NB the owner's chat still gets per-pick `[OI] 🎯 PRE-MATCH` alerts from `daily_pipeline_v2` — a SEPARATE path, untouched. No separate cron.
