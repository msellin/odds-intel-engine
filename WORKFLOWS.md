...
- **Results cross-check** every 2 h at :40 (`job_results_check`): API-Football final score vs Tonybet's regular-time result (matchStatusId 100 only) → `results_disagree` finding + Telegram listing the bets already settled on the match. Does NOT hold settlement (owner decision pending).
