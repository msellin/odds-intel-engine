...
05:30/17:30  1X2 Rating Shadow  job_rating_1x2_shadow()  Daily ([[#141]], 2026-09-24), subprocess. Walk-forward rating pass over every finished match + `rating_history_results`, fits the D8+ logit, writes `rating_1x2_predictions` (`r1x2_d8plus_v1`) for fixtures in the next 2 days; then refits the COMBINED model's per-group combiner on matches since 2026-05-01, stores it in `combiner_1x2_params` and writes `r1x2_comb_v1` rows. SHADOW ONLY — nothing stakes or publishes from it (except bot_rating_1x2_v1, experimental, which reads r1x2_d8plus_v1). ~3 min, ~1.5 GB. MODEL_WHITEPAPER §4.4/4.4b. **+ [[#152]] the COMBINED O/U model** (`workers/model/combined_ou.py`, `ou_comb_v1`): fits the O/U combiner on the same rating pass (finished matches since 2026-05-01) → `combiner_ou_params`, prices upcoming fixtures → `ou_model_predictions` (served `p_over` = Pinnacle where it prices the line, else combined); the :10/:40 refresh re-applies it at current prices.
:10/:40  Combined 1X2 Refresh  job_combined_1x2_refresh()  Every 30 min ([[#141]] round 3b), subprocess. Re-applies the latest stored combiner to CURRENT bookmaker prices (+ API-Football where unpriced) for fixtures that already have a rating prediction; upserts `r1x2_comb_v1`. No rating pass, no refit — seconds.
