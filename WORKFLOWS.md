...
Sun 02:00 ㊴ Competitor audits + fallback refresh  .github/workflows/competitor_audits_weekly.yml  COMPETITOR-AUDIT-WEEKLY 2026-06-25 — re-runs `scripts/audit_vs_*.py` (WinnerOdds live GraphQL pull; SignalOdds/DeepBetting/Forebet/Tipstrr re-audit existing snapshots) → refreshed `ledger/comparison_*.json`. LANDING-COMP-COHORT-DISCLOSURE + AUTOREFRESH 2026-07-06: after the audit commit, checks out `msellin/odds-intel-web` via `FRONTEND_REPO_TOKEN` PAT and runs `scripts/update_frontend_comp_fallback.py` to rewrite the hardcoded `COMP_FALLBACK` dict in `src/app/page.tsx` from the fresh audit JSONs — keeps the landing's last-resort fallback within one week of reality. No-ops cleanly if PAT unset (audit still commits engine-side).
