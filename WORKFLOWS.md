...
- **Optibet** — collector built 2026-09-26 (#101, `workers/automation/optibet_feed.py`), **NOT scheduled**: both VPS egresses get Optibet's 403 block page, and working round an IP block is not done. Tested by hand from the operator's Mac: `python3 -m workers.automation.optibet_feed [--dry-run]` — ~14–21 requests, next-48 h pre-match, 1X2/O-U/AH/BTTS/DC/DNB + halves, team totals, corners → `odds_snapshots` + `book_event_map`. Scheduling is the owner's decision (DATA_SOURCES.md → Optibet).
