...
:00/:30 LivePrice Backfill apply_backfill()        pick-time price producer (SCHEDULED-LIVE-PRICE-PRODUCER 2026-09-08; #159 2026-09-25): fills BOTH price bases — odds_at_pick_live (our 4 books, /admin/bots) and odds_at_pick_available (all publishable books, /performance) — for settled bets AND picks from the last 2 days still NULL, via workers/utils/pick_price.py (store_bet also calls it right after each insert). Only touches NULL rows.
22:30  HalfScore Sweep    backfill_half_scores      1H-HT-GOALS (2026-09-08): fills matches.ht_score_*/h2_score_* for finished matches still NULL, from AF /fixtures score.halftime+fulltime (20/call). The stale-match finish path writes HT via update_match_result, but the LIVE POLLER's finish_match_sql writes FT only — so settlement's `_ht_score` fetches HT from AF on demand for finished 1H-bet matches (HT-SCORE-FETCH-ON-SETTLE 2026-09-11); this is the safety-net sweep. NULL-only → cheap once caught up.
...
*/5 24/7  ㊵ Closing snap  run_closing_snap()        CLOSING-LINE-COVERAGE 2026-06-24 — per-fixture AF /odds for matches in T-15→T+5, stored with is_closing=TRUE.
                                                    ...
                                                    AF books only. The DIRECT-book close (Epicbet / Unibet-Site / Tonybet / Coolbet — Coolbet re-added 2026-09-24 after the 09-23 Imperva back-off) is the VPS systemd timer
...
02:30  ⑮ Pinnacle Drift Refresh  job_pinnacle_drift_refresh()  Daily. Runs `scripts/backfill_pinnacle_drift.py --since <today-3d>` to keep `match_feature_vectors.pinnacle_drift_*` populated. DRIFT-FEATURE-WRITER-2026-09-11: the script existed but was never registered, so the column was last written 2026-06-06 and sat at 0% coverage for three months while its queue row waited on a coverage trigger that could not fire. 3-day rolling window because the write is an idempotent UPSERT, so a missed night self-heals. Sits after the 01:00 settlement pass (closing prices land there) and before the 03:00 odds prune. NOTE: post-hoc feature, deliberately NOT in the training set — see MODEL_WHITEPAPER §3.1b.
03:00  ⑭ Weekly Retrain  job_weekly_retrain()      Sunday only — trains `v{YYYYMMDD}` (all data) AND a `v{YYYYMMDD}_cut14` twin (`--cutoff` today−14d, same flags), then `weekly_eval_and_compare.py {twin} {production}` scores the twin on the 14 days it never saw and emails the verdict. Promote the full `v{YYYYMMDD}` when its twin wins; promotion stays manual (operator flips `MODEL_VERSION_*` env). Changed 2026-09-24 (WEEKLY-EVAL-NO-HOLDOUT, [[#141]]): evaluating the bundle trained through today left no honest window, so the eval exited 2 and sent nothing on every run from 09-06. (It called `compare_models.py` until WEEKLY-EVAL 2026-05-24.)
05:30/17:30  1X2 Rating Shadow  job_rating_1x2_shadow()  Daily ([[#141]], 2026-09-24), subprocess. Walk-forward rating pass over every finished match + `rating_history_results`, fits the D8+ logit, writes `rating_1x2_predictions` (`r1x2_d8plus_v1`) for fixtures in the next 2 days; then refits the COMBINED model's per-group combiner on matches since 2026-05-01, stores it in `combiner_1x2_params` and writes `r1x2_comb_v1` rows. SHADOW ONLY — nothing stakes or publishes from it (except bot_rating_1x2_v1, experimental, which reads r1x2_d8plus_v1). ~3 min, ~1.5 GB. MODEL_WHITEPAPER §4.4/4.4b. **+ [[#152]] the COMBINED O/U model** (`workers/model/combined_ou.py`, `ou_comb_v1`): fits the O/U combiner on the same rating pass (finished matches since 2026-05-01) → `combiner_ou_params`, prices upcoming fixtures → `ou_model_predictions` (served `p_over` = Pinnacle where it prices the line, else combined); the combined refresh chained into every betting run re-applies it at current prices ([[#176]]). **#191 (2026-09-26, migration 479):** both prediction tables keep only the latest value, so every write (fit and refresh) ALSO appends to the private append-only `model_prediction_history` — pre-kick-off only, and only when the probabilities changed — for point-in-time backtests (MODEL_WHITEPAPER §4.4b).
(chained)  Combined 1X2 + O/U Refresh  betting_pipeline.refresh_served_probabilities()  **No own cron since 2026-09-26 ([[#176]])** — runs as the FIRST step of `run_betting()` (morning chain step 7 and every :05/:35 betting refresh), subprocess, pipeline_runs job `combined_1x2_refresh`. Re-applies the latest stored NEW+ combiner and O/U combiner to CURRENT bookmaker prices (+ API-Football where unpriced); upserts `r1x2_comb_v1` and `ou_comb_v1`. No rating pass, no refit — ~5 s. **Why chained:** at :10/:40 it ran five minutes AFTER the bots decided, so every NEW+/combined-O/U decision read a ~25-min-old probability computed before the :00/:30 odds it was compared with (measured over the 7 days to 2026-09-26, reconstructed: 44/91 NEW+ EV5, 9/108 NEW+ EV3 and 21/26 O/U-model picks would not have cleared their EV floor on the fresh probability). Order is now a property of the code, not two cron minutes. The pipeline also ignores any combined row older than `SERVED_P_MAX_AGE_MIN` (20 min), so a failed refresh idles those bots instead of feeding them a stale p. `job_combined_1x2_refresh()` is kept for manual runs only.
:14/:44  O/U Sharp Outlier     job_ou_sharp_outlier()    Every 30 min ([[#149]], 2026-09-25). Soft books' latest O/U 1.5/2.5/3.5 quotes (≤ 3 h old) vs Pinnacle's power-de-vigged fair price: EV 5–15% → `bot_ou_sharp_early_v1` (≥ 12 h before kickoff) and `bot_ou_sharp_2anchor_v1` (also beats the other books' consensus by ≥ 2%). One pick per (match, line) per bot. Paper → `simulated_bets`. `workers/jobs/ou_sharp_outlier.py`. Since 2026-09-26 (#162 W7.5) each run also writes its near-floor decisions (EV vs Pinnacle ≥ 0) to `candidate_funnel` as `source='ou_sharp'` — `accepted` / `drop_edge` / `drop_ev_cap` / `drop_odds_too_low|high` / `drop_too_late` / `drop_no_consensus` / `drop_consensus_edge` / `drop_one_per_match`, one row per (bot, match, line, side) per day, the book that got furthest (~150 rows per run, upserted). `bot_ou_sharp_early_v1` (VIP) picks go to the Pro/Elite DMs + VIP channel through `pick_sender.send_vip_pick` (#162 W5.3: pause, `bot_distribution.vip_channel`, `pick_sends`). Since #162 W7.6 (2026-09-26) both bots are `SharpRule` configs of the ONE sharp engine (`workers/automation/sharp_engine.py`); the job keeps its own loader, write and funnel. Replayed identical over 2 days before the move — rule_version stays r1.
:18/:48  AH Sharp Outlier      job_ah_sharp_outlier()    Every 30 min ([[#187]], 2026-09-26). The third market (👥 PICKS). API-Football books' Asian-handicap quotes vs Pinnacle's power-de-vigged fair price on the SAME line, same fetch (≤ 5 min apart, Pinnacle's newest fetch ≤ 60 min old): EV 3–15%, ≥ 45 min before kickoff, the book's own neighbouring lines consistent (ladder guard) → `bot_ah_sharp_v1`, one pick per match, ever. Every line type (whole / half / quarter). Selection stored as side + HOME-perspective line (`away -0.75`); quarter lines settle half win / half loss (`settle_fraction`, migration 476). Public price = the pick's own price. Paper → `simulated_bets`, EXPERIMENTAL. `workers/jobs/ah_sharp_outlier.py`, a `SharpRule` of the one sharp engine (rung key `asian_handicap:<line>`, `anchor.ah_line_market`). `clv_sharp` and `pick_price` price AH legs on their own rung since #187.
:05      Pick Triggers (Stage A) job_pick_triggers()  Hourly. Writes SHARP trigger windows to `pick_triggers` (de-vigged Pinnacle via the sharp engine's `anchor_fair` + `window`; the model strategies were deleted in #162 W7.2). Since #162 W7.6 (2026-09-26) the SHARP windows are a record for research scripts only; no bot reads them. `workers/jobs/pick_triggers.py`.
:15/:45  Pick Trigger Matcher  job_pick_trigger_matcher()  Per-book sharp bots (`BOOK_MARKET_BOTS`: Coolbet / Unibet-Site sharp 1x2 + O/U 2.5, and the tight instrument over 4 books). Since #162 W7.6 (2026-09-26) each (book, market, strategy) is decided by the ONE sharp engine at match time on `pick_triggers.sharp_rule(strategy, book)` — no longer by joining the :05 windows (rule_version r2). Paper → `shadow_bets`, per-book cohort, first write wins. `workers/jobs/pick_trigger_matcher.py`. The two merged sharp bots (`bot_trigger_1x2_sharp_v1` / `bot_trigger_ou_sharp_v1`) run through the same engine from `pick_generator` (`_generate_sharp`) on every odds arrival (`on_odds_written`), best price across Coolbet + Unibet-Site (r2).
08:00  ⑫ Weekly Digest   [DELETED 2026-09-26] run_weekly_digest()  Never had an add_job() registration (removed SCHEDULER-CLEANUP 2026-07-07); `workers/jobs/weekly_digest.py` and the unregistered `job_weekly_digest` were deleted in #162 (audit C §7). History is in git.
...
:05/:35 ⑨ Betting Refresh betting_refresh()         Every 30 min, 5 min after odds refresh, 24/7 (WC-OVERNIGHT-COVERAGE 2026-06-12 — was 07:05–22:35 UTC). DB-only, 0 AF calls. Dedup prevents duplicates. Cohort auto-detected from UTC hour. **First step since 2026-09-26 ([[#176]]): the combined 1X2 + O/U refresh** (see the chained row above), so the NEW+ / combined-O/U probabilities are written from the same odds the bots then price. **Since 2026-09-23 ([[#082]]) every run also writes its near-floor and gate-rejected candidates to `candidate_funnel`** (one row per candidate per day, latest decision wins, 90-day retention).
:05/:35 PICKS Forward Test job_publish_picks_forward_test() **The ONLY scheduled job that writes to a PUBLIC surface** (@oddsintelpicks Telegram) + `picks_forward_test`. Every 30 min since RULE-V4-2026-09-15 — it ran once daily at 10:00 UTC for a single day, and a daily run structurally could not see 47% of qualifying legs (candidate window is now+45min..now+14h) and never saw 00:00-03:00 kickoffs. Safe at this cadence only because of PUBLISH-CLAIM-BEFORE-SEND (claim before send, so no duplicate messages) and daily_room() (a cap counted across runs). Since 2026-09-23 ([[#082]]) each pass also writes both arms' near-floor legs to `candidate_funnel` with their fate (selected / selected_unsent / below_floor / above_ceiling / deduped_or_capped). Since 2026-09-25 ([[#161]]) the pass also claims two RECORDED, NEVER-SENT twin arms last (`record_twin_arms`, never raises): `sharp_own_book_aligned` (live v4 + own-book quote ≤ 5 min from the anchor) and `consensus_pin_confirmed` (consensus v2 + EV ≥ 0 vs a fresh tight Pinnacle) — same pool, same room, own-arm dedupe, not in `PUBLISHED_ARMS`. The 4h-out floor from the backtest is the constraint that matters; this cadence only ever publishes EARLIER, never nearer kickoff. Since 2026-09-26 (#162 W5.3) each send goes through `pick_sender.send_pick` (pause + `bot_distribution` re-checked, fail closed; one `pick_sends` row per claimed published-arm leg, sent or skipped with the reason; DB dedupe) and still stamps `picks_forward_test.telegram_message_id`.
...
:50    Corners Paper Settle [RETIRED 2026-09-26, #162 W1.3] job removed. corners_ou_* legs are now graded by the generic shadow settler (settle_ready, every 15 min) from match_stats corner counts (`settlement._corner_stats` → `_r_corners_ou`), which also writes the own-book close + CLV the old self-settler never did. The >1-day no-stats auto-void moved to `settlement.void_ungradeable_corners_bets` (same reason string). Same for the team-total (:55) and first-half 1x2 (:57) settle jobs — one shadow settlement path.
01:40    CLV vs sharp close  job_clv_sharp()  [[#024]] Scores every newly settled leg of simulated_bets / shadow_bets / picks_forward_test against a FRESH (≤60 min at kickoff), same-moment (±2 min) Shin-de-vigged Pinnacle close → `leg_clv_sharp` (migration 386). One CLV definition across all ledgers; reported BESIDE the forward test's pre-registered own-book decision variable, never instead of it.
                                                    ALSO (#113, 2026-09-23, migration 393): the same leg against a ≥5-book CONSENSUS close (workers/utils/anchor.py; Pinnacle and the leg's own book excluded) → p_close_cons / clv_cons / cons_status, for every leg incl. those with a Pinnacle close. Fills ~1/3 of the non-AH legs with no Pinnacle close; clv_sharp's definition is untouched. Runs 0.9 pts BELOW clv_sharp on average (r 0.976) — never pool the two.
02:20    xG late fill        job_xg_late_fill()  [[#111]] Since ~2026-08-31 API-Football adds `expected_goals` 1-4 days AFTER a match; settlement fetches statistics once, hours after kickoff. Re-fetches finished matches 36 h - 6 d old whose match_stats row still has no xG, in leagues that carried xG in the last 90 days (~100-500 `fixtures/statistics` calls/day). COALESCE upsert — never overwrites a stored value with NULL. Backfilled from 2026-08-25 on 2026-09-24.
                                                    ALSO (#116, 2026-09-24, migration 394): legs with NO ≥5-book close but 3–4 books get the same measure against that THIN close in separate columns (p_close_cons_thin / clv_cons_thin / cons_thin_n_books / cons_thin_status='ok_thin') — a leg carries at most one of clv_cons / clv_cons_thin. MEASUREMENT ONLY: no gate or staking path reads it. Quality and caveats: ANALYSIS_GOTCHAS §74.
8/12/16/20:27 First-Half 1x2 Paper Pick  job_fh_1x2_paper_pick()  Retired bot bot_1h_1x2_paper_shadow_v1 still writes paper picks (best Epicbet/Betano/Unibet 1H 1x2 price vs de-vigged Pinnacle) → shadow_bets. **Since 2026-09-23 ([[#103]]) it then calls verify_epicbet_picks(): each new Epicbet pick with edge ≥3% is re-fetched live from Epicbet by its book_event_map id and the site's price is stored in price_verifications (migration 382)** — the automated check that the recorded price was really clickable.
:57    First-Half 1x2 Paper Settle [RETIRED 2026-09-26, #162 W1.3] job removed — 1x2_1h legs are graded by the generic shadow settler (`_r_1x2_1h`, half-time score, fetched from AF when missing).
...
23:15  ㉗ Season phase   job_league_season_phase()       LEAGUE-SEASON-PHASE 2026-05-25 — per-match season_progress [0..1] → match_signals (+7.7pp Over 2.5 late vs early)
        ⚠️ ㉕–㉗ wrote NOTHING 2026-09-07 → 2026-09-24: SIGNALS-DEDUPE-BACKLOG called the dedupe guard on `tuples` before building it (UnboundLocalError, 17 nights). Fixed in SCHEDULER-DEAD-JOBS-2026-09-24; guard: smoke of the same name.
...
Mon 08:15 ㉜ Book price fidelity  job_book_price_fidelity()  BOOK-PRICE-FIDELITY-MONITOR-2026-09-16 — weekly per-book check that a recorded price is a price the book actually offered. Compares ACTUAL win rate on settled bets against the rate the recorded price implies; margin guarantees an edgeless bettor lands BELOW implied, so a book sustaining a gap above its peers was quoting prices longer than the truth, and every edge/ROI/CLV figure computed on it is inflated by that much. ⚠️ Threshold is RELATIVE to the fleet median gap, never to zero: a positive gap is ALSO what genuine line shopping produces (~+4 to +5 points in the healthy 1.05-1.25 band per OUTLIER-CEILING-CALIBRATED), so an absolute "gap > 0" alert fires on the behaviour we want and gets muted. Weekly not daily — the test needs n>=100 per book. Read-only: changes no price, gate or pick. First live run flagged Bet365 at +16.8 gap (n=1,087, won 75.3% where its price implied 58.4%).

:04/:34 ㉛ [RETIRED 2026-09-15] Unibet (Kambi) odds — ⚠️ WAS REFERENCE ONLY, NOT A PLACEABLE PRICE, and job_unibet_kambi_odds() is now NO LONGER REGISTERED (UNIBET-KAMBI-RETIRED). The feed was removed from ACCESSIBLE_BOOKMAKERS on 2026-09-06 (KAMBI-FEED-DIVERGENCE: unibet.ee left this API; 38% of stored prices read HIGHER than the site, median +3.3%, max +23.5%) and then kept writing for nine more days — ~815k rows into the largest table in the DB, read by NOTHING: it appears in daily_pipeline_v2, settlement, bot_inventory, lineshop_new_markets, own_line_movement, own_movement_snapshot, own_sharp_config_sweep and publish_picks_forward_test exclusively as an EXCLUSION. Its one remaining advantage was corners/cards breadth, now taken from the true site feed by UNIBET-SITE-MARKET-WIDENING. Honest cost: Kambi reached ~3,011 fixtures vs Unibet-Site's ~2,026, so this trades Unibet-brand breadth for prices that are actually obtainable. The module and job function are KEPT for manual runs. Guard: smoke UNIBET-KAMBI-RETIRED.
01:00  ⑧d Settlement      settlement_pipeline()     Overnight catch-up: 21:30+ KOs finishing after extra time
1st 03:30 ㉙ ALN auto    job_aln_auto_tune()             ALN-AUTO 2026-05-25 — monthly alignment-bump retune; emails diff if any class needs |Δ|≥0.005 with n≥100. Never succeeded until 2026-09-24 (ImportError on a function-local `_ALN_BUMP`; fixed in SCHEDULER-DEAD-JOBS-2026-09-24) — first real run 2026-10-01.
...
Sun 02:00 ㊴ Competitor audits + fallback refresh  .github/workflows/competitor_audits_weekly.yml  COMPETITOR-AUDIT-DAILY 2026-07-19 (bumped to daily) — re-runs `scripts/audit_vs_*.py` against `dev/active/*.json`. Now that the Sunday-01:00 scrape workflow refreshes those inputs weekly, each Monday's audit shows moved competitor `their_stats` instead of the same-forever numbers we had 2026-06-24 → 2026-08-01. LANDING-COMP-COHORT-DISCLOSURE + AUTOREFRESH 2026-07-06: after the audit commit, checks out `msellin/odds-intel-web` via `FRONTEND_REPO_TOKEN` PAT and runs `scripts/update_frontend_comp_fallback.py` to rewrite the hardcoded `COMP_FALLBACK` dict in `src/app/page.tsx` from the fresh audit JSONs — keeps the landing's last-resort fallback within one week of reality. No-ops cleanly if PAT unset (audit still commits engine-side).
...
Sun 02:00 [RETIRED 2026-09-24] Prune anon users  job_prune_anon_users()  ANON-AUTH PHASE 4 — NO LONGER REGISTERED (SCHEDULER-DEAD-JOBS-2026-09-24). It deleted idle anonymous rows from `auth.users`, which stayed in Supabase at SUPABASE-TO-VPS, so it failed every Sunday from 2026-07-12. Not repointed: the web app no longer creates anonymous users and the FK cascade it relied on cannot span two databases. Listed in `retired_jobs` (migration 426) so /admin Jobs and the attention inbox stop showing its last failed run. **When you unregister a job, add a `retired_jobs` row in the same commit** — scheduler job ids and logged job names are different vocabularies, so retirement cannot be inferred.
...
Sun 06:30 ㉟ Bot review   job_weekly_bot_review()         BOT-MATURITY-REVIEW-WEEKLY 2026-06-15 — runs scripts/weekly_bot_review.py and emails per-bot 30/60/90d hit/ROI/CLV/sim-vs-real-divergence + PROMOTE/REVIEW/HOLD verdict. Closes the manual-promotion loop that let bot_high_alignment (beta, -€56 over 50 real bets) auto-place real money in the 2026-06-13 incident. **Since 2026-09-26 (#162 W6.5) REVIEW is read from the view `bot_review_flag` (migration 437: ≥ 50 legs with a sharp-anchor CLV, upper 95% CI < 0) — the same flag /admin/bots and the Overview inbox show**; the job's own DEMOTE rules (t ≤ −1.65 on its own CLV coalesce, and the real-money ROI tripwire) are gone, so the email and the page cannot disagree. PROMOTE keeps its t ≥ +1.65 test; promotion stays a manual migration.
...
[DELETED 2026-09-25] 10s  Manual placement drain  _drain_manual_placement_queue()  #162 W4.6 — drained Telegram "Record at Coolbet" taps into coolbet_placer.place_bet_by_id (pinned paper). Deleted with the API placer, the button and the webhook enqueue; the queue had never held a row.
[DELETED 2026-09-25] :03/:33  Coolbet daemon healthcheck  job_coolbet_daemon_healthcheck()  #162 W4.6 — de-registered 2026-09-10 with the paper Mac daemon; module workers/jobs/coolbet_daemon_healthcheck.py now deleted.
...
Telegram public channel posting — ⚠️ **CHANGED 2026-09-26 ([[#174]], owner): ONE public-Telegram rule for BOTH senders (`bot_status.public_channel_skip_reason`, enforced in `pick_sender.send_pick`): every ACTIVE pick (BETA/CALIBRATED until they were merged into ACTIVE, [[#175]] 2026-09-26) + TESTING picks at EV ≥ 7% (**raised from 5% on 2026-09-26, [[#184]]**, to match /picks; bot probability × published odds − 1); below it the pick stays on /picks and is recorded `skipped · testing_below_ev5` in `pick_sends`. The signaler's public path NO LONGER applies the Coolbet real-money edge floors (13pp 1x2 / 8pp O/U) — they gate only the (off-by-default) operator prompt via `clears_placement_floor` — and its public markets now include O/U 1.5 / 3.5. The forward-test job passes each leg's `edge` as its EV.** ⚠️ **CHANGED 2026-09-25 ([[#155]] ONE STATUS DECIDES DISTRIBUTION): the gate is now "any bot in the group whose STATUS sends" (`bot_distribution.sent_public` = TESTING/BETA/CALIBRATED, not VIP), not "calibrated only"; a sent bot's row supplies the message. The forward-test publisher gates the same way (`arm_bot_sends`; grade D's bot is EXPERIMENTAL → recorded, never sent). Read "calibrated" below as "status sends".** Originally: every calibrated-maturity pre-match pick (1x2/OU/BTTS, no AH) auto-posts to `@oddsintelpicks` via `workers.notify.telegram.send_telegram_public()` (since #162 W5.3, 2026-09-26: via the one audited sender `workers/notify/pick_sender.send_pick`, which records a `pick_sends` row with the message id) from inside `coolbet_signaler.py` (PUBLIC-CHANNEL-POST hook, 2026-06-24). **SIGNALER-PUBLIC-ONLY + PUBLIC-CHANNEL-DECOUPLED (2026-09-11): this is now the ONLY sink, and it is independent of our own staking.** Two fixes: (a) the candidate query no longer excludes picks already in `real_bets` — it used to, which silently denied customers the picks we had backed with real money, i.e. our highest-conviction ones (8 publishable picks lost in 30d, edges 0.08-0.15; placing every pick on a given day would have left the channel EMPTY); (b) the public post is no longer nested inside the operator send's success branch, where an operator-side dedup-skip or missing creds dropped a customer pick with nothing logged. **OPERATOR-PICK-ALERTS-OFF (2026-09-11): per-pick messages to the owner's PRIVATE chat are off by default, on BOTH paths, behind the single `OPERATOR_PICK_ALERTS` flag** — `coolbet_signaler`'s manual-placement prompt AND `daily_pipeline_v2`'s `[OI] 🎯 PRE-MATCH` alert. Together they were sending 16 private messages for 10 picks; the placer is unattended now, so both duplicated the public feed. One flag governs both on purpose — two env vars for one intent is how the edge floors reached six copies. Kept rather than deleted for a possible paid invite-only channel; the inline buttons (`sigplaced:`/`sigskip:`) and their webhook handlers are untouched and work again the moment it flips. (The "Record at Coolbet" button, its `manual_placement_queue` enqueue and the 10 s VPS drain were DELETED 2026-09-25, #162 W4.6.) Summaries, health/silence alerts and placement confirmations are NOT affected, and neither is the `send_telegram_to_users` broadcast (different audience). Beta/active/experimental picks have no sink while it is off and are deliberately left unmarked, so they can still publish if a calibrated bot joins their group before kickoff. No separate cron.

**Pause semantics (PICKS-PUBLISH-DECOUPLED-FROM-OWN-PAUSE, 2026-09-15 — supersedes SIGNAL-PAUSE-DECOUPLE, 2026-08-27).** Publishing and placement are now two switches, and **no kind of placement pause mutes the customer channel**:
...
Publishing is notification-only — no Coolbet API call, no `real_bets` write — so it is safe while placement is down, and `is_publishing_paused()` falls *open* for the same reason: a transient DB error must not silently mute customers, and it cannot stake money. ⚠️ **CHANGED 2026-09-26 (#162 W5.3):** the SEND itself now goes through `workers/notify/pick_sender.send_pick`, which reads the pause (and the bot's `bot_distribution` row) itself and fails CLOSED on an unreadable read — that one pick is not sent and a `pick_sends` row says why (it is retried next pass where the caller retries: the signaler leaves `signaled_at` unset; a forward-test claim is not re-sent). `is_publishing_paused()` keeps falling open for its logging/pre-check callers. The pause now also stops the **VIP channel and the Pro/Elite DMs**, which ignored it before. Every send / skip / failure is one row in **`pick_sends`** (migration 457, private), unique on (channel, pick) — the DB dedupe that replaced the VIP DMs' in-memory 600 s key.

**This coupling cost picks twice before it was fully split.** (1) 2026-08-23: a daemon self-pause silenced every Telegram signal for 4 days / 12 picks with nothing erroring. SIGNAL-PAUSE-DECOUPLE fixed the self-pause branch and deliberately left the operator branch coupled. (2) 2026-09-14 12:05 UTC: migration 343 set `placement_paused` to close the 🤖 OWN automated-betting product — a decision about what *we* stake — and the surviving branch armed a silent outage of the 👥 PICKS customer feed. It cost nothing only because the last pick had been generated 23h earlier and the slate stayed flat; the next qualifying pick would have vanished with no error anywhere. `is_daemon_self_pause()` survives because it still governs pause auto-clear.

**Direct-feed staleness (DIRECT-FEED-STALENESS, 2026-09-16).** `health_alerts.check_direct_feed_staleness()` runs **hourly at :50, all 24 hours** (`job_health_alerts_feeds`) and alerts on **Coolbet / Unibet-Site / Epicbet** — the three books we collect on our OWN crons — when a book's newest `odds_snapshots` row is older than `DIRECT_FEED_STALE_AFTER_MIN` (120 min). Email **and** Telegram.

Two design points, both load-bearing, both learned from an outage the existing guard slept through:

* **It measures AGE, not volume.** `check_bookmaker_disappearance()` (09:35 UTC) compares 24h row counts against a book's prior daily rate and is *structurally blind to any outage shorter than its own window*. Replayed at 07:00 UTC on 2026-09-16, with Coolbet ~6h dark and Unibet-Site ~20h dark, it read **Coolbet 245%** (it had been writing *above* its prior rate) and **Unibet-Site 5.8%** (missing the 5% floor by 0.8pp) — silent on both. The two checks are complementary, not redundant: volume catches AF dropping books for days, age catches our own collectors dying for hours.
* **It is NOT windowed.** It has its own runner rather than joining the hourly 10-22 bundle, because that bundle is scoped to live-match hours and the outage began at **01:25 UTC** — nine hours before a windowed check could have looked.

Threshold is measured, not chosen: 7-day inter-write gaps are p99 **59.2 min** (Coolbet), **56.0** (Unibet-Site), **62.0** (Epicbet) — one missed 30-minute cron. 120 min is two missed ticks plus slack.

`check_bookmaker_disappearance()` also now **alerts on the transition, not the state**: a book whose last row is older than 48h is reported in the mail body and the console but does not raise a new alert. It was re-sending the same five long-dead AF books (10Bet, 888Sport, Dafabet, Superbet, Unibet) every day, and five standing alerts is how the sixth gets skimmed past.
...
**Verification queue (VERIFY-QUEUE, #168a, 2026-09-25).** `job_verify_queue` → `workers/jobs/verify_queue.py`, **every 30 min at :10/:40, 24/7** (just after the :05/:35 betting refresh). Tasks no longer wait for a deploy to check its live effect: they commit the check to **`ops/verify/<task>.yml`** (`sql` or `cmd`, `expect` like `>= 1` / `== 0` / `true`, `mode: now | eventually | invariant`, `run_after: {migration: NNN | time: ISO | job: <pipeline_runs job_name>, since: ISO}`, required `expires`), or put a `-- verify: <boolean SQL>` line in their migration (due once applied, 48 h window, also printed PASS/FAIL non-fatally by `migrate.yml` right after apply). Each due check runs on its own connection with `default_transaction_read_only=on` + 20 s `statement_timeout`, single SELECT/WITH only. State is in **`verify_results`** (migration 447, private): `passed`/`expired` are terminal, an edited spec restarts, `invariant` re-runs every cycle until expiry. **Telegram goes to the operator chat only** on a transition into `failed` / `error` / `expired` — one message per run listing them, never for a pass. Spec files stay in git (the VPS checkout is a deploy target — nothing commits from it); delete old ones by hand when convenient. Local: `python3 -m workers.jobs.verify_queue --validate` (parse) / `--dry-run` (run due checks, write nothing). Pipeline row: `pipeline_runs.job_name='verify_queue'`, `records_count` = checks run.
...
| `com.oddsintel.coolbet-ui-placer` | **UNLOADED 2026-09-15** (was hourly 06–21 UTC `--all-enabled --execute`) | **The real-money path.** UI-driven Coolbet placement. Parked in `~/Library/LaunchAgents/paused/` while OWN is paused (mig 343) and DISARMED (mig 354 `real_money_armed=FALSE`); every executor now calls `placement_gate.assert_may_place()` first (OWN Phase 0). Reload only after Phase 3 arming. | ✅ repo |
...
| ~~`com.oddsintel.residential-egress`~~ + `residential-socks` | **⛔ RETIRED 2026-09-23** — superseded by the zone.ee Estonian exit (`oddsintel-zone-egress` on the VPS, `socks5h://127.0.0.1:1081`); nothing referenced `10.8.0.2` any more. VPS `wg-quick@wg0` disabled and UFW 51820 closed; config kept in `/etc/wireguard/`. Mac daemons booted out by the operator (sudo) and plists parked in `~/Library/LaunchAgents/paused/`. Was: every 120s + at boot | **RESIDENTIAL-EGRESS 2026-09-22.** LaunchDAEMON (root — `wg-quick` configures an interface). Keeps the WireGuard tunnel (`10.8.0.2`) + `microsocks` on `10.8.0.2:1080` alive, so the **VPS** can reach Epicbet/Coolbet through this machine's Estonian residential line with plain requests instead of FlareSolverr. The VPS side is `OI_RESIDENTIAL_PROXY` and has **no fallback** by design — if this dies, VPS collection FAILS loudly rather than collecting from the Hetzner IP. Not a full-tunnel VPN: `AllowedIPs = 10.8.0.0/24`, the Mac's own default route is untouched. | ✅ repo |
...
| `com.oddsintel.best-price-router` | **UNLOADED 2026-09-15** (was :20 / :50 `--execute`) | `best_price_router --execute` — **REAL MONEY at the best book** (Coolbet or Unibet-Site). ⚠️ The 2026-09-12 cutover set `ROUTER_ALLOW_REAL=true` in `.env`, so it ran in REAL mode every 30 min through the 09-14 "pause" and staked nothing only because it found no candidates; and it iterated `PLACEABLE_BOTS`, never the DB toggle. Since OWN Phase 0 the env var is no longer sufficient: the run-level gate (`placement_paused` + `real_money_armed` + the money-gate contract, migration 436 — locked until #162 W4) and the DB allowlist gate every dispatch, incl. the Unibet arm. Parked in `~/Library/LaunchAgents/paused/`. |
| `com.oddsintel.vps-postgres-tunnel` | KeepAlive | `autossh` tunnel to the VPS Postgres. | ⚠️ **no repo plist** |
| ~~`com.oddsintel.inplay-coolbet-collector`~~ | **⛔ NOT LOADED (verified 2026-09-24, #112)** — the plist still sits in `~/Library/LaunchAgents` but `launchctl list` does not show it, and the last `inplay_book_quotes` row with `book='Coolbet'` is from 2026-09-18 (533 rows). In-play betting is retired and the suspension study it served was closed by the #131 audit. Was: KeepAlive (90 s cycle, ≤8 fixtures, **serial**) | **INPLAY-SUSPENSION-LEAD-SIGNAL (2026-09-18).** `workers.jobs.inplay_coolbet_collector` — Coolbet live board → `inplay_book_quotes` with `book='Coolbet'`, beside the Epicbet arm. **Not a breadth collector:** it reads only fixtures that are AF-live AND Coolbet-mapped in `book_event_map`, so every row has an Epicbet row to compare against. Exists to answer whether Coolbet still quotes when Epicbet suspends. **~6 s/fixture** (FlareSolverr + Imperva) vs Epicbet's 0.13–0.8 s, hence the slower cadence. **SERIAL by construction** — parallel reads through one FS session silently return another match's markets. Uses its own FS session `coolbet_inplay`; **never `coolbet_prod`** (real-money placer). Places NOTHING. Pinned by `COOLBET-INPLAY-*` smoke tests. | ✅ repo |
| ~~`com.oddsintel.inplay-collector`~~ | **⛔ MOVED TO THE VPS 2026-09-22** | Now `oddsintel-inplay-collector.service` on the VPS, via the residential egress. Epicbet 403s the VPS (`CF-CHALLENGE`, live endpoint too), so before the tunnel every call there needed FlareSolverr — which was already dropping ~15% of pre-match Epicbet runs. Through the tunnel the VPS uses **plain requests** and returned *identical prices* to the Mac in the same minute. Cutover verified: continuous rows, **0 duplicates**, longest gap 46s (= the 45s cadence). Plist parked in `~/Library/LaunchAgents/paused/`. ⚠️ `inplay_book_quotes` has **no unique constraint** — never run both. | ✅ repo |
...
| Label | Runs | Purpose |
|-------|------|---------|
| ~~`com.oddsintel.coolbet-mac-daemon`~~ **RETIRED 2026-09-10, code + plist + keepalive DELETED 2026-09-25 (#162 W4.6)** | — | Paper daemon (`execute=False`) — retired as legacy. Its operator heal-button drain (`_drain_operator_commands`) now lives in `workers/jobs/coolbet_feed_watchdog.py`. Real money = `coolbet-ui-placer`; paper sim = pipeline `simulated_bets`/`shadow_bets`; session-keep (JWT heal) moved to `coolbet-feed-watchdog`. See COOLBET_RUNBOOK. |
...
- **Betfair Exchange** read at :04/:19/:34/:49 UTC (`job_betfair_exchange_snapshot` → `workers/automation/betfair_exchange_feed.run_bulk`, #117, 2026-09-24): 1 listing request (all football MATCH_ODDS + O/U 2.5 in 48 h) + prices 40 markets/request (~11 requests), through the London exit `oddsintel-egress@betfair` (:1082). Back/lay/sizes/matched volume → `exchange_quotes` (migration 395), NOT odds_snapshots. A reference, never placeable. Raises when 20+ events matched and 0 rows stored.
- **Tonybet** ingested at :01/:31 UTC (`job_tonybet_odds_snapshot` → `workers/automation/tonybet_feed.run_bulk`, #101, 2026-09-23): next-48 h pre-match main board, ~6 requests via the zone.ee exit; odds → `odds_snapshots`, Sportradar fair probabilities → `book_fair_probs`, raw pages → `/opt/oddsintel/raw/tonybet/`. Raises on zero events or zero rows stored; staleness watched by `health_alerts.DIRECT_FEED_BOOKS`.
- **Optibet** ingested at :06/:36 UTC (`job_optibet_odds_snapshot` → `workers/automation/optibet_feed.run_bulk`, #101, live 2026-09-26): next-48 h pre-match, ~14–21 requests via the zone.ee exit (the bare Hetzner IP gets Optibet's 403 page — never used), 1X2/O-U/AH/BTTS/DC/DNB + halves, team totals, corners → `odds_snapshots` + `book_event_map`, raw pages → `/opt/oddsintel/raw/optibet/`. Raises on zero events or zero rows stored; `auto_pause` on the feed; staleness watched by `health_alerts.DIRECT_FEED_BOOKS`; close captured by the near-kickoff timer in one batched request. **Collection only** — not in `ACCESSIBLE_BOOKMAKERS`.
- **Data-quality guards** (#120/#121, 2026-09-24). WRITE time, in every direct-book writer (`store_book_odds_snapshots`, `store_odds`, the Coolbet board writer), before the 1X2 mirror guard: `workers/utils/board_guard.screen_board` refuses a book's WHOLE board when >=2 of 1X2 / O/U 1.5-3.5 / BTTS are >1.5625x AND >4 prob-pts from a >=4-book median (identical feeds count once; a self-scraped book that independently agrees corroborates; Pinnacle never judged; peers <=6 h old) — another match's prices; two-way markets stored the wrong way round are refused on their own; an over_under_* row whose `handicap_line` is not its label's line is dropped (`odds_quality.ou_line_matches`). Refused rows → `odds_snapshots_quarantined`, detections → `data_quality_findings` (migration 398), shown on /admin/feeds → Data quality, counted as DISTINCT problems (same match + book + kind = one; the checks re-flag the same price every 30 min) — the Overview inbox uses the same rule (`dqAdvice`, web `admin-feeds-model.ts`) and only asks for a look when a problem was NOT set aside automatically.
- **Board audit** at :25/:55 (`job_board_audit` → `workers/jobs/board_audit.py`): re-screens boards already stored (kickoff now-3 h … now+48 h) — a board written before enough other books priced the fixture passed every write-time guard with no evidence. Pre-filter on latest boards, then a verdict per snapshot (grouped by a 120 s gap) against peers AS OF that snapshot; offending snapshots are MOVED to quarantine (copy incl. original id + is_opening, then delete, one transaction). Telegram when a book has >=3 wrong-fixture boards in 24 h.
- **Coolbet licensed fallback** every 2 h at :50 (`job_odds_api_fallback`, #110 step 4): does nothing unless `coolbet_prematch` is paused; then reads Coolbet 1X2 + main total from The Odds API (`OA_KEY`) into bookmaker `Coolbet-OddsAPI` (not placeable). Free `/events` pre-check, stops at a 60-credit floor — see DATA_SOURCES.
- **Results cross-check** every 2 h at :40 (`job_results_check`): API-Football final score vs Tonybet's regular-time result (matchStatusId 100 only) → `results_disagree` finding + Telegram listing the bets already settled on the match. **Tie-break (2026-09-24):** a disagreement gets a third read — AF re-fetched now + ESPN where covered; two agreeing against the stored score → `matches` corrected and settled bets re-graded (`results_corrected` finding); ESPN backing AF → `af_confirmed`, no change; otherwise alert-only. Does NOT hold settlement (owner decision pending).
- **Observatory metrics** daily 02:15 (`job_observatory_metrics`, migration 400 `obs_metric_values`): CLV coverage per ledger (legs on matches of the last 7 d), per-book 1X2 price fidelity vs Pinnacle (last 24 h, both quotes <=60 min pre-KO), 7-day CLV per bot (in-play bots excluded), and (#13) per-book **frozen-quote share** (≥6 snapshots of one price in the final 12 h while Pinnacle moved >8%) and **last-quote lag p90** (minutes from a book's last pre-KO quote to kickoff — first reading: Coolbet 239 min, Tonybet 59, every AF book ~5). A value >=3 robust-z from its own 28-day baseline (>=7 days, min n) → `metric_shift` finding. Levels never alert.
- **Model accuracy** daily 02:40 UTC (`job_model_accuracy` → `workers/jobs/model_accuracy.run`, [[#153]], migration 466): scores every production probability source on settled matches — old `predictions` ensemble per model_version (1X2 + O/U 1.5/2.5/3.5), `rating_1x2_predictions` NEW / NEW+ (gated), `ou_model_predictions` ou_comb_v1 (model and served) and Pinnacle's de-vigged close — over 7 / 30 / 90-day windows, ONLY probabilities written before kickoff: n, log-loss, Brier, the window's base-rate log-loss, and Pinnacle on the same rows. Writes the private table `model_accuracy` (upsert per model/market/window) read by /admin/models. ~1 min. Manual: `python3 -m workers.jobs.model_accuracy [--dry-run]`.
- **OWN board** every 10 min, 24/7 (`job_own_bet_board` → `workers/jobs/own_bet_board.run`, [[#182]], migration 470): every pending pre-match pick of a non-retired bot (`bot_ledger`), grouped per (match, market, selection), priced at each `ACCESSIBLE_BOOKMAKERS` book vs the v2 ANCHOR (`workers/utils/anchor.py`: Pinnacle + Betfair Exchange blend / either alone, a Pinnacle-vs-exchange conflict = NO price; else a ≥5-book consensus WITHOUT the book being priced; else wide Pinnacle — `anchor_source` on every row, migration 471), gated by the sharp engine's `price_refusal` (60-min book freshness, 8% ceiling, outlier cap, wrong-fixture guard) and a MARKET-SPLIT guard (median of our books' de-vigged lines > 8 pts from the anchor on any side = no price, for EVERY market of that match — the 1X2 split counts match-wide; normal gap 0.6 pts, p90 2.2) and DE-VIG ROBUSTNESS (the edge must also clear 3% with the anchor's members re-de-vigged by POWER — San Marino v Finland draw "+5.4%" was a Shin longshot artefact: power 13–20, Sportradar fair 17.4). The sharp BOTS stay Pinnacle-only until #119. Per book: odds, age, EV, refusal; plus best clearing book, `take_at` = (1 + 3%) / fair (the lowest price still worth taking — to match at another book). REPLACES the private table `own_bet_board` in one transaction; read by the admin OWN board (/admin/shadow-bots). Seconds. Manual: `python3 -m workers.jobs.own_bet_board`.
- **OWN bot** every 10 min, 24/7 (`job_own_bots` → `workers/jobs/own_bots.run`, [[#182]], migration 473): `bot_own_1x2_v1` — every 1X2 selection kicking off in 3 min…3 h, priced like the OWN board (v2 anchor, sharp-engine gates) at Coolbet / Unibet-Site / Epicbet / Tonybet; a pick needs EV ≥ 3% and ≥ 3 other books whose median fair probability is ≥ 0.97× the anchor; the best such book is recorded. Paper (`shadow_bets`, cohort `own`, first decision is the pick). Manual: `python3 -m workers.jobs.own_bots [--dry-run]`.
- **Bot config export** daily 03:40 (`job_export_bot_config` → `scripts/export_bot_config.run`, #139 phase 1, migration 410): writes one `bot_config` row per bot — active, retired, the forward-test arms and `control_junk_anchor` — from the config objects the running code imports (floors, odds bands, gates with `file:line` sources, books, family, admissible metric). Read by the admin-only views `bot_ledger` / `bot_scoreboard` / `bot_capabilities`. A bot whose config is no longer in code gets `family='unknown'`. It is not re-run on deploy yet — after a config change, run `python3 scripts/export_bot_config.py` (`--dry-run` prints JSON).
- **Exchange quotes prune** daily 03:10: after 2 days, captures more than 6 h before kickoff are thinned to the first per hour; near-kickoff captures kept in full.
- **Feed health** every 5 min (`job_feed_health` → `workers/jobs/feed_health.run_feed_health`, #107): evaluates every feed in `workers/registry/feed_registry.py` into `feed_status` / `feed_book_stats` (migration 388) for **/admin/feeds**. Book sweeps are judged on DATA WRITTEN, live/results jobs on runs, services on systemd/docker state. Coolbet and Epicbet sweeps now FAIL when they store nothing for 20+ fixtures (they used to record `completed` — #108).
- **Feeds card — Betfair Exchange** (2026-09-24): `feed_health` counts the exchange's rows from `exchange_quotes` (it never writes `odds_snapshots`, so the card used to read "0 prices stored") and writes `feed_book_stats.liquid_today/_yesterday` (migration 403) — fixtures whose latest 1X2 is LIQUID (spread ≤5% every runner, ≥€1k matched, `betfair_exchange_feed.is_liquid`), shown beside the LISTED count. First day: 91 fixtures → 65 listed → 19 ≥€1k matched → **9 liquid**; median €295 matched.
- **Every scheduled job runs its work INSIDE `_run_job`** (#034, 2026-09-24): `job_<name>()` = `_run_job("<name>", _job_<name>_impl)`. That wrapper is what writes the `pipeline_runs` row (failed on an exception, with the message) and pushes Uptime Kuma; it also honours a feed pause, but only for jobs registered with a pause control in `workers/registry/feed_registry.py`. Never do the work first and then call `_run_job(name, lambda: None)` — 20 jobs did, which made a crash invisible (no row) and a success a no-op row; smoke `OBS-LOG-ALL-JOBS` forbids it.
- **Feed controls** (#107 phase B): `/admin/feeds` writes pause / resume / run-now requests to `feed_controls` (audit in `feed_actions`, migration 389). `_run_job` skips a paused feed's job (cached 30 s — skipped, not failed); `job_feed_control_drain` every 30 s turns run-now into a one-off run of the feed's own wrapper (not logged to pipeline_runs itself; the run it starts is). **Auto-pause (circuit breaker, migration 391):** the four bot-protected book sweeps (`auto_pause` in the registry) pause themselves after 2 failed runs in a row, then auto-resume with ONE test run after a backoff of 1 h → 2 h → 4 h → 8 h → 12 h cap; a failed test re-pauses with the doubled backoff, a success resets it. Only engine-set pauses auto-resume — an operator's pause is never overridden. Why pause and not restart: #108 showed a full FlareSolverr restart does not clear a bot-protection flag on the exit IP. **Request footprint (#110, migration 392):** every request to Coolbet / Tonybet / Unibet-Site / Epicbet is counted per clock hour in `book_footprint` by `workers/utils/footprint.py` (shared across scheduler, near-kickoff timer and in-play collector) and refused before sending once that book's hourly budget is spent (Coolbet 500, Tonybet 350 (150 until 2026-09-25, #151; 250 until 2026-09-26 — Saturday must-run traffic ~136/h outgrew the 40% reserve and starved tonybet_live), Unibet-Site 400, Epicbet 1200, Epicbet-inplay 3000, Betfair-Exchange 150 — ~2× the measured peak, 2026-09-24; env `FOOTPRINT_BUDGET_<BOOK>`; Coolbet POSTs — the placement path — are counted, never refused). Each batch is booked under the clock hour it was COUNTED in (FOOTPRINT-HOUR-BOOKING, 2026-09-24 — refusals at xx:59 used to land in the next hour); "budget spent" is only claimed when the hour really is at its budget, refusals under budget are named as such, with the refusing `host/proc/pid` from `book_footprint.refused_by` (#151, migration 445). **Priority reserve (#151, 2026-09-25):** deferrable requests — Tonybet deep boards in `run_bulk`, the Coolbet sweep's refresh of fixtures ≥ 3 h out — check `footprint.has_headroom()` and skip once the hour is past its deferrable share (reserve: Tonybet 40%, Coolbet 20%), so the near-kickoff close, live stats, results and health ping still fit after a busy sweep; the weekend 09-25 slate had them refused. `/admin/feeds` reads `book_footprint` itself and names the hour whose budget ran out. `feed_health` copies this hour's requests / budget / bot-checks / errors and 24 h requests into `feed_book_stats` and turns the book's pre-match block amber at 80% of budget or a bot-check share ≥5% — the early warning #108 lacked. `feed_health` sends Telegram on TRANSITIONS only — a feed newly red, or recovered — never for a paused feed, with a link to /admin/feeds.
- **Tonybet live stats** every 120 s (`job_tonybet_live` → `tonybet_feed.run_live`, 1 request): score/clock/status/corners/cards for every live football event → `book_live_stats`. **Since #130 (2026-09-24) the same request also carries the in-play odds board** (`relations[]=odds`, zero extra requests) → `inplay_book_quotes` book='Tonybet': every goals total (FT/1H/2H/team, ALL lines incl. whole and quarter), 1x2, AH (1.25–4.0 band), BTTS; a board identical to the event's previous one is skipped; written after the stats commit and fails the job loudly if it cannot be stored. `minute` is often NULL (Tonybet blanks the clock) — join AF minute on match_id + time. **Tonybet results** every 2 h at :20 (`job_tonybet_results` → `run_results`): FT/HT/2H + final corners/cards (falling back to the last live snapshot) → `book_match_results`. Tonybet's full board is also fetched at T-24h/T-3h/T-30m inside the 30-min sweep, and at the close by the near-kickoff timer (`--books …,Tonybet`).
...
nightly, 15-min catch-up) and settles EVERY arm — `live`, `consensus_anchor`, the
unpublished `junk_anchor` negative control and the two unpublished [[#161]] twin arms
(`sharp_own_book_aligned`, `consensus_pin_confirmed`) — through the same code path with
no branch on `arm`; `clv_sharp` (`leg_clv_sharp`) is arm-agnostic the same way. (The 2026-09-14 junk rows are degenerate and carry a
...
3. **Settle-ready sweep (every 15 min):** `settle_ready_matches()` — catch-all for
   ...
   **#162 W1.3 (2026-09-26): one shadow settlement path.** The corners,
   team-total and first-half-1x2 paper bots used to grade their own
   `shadow_bets` (`settle_picks()` on :50/:55/:57) with no close and no CLV;
   those functions and jobs are deleted. The generic shadow loop now grades all
   three through the resolver registry — for `corners_ou_*` it loads the corner
   counts itself (`_corner_stats`, finished matches only; no counts yet = a
   quiet skip, not the "Unsettleable market" alert), and after the 1H void sweep
   `void_ungradeable_corners_bets()` voids corners legs whose match finished
   more than a day ago with no counts (reason `corners unsettleable — …`,
   unchanged from the bot). `resettle_wrongly_voided_bets` passes no stats, so it
   still leaves corners voids alone.
