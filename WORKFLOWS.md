...
| `com.oddsintel.best-price-router` | :20 / :50 | `best_price_router --execute` — **REAL MONEY at the best book.** Checks BOTH placeable books per candidate, gates each on its OWN price, places ONCE at the best clearing book. **Double-gated:** `--execute` alone does nothing; it also needs `ROUTER_ALLOW_REAL=true` in `.env`, else it degrades to report-only. This is what makes Unibet an automatic placement venue — before 2026-09-12 a pick whose best price was at Unibet was never placed at all. Shares the Coolbet placer's caps (€10 flat, 80 bets/day, €800/day, nothing inside 3 min of kickoff) and dedups cross-book on `real_bets`. Superseded `...-router-monitor` (report-only): a job named "monitor" that moves real money is the misleading-name failure this repo keeps paying for. | ✅ repo |
