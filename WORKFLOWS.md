...
Sun 02:00 [RETIRED 2026-09-24] Prune anon users  job_prune_anon_users()  ANON-AUTH PHASE 4 — NO LONGER REGISTERED (SCHEDULER-DEAD-JOBS-2026-09-24). It deleted idle anonymous rows from `auth.users`, which stayed in Supabase at SUPABASE-TO-VPS, so it failed every Sunday from 2026-07-12. Not repointed: the web app no longer creates anonymous users and the FK cascade it relied on cannot span two databases.
