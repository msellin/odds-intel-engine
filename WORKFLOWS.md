...
| `com.oddsintel.inplay-collector` | KeepAlive (45 s cycle, ≤60 fixtures, ≤12 concurrent fetches) | **OWN Phase 1b (2026-09-15).** `workers.jobs.inplay_collector` — Epicbet live board → `inplay_book_quotes` (90 d, pruned on the VPS 03:20) + the two PAPER slow-state bots (`bot_inplay_slowstate_v1` at the on-screen price, `_afctl_v1` control off AF's aggregate) into `shadow_bets`; heartbeat row `pipeline_health_state.inplay_collector`. Places NOTHING. Epicbet only — Coolbet in-play is a follow-up with its own Imperva budget. Read with `scripts/inplay_slowstate_eval.py`. **The cycle line's `pickfail` counter is the one to watch: `picks 0 | pickfail >0` means the rig is collecting but recording nothing** — the failure mode that cost 2026-09-15→18 (RELIABILITY_LEDGER §15). Cap raised 15→60 on 2026-09-18 after the live board measured 27 fixtures. | ✅ repo |
