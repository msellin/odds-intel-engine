...
| BET-TIMING-ANALYSIS | **Analyse optimal bet placement window — hours before kickoff vs P&L/CLV.** Data already captured: `simulated_bets.pick_time` + `matches.date` + `timing_cohort` column (migration 032). Five refreshes run at 09:30/11:00/15:00/19:00/20:30 UTC, giving natural variation. **Gaps in current coverage:** 12:00–14:00 and 16:00–18:00 kickoffs get <2h pre-KO refresh. **Quick fix (cheap, no AF cost):** add 13:00 + 17:00 refresh slots to `scheduler.py` — betting refresh makes zero AF calls, just re-runs logic on DB odds. **Analysis task:** query `pick_time`, `match.date` → compute `hours_before_ko`, bucket into 1–2h / 2–4h / 4–8h / 8h+ windows, compare avg CLV + win rate per bucket. Target: 200+ settled bets per bucket before drawing conclusions. | 1h analysis + 30min scheduler fix | ⬜ | ✅ Ready | |
