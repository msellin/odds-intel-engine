...
| TIER-C-EXPAND | 🔄 In Progress | 1-2h | **football-data.co.uk extras ingest shipped — needs local run to actually pull data.** `scripts/ingest_football_data_extras.py` generalises `add_romanian_league_data.py` into a parameterised, batch-capable script for 15 more countries: ARG/AUT/BRA/CHE/CHN/DNK/FIN/IRL/JPN/MEX/NOR/POL/RUS/SWE/USA. Each is mapped to a unique `league_code` (`AR1`, `AT1`, … `US1`) — no collision with the existing 19 codes (`B1/D1/D2/E0/...`). Pinnacle closing odds (`PSCH/D/A`) required — rows without are dropped so the file stays Tier A quality. Dedupes on `Date+home_team+away_team`. Top divisions only (lower divisions only ship when football-data publishes separate CSVs). 5 most recent seasons per league. **Run command:** `python3 scripts/ingest_football_data_extras.py` (or `--league USA` for one country). After run, commit the updated `data/processed/targets_poisson_history.csv`. **Effect once committed:** moves top-flight matches in those countries from Tier C → Tier A (closing odds present + form data → ensemble + 0% edge bump, vs Tier C's AF xG + 8% bump). Smoke: TIER-C-EXPAND. Pending: post-ingest verification — count team additions, run a sanity-check prediction on a known fixture. |
