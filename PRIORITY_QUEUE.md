...
| BOOKMAKER-DISPLAY | ✅ Done 2026-05-11 — Server-side `getValueBetBookOdds()` fetches Unibet + Bet365 odds per bet in one round-trip; "Bet365: 2.10 · Unibet: 2.05 ← Bet365" shown in match cell for Elite users. `recommended_bookmaker` + `matchId` added to `LiveBet`. No separate API route needed — server component. | 3h | ✅ Done 2026-05-11 | ✅ Ready | Both tracks: personal betting (user sees exact book to check) + signal product (users trust picks more when named bookmaker is shown). |
...
| INPLAY-ZERO-FIRE-AUDIT | Strategies g, h, i, j, m, n, q have placed 0 bets in 14+ days of live running (observed 2026-05-11 via `inplay_bot_report.py`). Diagnose each: (1) run `inplay_bot_report.py --strategy inplay_g` etc. to confirm zero tried-to-fired conversion; (2) add temporary debug logging to each `_check_strategy_*` function to print which condition fails most often (score state? minute window? odds floor? league xG gate?); (3) for each silent strategy, decide: loosen one threshold, widen the minute window, or drop it. **Per-strategy notes from design:** G (corner cluster ≥3/10min) and H (0-0 at HT) are expected low-frequency by design. N (late fav push, min 72-80) and Q (red card) are rare-event strategies — 0 bets/week is normal. I (fav stall 0-0 min 42-65, live odds ≥3.0) and J (goal debt 0-0 min 30-52) may be blocked by the OU1.5 live odds coverage gap (~12% before the prematch fallback fix). M (equalizer magnet) fired heavily in replay but 0 live — check if BTTS/OU floors are too tight with real (not fallback) odds. | 2h | ⬜ | ⏳ After 4 weeks live (enough sample to distinguish "rare by design" from "broken") | Use `scripts/inplay_bot_report.py --days 30` as the entry point. Don't loosen thresholds speculatively — only for strategies where `tried > 500` and `fired == 0`. |
