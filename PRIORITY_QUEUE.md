...
| ID | Task | Effort | ☑ | Ready? | Notes |
|----|------|--------|----|--------|-------|
| ML-PIPELINE-UNIFY | Master task — see `dev/active/unified-ml-pipeline-plan.md`. Critical path: Stage 0 (derived-data backfills: ELO + form + referee + team_season_stats + MFV rebuild) → Stage 1 (wire train.py output to xgboost_ensemble loader, add `MODEL_VERSION` env var) → Stage 2 (NaN-aware training so historical rows aren't dropped) → Stage 3 (A/B harness: model_version columns + shadow mode + compare_models.py) → Stage 4 (first retrain through harness) → Stage 5 (weekly cron + manual promotion). Each stage has its own checklist in `unified-ml-pipeline-tasks.md`. **Status 2026-05-10:** Stages 0a, 0b, 0c, 0d, 1a, 1b, 1c, 1d, 1e, 2a, 2b, 3a, 3b, 3d, 5a, 5b, 6a all done. Pending: 0e (re-run unblocked), 4a/4b (offline retrain — runs after 0e), 4c/4d/4e (calendar-bound shadow window + promotion). 3c (shadow-mode predictions write) deferred until first shadow candidate exists. | ~2 days agent + 14 days shadow | 🔄 In Progress | ✅ 0e ready to run | Stage 0 (5 backfill scripts) and Stage 6 (backtesting harness) are parallelizable; Stages 1-4 are critical path. **Inference-side wire-through `ML-INFERENCE-MFV-WIRE` is a hidden critical-path blocker for shadow deploy** (xgboost_ensemble loader still uses Kaggle-era column names; v10 trained on MFV columns can be CV-compared offline but not live-loaded). |
...
| OU-MARKET-FEATURES | **v14 candidate (filed 2026-05-10).** Audit triggered by triaging whether v10/v11/v12 were affected by today's OU-PIN-REQUIRED fix revealed a real feature gap: all three model heads (1X2, OU 2.5, BTTS) train on the same feature set, which contains **only 1X2 market signals** — `opening_implied_home/draw/away`, `bookmaker_disagreement` (1X2 selection='home'), v11+ `pinnacle_implied_home/draw/away` (filtered `market='1x2'`). OU and BTTS targets are derived from real match scores so labels are clean, but no OU or BTTS odds ever enter as features. The OU/BTTS heads are flying blind on the very markets they're predicting, despite `odds_snapshots` already storing all five OU lines (0.5–4.5) and BTTS yes/no for every match (`supabase_client.py:644-664`). Plan: extend MFV with Pinnacle-anchored OU 2.5 implied prob (~85% Pinnacle coverage per today's audit), Pinnacle BTTS implied prob, and OU 2.5 bookmaker disagreement; add `_missing` indicators (Stage-2a pattern) since coverage is incomplete; train v14 = v12 features (post-0e) + Pinnacle 1X2 + new OU/BTTS market features. Should lift OU/BTTS log_loss the same way Pinnacle 1X2 lifted v13's OU/BTTS heads. **Double-check odds quality as part of this task** — this is the same OU data path that has produced wrong odds 10× already (most recent fix today: 9d4166e OU-PIN-REQUIRED, before that dd78fea OU-PINNACLE-CAP). Required pre-train audit: (a) Pinnacle 1X2 rows clean (no obvious outliers — see scratch query in CLAUDE convo 2026-05-10), (b) Pinnacle OU 2.5 rows survive the new placement-time `pin_price is None` skip, (c) no non-Pinnacle bookmakers mislabelled under `bookmaker = 'Pinnacle'` (especially in OU markets), (d) BTTS Pinnacle coverage measured before training. If any of these surface bad rows, fix at source (`odds_snapshots` ingestion path / blacklist in `workers/utils/odds_quality.py`) BEFORE training — feeding garbage features trains worse models than no features. | 4-6h (audit + feature build + train + offline_eval vs v11_pinnacle) | 🔄 In Progress | ✅ Ready | **Started 2026-05-10** — Phase A odds-quality audit first per task spec. Don't ship the model until the audit clears — recurring history (10× user reports) means default assumption is the data is dirty until proven otherwise. Coordinate with any further OU/BTTS placement-layer fixes; the source-of-truth definition for "clean Pinnacle OU row" needs to match between training-feature SQL and `daily_pipeline_v2._load_today_from_db` placement aggregation. Compare v14 vs v11_pinnacle on 1X2/OU 2.5/BTTS via `scripts/offline_eval.py` before promoting. |
...
| AF-BREAKDOWN-REVIEW | Read 24h+ of post-migration `api_budget_log.endpoint_breakdown_today` JSONB and decide which AF Quota Optimization task to pick up first. Pre-req: AF-FETCHES-AUDIT shipped 2026-05-10 + Railway has logged a full day with the new instrumentation. **How:** `venv/bin/python3 scripts/af_call_breakdown.py --days 2` — gives daily totals, sorted endpoint share, hour-by-hour endpoint matrix. **Decision rules:** (a) if `fixtures/statistics` or `fixtures/events` >40K/day → pick `AF-COVERAGE-AUDIT` (gate per-match live-poller calls by `coverage_*` flags, potentially halves live volume); (b) if `predictions` >5K/day → pick `AF-PREDICTIONS-FREQ` (drop 5×→2×); (c) if `standings` >300/day → pick `AF-STANDINGS-DAILY`; (d) if `fixtures/headtohead` >300/day → pick `AF-CACHE-H2H`; (e) if `teams/statistics` >150/day → pick `AF-CACHE-TEAM-STATS`; (f) if calls are spread thin with no 5K+ deviator → ship the AF-CACHE-* batch (H2H + TEAM-STATS + STANDINGS-DAILY + PREDICTIONS-FREQ + INJURIES-LATE) as one commit. Then file the chosen task as 🔄 In Progress. **Caveat (2026-05-10 partial spot-check, see `dev/active/af-fetches-audit.md`):** any Railway redeploy mid-day resets `_endpoint_counts_today` while `calls_today` is re-synced from AF `/status`, so the script's "unattributed" portion is real — it represents pre-restart calls. To close the gap permanently, seed `_endpoint_counts_today` from the latest `api_budget_log.endpoint_breakdown_today` row at `BudgetTracker.__init__` (one DB read at process start). Either fix this first or pick a calendar day with no Railway redeploy for the review. Initial 2026-05-10 partial signal: `predictions` 3,115 in one 04:45 UTC burst was the AF-PREDICTIONS-FREQ candidate; **P-PRED-1 shipped 2026-05-10** (drop run_predictions from job_betting_refresh entirely → morning-only). Re-run the breakdown 2026-05-11+ to verify predictions stays at one burst/day instead of climbing through the day. | 30m | ⬜ | ⏳ After 24h of post-2026-05-10 traffic | This is the diagnostic that AF-FETCHES-AUDIT was built for — without reviewing the output, the cache/frequency tasks below are blind guesses. Park the breakdown numbers in `dev/active/af-fetches-audit.md` under a "Measured YYYY-MM-DD" section so future agents can compare. |
...
| ID | Task | Effort | ☑ | Ready? | Notes |
|----|------|--------|----|--------|-------|
| ML-RETRAIN-1 | **Retrain XGBoost on full dataset** — backfill complete 2026-05-10. match_stats covers 34,675 / 47,228 finished matches (73.4%) — short of the 80% target but the AF-permanent-gap leagues won't fill any further (per-dim escape hatch in `backfill_historical.py` already marks these L/S complete). 73.4% is the ceiling. Bigger training set (5K → ~30K finished matches with full stats coverage) should improve calibration especially in lower leagues. Subsumed into ML-PIPELINE-UNIFY Stage 4a/4b (run after 0e finishes MFV rebuild). | 2h | ⬜ | ⏳ After ML-PIPELINE-UNIFY Stage 0e completes | Run `workers/model/train.py --version v10_pre_shadow` + `compare_models.py v10_pre_shadow v9a_202425` for offline CV. Live deploy gated on ML-INFERENCE-MFV-WIRE. |
