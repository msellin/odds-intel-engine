...
> Last updated: 2026-05-07 — AH-SIGNALS done (Asian Handicap parsing + 3 signals + BTTS signal, migration 066). AH-XGBOOST added for ~May 17. Audit found BTTS/H2H-splits/injury-severity as additional throw-away data. AF-VENUES done earlier today.
...
| H2H-SPLITS | Extract home/away split from h2h_raw JSONB already stored in DB. Current: only total H2H counts (wins/draws/losses). Signal: `h2h_home_win_at_venue_pct` — home team's win % in H2H played at this specific ground (away team has never won here → strong prior). Also `h2h_avg_goal_diff` (dominance metric), `h2h_recent_form` (last 3 vs last 10). No API calls — data already in h2h_raw. Found in data-audit 2026-05-07. | 2h | ⬜ | ✅ Ready | `supabase_client.py` batch_write_morning_signals — add block using h2h_raw JSONB. Query `SELECT id, h2h_raw FROM matches WHERE id = ANY(...)`. |
