...
> Last updated: 2026-05-08 — Pipeline stabilization: fixed orphaned 'running' records stuck in DB forever (startup cleanup threshold 30→10 min, new periodic orphan cleanup job every 30 min with 60-min threshold). Fixed enrichment hangs: transfers capped at 100 teams/run + cache extended 7→30 days; coaches capped at 50 teams/run. Manual DB cleanup of 4 stuck records. See PIPELINE-STABILIZE below.
...
| PIPELINE-STABILIZE | Fixed 3 sources of ops-page rot: (1) orphaned 'running' records — startup cleanup threshold 30→10 min, new periodic cleanup job (every 30 min, 60-min threshold); (2) enrichment hanging for hours — transfers capped 100/run + cache 7→30 days, coaches capped 50/run; (3) manual DB cleanup of 4 stuck records from today. | — | ✅ Done 2026-05-08 | ✅ Ready | Deploy to Railway to activate the periodic cleanup job. |
...
| STAKE-RANK | Exposure cap should rank bets within a league by edge before applying declining stakes — currently processes in DB query order so the highest-edge bet in a league can end up with the smallest stake if evaluated last. Fix: collect all bet candidates per league first, sort descending by edge, then apply 50% halving in ranked order so best bet always gets most money. | 2h | ⬜ | ✅ Ready | `workers/jobs/daily_pipeline_v2.py` betting loop. Affects any day with 3+ bets in same league (Austria, Serie B, Victoria NPL etc). |
