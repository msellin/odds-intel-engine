...
| BOT-STRATEGY-DEEP-REVIEW | P2 | ~3-4 days | Thread 1 complete (2026-05-13) — all 23 prematch + 13 inplay bots audited, ranked lists written to `dev/active/bot-strategy-audit-results.md`. Threads 2–3 (gap survey, data readiness) pending. 4 follow-up tasks queued below. |
...
| INPLAY-LIVE-OU-COVERAGE | P1 | 3-4h | Live OU2.5/1.5 odds are only present in ~17% of `live_match_snapshots` — the single biggest bottleneck for all OU-based inplay bots (e, h, a, d, j). Audit why: are we not fetching from AF live odds correctly? Are we dropping rows? Fix live tracker to capture OU odds more reliably. This would directly improve all 5 OU-based inplay strategies. |
