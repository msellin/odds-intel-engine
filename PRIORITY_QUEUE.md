...
| ML-ELO-GAP | **Add ELO to `FEATURE_COLS` in train.py** — `home_elo`, `away_elo`, `elo_diff`, `home_elo_exp` are computed at inference (`xgboost_ensemble.py:152-157`) but absent from `FEATURE_COLS` in `train.py` so the new AF model never learns from them. Add all 4 to FEATURE_COLS, re-run train.py, compare log_loss. (Hvattum & Arntzen 2010: ELO is most valuable for promoted teams, post-international-break, early season.) | 1h | ⬜ | ✅ Ready | Applies to `workers/model/train.py`. Verify `match_feature_vectors` has `home_elo`/`away_elo` populated before running — check a sample row. |
...
| ML-PINNACLE-FEATURE | **Pinnacle odds as training feature** — include `market_implied_home`, `market_implied_draw`, `market_implied_away` (vig-removed) directly in FEATURE_COLS. R4 cites Kaunitz et al. 2017: Pinnacle closing line beats any pure model-based strategy. The market already encodes team quality, injuries, suspensions, and everything else — teaching the model to predict vs market rather than raw outcome is the key shift. Also add Asian Handicap line/movement: Štrumbelj & Šikonja 2010 found AH encodes strength more precisely than 1X2 and is orthogonal to sharp_consensus. | 2h | ⬜ | ⏳ Need historical Pinnacle odds in match_feature_vectors — check coverage | Check: `SELECT COUNT(*) FROM match_feature_vectors WHERE pinnacle_implied_home IS NOT NULL`. Add `pinnacle_ah_line_move` once AH is consistently stored. |
| ML-HYPERPARAMS | **Switch to LightGBM + tune hyperparameters** — LightGBM within 0.2-0.5% log-loss of XGBoost on football data, with better null handling (enables ML-MISSING-DATA to be even simpler). Key params per R4: `LGBMClassifier(n_estimators=300, max_depth=6, learning_rate=0.05, num_leaves=31, min_child_samples=20, subsample=0.8, colsample_bytree=0.8, reg_alpha=0.1, reg_lambda=1.0)`. `min_child_samples=20` is the key overfitting guard at this data size. | 3h | ⬜ | ⏳ After ML-RETRAIN-1 | `pip install lightgbm`. Swap `XGBClassifier` → `LGBMClassifier`. Compare log_loss with TimeSeriesSplit — if <0.2% difference, keep XGBoost for simplicity; if >0.5%, adopt LightGBM. |
| ML-BLEND-DYNAMIC | **Dynamic Poisson/XGBoost blend weights per market and data tier** — `fit_blend_weights.py` produces one global weight. Per R4 (Constantinou 2019): for teams with <50 prior matches use Poisson weight ~0.65, 50-200 → ~0.50, >200 → 0.45-0.50. O/U and BTTS should be Poisson-heavier at all tiers (Koopman & Lit 2015). Split by (data_tier, early_season) and store separate weights per combination. | 2h | ⬜ | ⏳ After ~500 settled predictions per tier | O/U and BTTS are more goals-model-friendly. 1X2 may benefit from more XGBoost weight. Estimated lift: 0.2-0.4% log-loss. |
