...
| ML-INFERENCE-MFV-WIRE | **Hidden critical-path blocker discovered 2026-05-10.** `xgboost_ensemble.py:155-178` builds the inference feature vector from Kaggle-era column names (`home_elo`, `h_*`, `a_*`, `xg_diff`, `form_diff`, `tier`) that don't match `match_feature_vectors`'s schema (`elo_home`, `form_ppg_home`, `goals_for_avg_home`, `h2h_win_pct`, etc.). When `feature_cols.pkl` is loaded from a v10+ bundle, the line `pd.DataFrame([row])[feature_cols]` raises KeyError on the new column names. Stage 4c shadow deploy is blocked on rewriting `_predict_xgb` to read from `match_feature_vectors` directly (or build a row matching the new schema). Until then a v10 model can only be CV-compared offline via `compare_models.py` against settled predictions tagged `v9a_202425`. | 4-6h | ⬜ | ✅ Ready (after 4a/4b confirms a v10 worth deploying) | Two options: (a) extend `_predict_xgb` to fetch `match_feature_vectors` row by `match_id` and use it directly when feature_cols.pkl uses MFV column names; (b) build a translation layer mapping Kaggle names → MFV names. (a) is cleaner long-term — drops the 2-table-fetch (`team_form_features` + ELO) for one MFV read. |
