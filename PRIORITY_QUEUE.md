...
| BACKFILL-IDS-BATCH | `backfill_historical.py` was firing 2 individual AF calls per match (`/fixtures/statistics?fixture=N` + `/fixtures/events?fixture=N`) plus one wasted `?league=&season=` refetch per league/season pair — 7,086 matches still missing stats meant ~14,200 calls, ~9 days at the scheduler's 30 req/run × 25-min cadence. Settlement already used `get_fixtures_batch` (`?ids=N1-…-N20`, 20 fixtures/call with embedded statistics + events + lineups + players) but backfill didn't. Probed AF embed coverage on 20 historical fixtures (5 each from 2023/2024/2025/2026, `scripts/probe_fixtures_batch_embed.py`): all 4 years return 5/5 populated `statistics`+`events`; 2026 lower-tier leagues (Serie D etc) miss lineups/players, which are AF coverage gaps that affected the per-fixture path equally. Refactor: `backfill_league_season` now (a) queries `matches` directly for finished AF IDs lacking stats/events in this league/season — no `/fixtures?league=&season=` call, (b) runs one `get_fixtures_batch(af_ids)` then parses embedded `statistics` + `events` from the prefetched dict, (c) keeps the bulk `execute_values` insert for events. Throughput: 14 matches/run → ~600 matches/run (40× speedup); ~9 days → **~5 hours at scheduled cadence**, or ~5 minutes wall time with a one-shot `--max-requests 500` manual run. 1 smoke test asserts `get_fixtures_batch` is called and the per-match endpoints are not. | 1.5h | 🔄 In Progress | ✅ Ready | After ship, run `python3 scripts/backfill_historical.py --max-requests 500` once to clear the backlog in ~5min wall time. |
