...
| COMBO-RESEARCH-PHASE-A | P1 | 2h manual | **Audit Coolbet's combo & SGM pricing — no code, pen and paper.** Goal: find out whether Coolbet adds extra margin on accumulators and whether it prices Same-Game Multis (SGMs) as the product of marginals (= correctly correlated bets are mispriced) or with a fair correlation model. Worksheet in `dev/active/combo-research-phase-a-worksheet.md`. Verdict from this gates Phases B-D. Without Phase A results, B/C/D are speculative. |
