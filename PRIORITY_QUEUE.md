...
| PER-BOT-EDGE-THRESHOLD-SWEEP | ✅ Done 2026-05-18 (analysis only) | 1h | Built `scripts/per_bot_edge_threshold_sweep.py` — sweeps edge thresholds 1-15% per bot against expanded backtest CSV (19K rows / 9K matches). Finds ROI-maximizing threshold per bot with min-50-bets guard. **Headline findings (raw backtest, no filter stack):** bot_aggressive_v2 +17pp if raised 5%→15%; bot_ou35_attacking +12pp at 15% (+39% ROI); bot_btts_conservative +9pp at 13%; bot_btts_all +7pp at 12%. Config NOT yet changed — needs careful live validation before deploying. |
| PER-BOT-EDGE-THRESHOLD-APPLY | P1 | 2h | Apply the threshold sweep findings to BOTS_CONFIG. Process: (1) cross-reference each bot's current `edge_thresholds` vs sweep recommendation, (2) account for volume trade-off (15% threshold = ~5x fewer bets), (3) update BOTS_CONFIG with new per-tier × per-market thresholds, (4) note: backtest ROIs are RAW without the filter stack, so absolute targets won't match live — use sweep as directional guide. Likely candidates for change: bot_aggressive_v2, bot_ou35_attacking, bot_btts_conservative, bot_btts_all, bot_ou25_global. Live validation required for 2-3 weeks after change before considering further tightening. |
