...
| REAL-MONEY-TRACKER | `real_bets` table (migration 092) and DB schema ready. Need: (1) admin UI `/admin/place` to log a real bet with actual odds + bookmaker; (2) `scripts/real_perf_report.py` — paper vs real P&L comparison showing slippage % and whether real outperforms paper. Settlement for real_bets mirrors simulated_bets settlement (same match result). | 3d | ⬜ | ✅ Ready | Schema: `real_bets` has `captured_odds` (what pipeline showed), `actual_odds` (what user got), `slippage_pct` (generated column). Tells us exactly how much we lose to price movement and execution friction. |
| BOOKMAKER-DISPLAY | Frontend value-bets page: show per-accessible-bookmaker odds breakdown ("Bet365: 2.10 | Unibet: 2.05 | best at Bet365") alongside each pick. Requires backend odds endpoint `/api/match-bookmaker-odds` that reads `odds_snapshots` filtered to `ACCESSIBLE_BOOKMAKERS` for the match. | 3h | ⬜ | ✅ Ready | Both tracks: personal betting (user sees exact book to check) + signal product (users trust picks more when named bookmaker is shown). |
| FRESHNESS-INDICATOR | Show "Odds verified Xmin ago" on value bets page — derive from latest `odds_snapshots.timestamp` for each match. Sets user expectations: if odds are 3h old and kickoff is in 30min, user knows to verify before placing. | 1h | ⬜ | ✅ Ready | Simple: JOIN `odds_snapshots` on match_id, `MAX(timestamp)`, compute age. Frontend renders "verified 4m ago" in green / "verified 2h ago" in yellow / "verified Xh ago (check)" in red. |
