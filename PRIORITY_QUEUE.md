...
| — | ML-1 | Team crests/logos on match rows | 4-6h | ⬜ | **High** | UX audit (2026-04-29) | Next | API-Football already returns `team.logo` URL per fixture. Store `logo_url` in teams table (backfill from fixture data). Display as 20px circle next to team name in LeagueAccordion. `loading="lazy"` on all images. `onError` fallback: colored circle with first letter. AI validation: highest impact/effort ratio of match list improvements — makes it look like a sports product immediately. |
| — | ML-2 | Live match timer + FT/HT status label | 2-3h | ⬜ | **High** | UX audit (2026-04-29) | Next | Finished matches: show "FT". Live matches: show "22'" using `live_minute` from live_match_snapshots (already polled every 60s). HT: show "HT". Scheduled: show kickoff time. Do NOT estimate minute client-side from kickoff time — misleads on delays/stoppage. Current LIVE badge with no minute is worse than no badge. |
| — | ML-3 | W/D/L form strip on match rows | 2-3h | ⬜ | Medium | UX audit (2026-04-29) | Next | 5 colored dots per team from form string (already in league_standings). Only render when BOTH teams have form data — inconsistent rows look broken. Green=W, amber=D, red=L. ~40% of matches have this data today, mainly top leagues. |
| — | ML-4 | Per-match favourite star | 1 day | ⬜ | Low | UX audit (2026-04-29) | With ALERTS | Star icon on individual match rows (separate from league star). Use case: track specific games user has bet on. localStorage for anon, `user_match_favorites(user_id, match_id, created_at)` table for logged-in. Merge on login. Build when ALERTS exists — without notifications the star has no payoff. Design DB schema now. |
