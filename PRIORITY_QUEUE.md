...
| PIN-VETO | Pinnacle disagreement veto for 1X2 home bets | ✅ Done 2026-05-06 | — | Batch-loads `pinnacle_implied_home` before betting loop. If `cal_prob − pinnacle_implied > 0.12` on a home bet → skip. Empirical: won bets all have gap ≤ 12.9%, lost bets avg 14.1% (up to 21.7%). Catches 22/34 losses, filters 6/40 wins. `PINNACLE_VETO_GAP = 0.12` constant in `daily_pipeline_v2.py`. |
| ODDS-API | ~~Activate The Odds API for Pinnacle odds ($20/mo)~~ | — | ❌ Cancelled | AF already provides Pinnacle in its 13-bookmaker feed. ODDS-API is redundant. | Only gap: Singapore SPL (no AF odds) — not worth $20/mo for one league |
| **PIN-2** | **Extend Pinnacle signals to all bet markets** | **1h** | **⬜** | **✅ Ready** | **Currently only `pinnacle_implied_home` stored. Add `pinnacle_implied_draw`, `pinnacle_implied_away`, `pinnacle_implied_over25`, `pinnacle_implied_under25` to `batch_write_morning_signals()` in supabase_client.py. One query, filter by bookmaker='Pinnacle'. Unlocks PIN-3, PIN-4, PIN-6. Pinnacle covers 59% of our matches (1,493/2,515 in 30 days).** |
| **PIN-3** | **Extend disagreement veto to draw/away/O/U markets** | **1-2h** | **⬜** | **⏳ After PIN-2** | **Apply same veto logic to draw, away, over 2.5, under 2.5 bets. Need PIN-2 signals first. Threshold may differ per market — start at 0.12, tune once 50+ settled bets per market. For O/U: Pinnacle prices totals very efficiently so gap matters more.** |
| **PIN-4** | **Pinnacle line movement signal** | **1-2h** | **⬜** | **✅ Ready (data exists)** | **Compute `pinnacle_line_move_home` = Pinnacle opening implied − current implied per match. Data: avg 8.7 Pinnacle snapshots/match in 30 days. Positive = home shortened (sharp money backing home). Store in `match_signals`. This is purer than generic `odds_drift` — Pinnacle lines move ONLY on sharp money (they cap/ban soft bettors). Add draw and away variants too.** |
| **PIN-5** | **Pinnacle-anchored CLV** | **2h** | **⬜** | **✅ Ready (data exists)** | **Current CLV: `(odds_at_pick / best_available_closing_odds) - 1`. Better: `(odds_at_pick / pinnacle_closing_odds) - 1`. Pinnacle CLV is the industry standard — consistently beating Pinnacle's close = genuinely finding edge before sharp money. Add `clv_pinnacle` column to `simulated_bets`, compute in `settlement.py` alongside existing CLV. Requires Pinnacle closing odds lookup via `odds_snapshots WHERE bookmaker='Pinnacle' AND is_closing=TRUE`.** |
| **PIN-6** | **Pinnacle-anchored calibration shrinkage** | **2-3h** | **⬜** | **⏳ After PIN-2 + 200+ settled bets** | **Current shrinkage: `shrunk = (1-α) * model_prob + α * market_avg_implied`. Better: `shrunk = (1-α) * model_prob + α * pinnacle_implied` (when Pinnacle available, fallback to market avg otherwise). Pinnacle has the sharpest probability estimates — shrinking toward Pinnacle rather than market average is more principled. Requires PIN-2 for all markets. Validate with ECE check before/after on 200+ settled bets.** |
