...
| B-ML3 | First meta-model: 8-feature logistic regression, target=pseudo_clv>0 | 1 day | ⬜ | ⏳ MFV rows since May 6: 3,819 ✅ (3K threshold met). CLV-outcome rows: ~582 (still growing, ~60/day avg). Note: `pinnacle_implied_home` column does not exist in current MFV schema — filter condition needs updating to `opening_implied_home IS NOT NULL` (2,266 rows). Run NEWS-LINEUP-VALIDATE first (prerequisite). | **Data quality cutoff: use only `match_feature_vectors` rows WHERE `match_date >= '2026-05-06'`** — pre-cutoff rows lack key signals. `pinnacle_implied_home` was never added to MFV schema — use `opening_implied_home` as Pinnacle proxy instead. Features per META-2. Feature notes: (1) `ensemble_prob_home - opening_implied_home` — likely strongest; (2) keep `odds_drift`, drop `overnight_line_move` (0.7+ correlated); (3) validate `news_impact_score` AUC > 0.52 first (see NEWS-LINEUP-VALIDATE); (4) add `odds_at_pick` (raw); (5) add `time_to_kickoff` (hours). Source: 4-AI Calibration Review + data quality analysis 2026-05-06 |
...
| CAL-PLATT-UPGRADE | Replace single-input Platt with 2-feature logistic: `X = [shrunk_prob, log(odds)]` | half day | ⬜ | ✅ O/U ready (353 settled ≥ 300). 1X2 close (223, need 77 more). AH=7, BTTS=20 not ready. Implement O/U first, 1X2 soon after. | Learns that "40% at odds 3.6" needs different correction than "40% at odds 1.8". Do NOT implement sooner — will overfit. Source: 4-AI Calibration Review 2026-05-06. **As of 2026-05-12:** O/U threshold met. Start with O/U market, add 1X2 ~2 weeks later. |
| ALN-1 | Dynamic alignment thresholds | 2h | ⬜ | ✅ Threshold met 2026-05-12 (590 quality settled bets ≥ 300). Ready to implement now. | **Data quality cutoff: validate on bets WHERE `created_at >= 2026-05-06` only** — pre-cutoff bets were placed by the old pipeline (no Pinnacle anchor, no CAL-ALPHA-ODDS, different veto coverage). Have 347 bets with `alignment_class` set: HIGH=3 bets (+223% ROI), MEDIUM=11 (+29.6%), LOW=255 (+16.4%), NONE=78 (+19.1%). HIGH/MEDIUM sample too small for statistical confidence but direction is correct — monotonic increase. Proceed with implementation; thresholds can be tuned as sample grows. |
| NEWS-LINEUP-VALIDATE | Validate `news_impact_score` AUC and `lineup_confidence` accuracy before including in B-ML3 | 1h | ⬜ | ✅ Ready now (415 matches with signals ≥ 100 threshold) | Prerequisite for B-ML3. (1) Compute AUC of `news_impact_score` vs actual outcome divergence from base model — gate: AUC > 0.52 before including in meta-model. (2) Check `lineup_confidence >= 0.9` accuracy: does `lineup_confirmed=true` predict actual XI 90%+ of the time? Query: `SELECT COUNT(*) FROM match_feature_vectors WHERE match_date >= '2026-05-06' AND news_impact_score IS NOT NULL` = 415. |
...
| CAL-PLATT-UPGRADE ready — O/U | Settled O/U bets since May 6 | 300+ | **353 ✅** | ✅ Ready now |
| CAL-PLATT-UPGRADE ready — 1X2 | Settled 1X2 bets since May 6 | 300+ | **~223** | ~2 weeks |
