...
> Last updated: 2026-05-10 — P0 + P1 inplay batch landed. Strategy E proxy disabled (182 bad bets voided via migration 079, −4.7% ROI bleeding stopped); B confirmed already fixed. P1: AF "Next 10 Minutes Total" market captured (free, zero new AF calls), `_funnel` skip counters wired into heartbeat, `_remaining_goals_prob` Bayesian helper extracted (J/L collapsed onto it), `retired_at` column added to bots (a2/c_home/f hidden from public /performance, collapsible behind a toggle on /admin/bots). Backfill replay against fixed strategies → 12 new candidate bets, +71% ROI on tiny sample (CSV at dev/active/). Migrations 079, 080, 081 applied via GH Actions on push.
