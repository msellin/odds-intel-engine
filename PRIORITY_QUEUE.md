...
| ID | Task | Effort | ☑ | Ready? | Notes |
|----|------|--------|----|--------|-------|
| ML-RETRAIN-1 | **Retrain XGBoost on full dataset** — backfill complete 2026-05-10. match_stats covers 34,675 / 47,228 finished matches (73.4%) — short of the 80% target but the AF-permanent-gap leagues won't fill any further (per-dim escape hatch in `backfill_historical.py` already marks these L/S complete). 73.4% is the ceiling. Bigger training set (5K → ~30K finished matches with full stats coverage) should improve calibration especially in lower leagues. | 2h | ⬜ | ✅ Ready (backfill at terminal coverage 2026-05-10) | Run `workers/model/train.py` + re-run Platt + blend weight scripts. Compare log_loss before/after. |
