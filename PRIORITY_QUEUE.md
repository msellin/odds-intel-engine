...
> Last updated: 2026-05-05 — INFRA-5/6/8 done (Speed Insights, Sentry Crons, Resend webhook tracking). ENG-3 schema fixes (predictions pivot, injuries, odds). ENG-4 fully live: migration 042 backfills notification_settings, branded email template (dark header, green logo), tested end-to-end.
...
| BOT-PROVEN | `bot_proven_leagues` — focused strategy targeting only the 5 cross-era backtest-confirmed leagues (Singapore/Scotland/Austria/Ireland/S.Korea) | 1h | 🔄 In Progress | Separate clean performance track for the strongest backtest signals. Midday cohort. |
| RHO-DYN | Dynamic Dixon-Coles rho per league tier — fit rho from historical scoreline frequencies instead of global -0.13 | 2h | 🔄 In Progress | `scripts/fit_league_rho.py` → `model_calibration`. Auto-loaded by pipeline per tier. Added to Sunday refit. |
...
| LEAGUE-ORDER | Audit + redesign league priority tiers for today's matches page | 2-3h | ⬜ | ✅ Ready | Current system: flat priority=1 for all featured cups (alphabetical tie-break). CL now fixed to priority=1 alone (CONMEBOL=2), but the full 6-tier ordering hasn't been designed. Task: check FlashScore/SofaScore/Forebet ordering on a typical week, map ~50 shown leagues into 6 priority tiers (CL/WC/Euros → EL/Copa/Libertadores → Big 5 → secondary domestics → cups → rest), update `FEATURED_PRIORITY` + `BASE_PRIORITY` in `pipeline_utils.py`. Goal: any user landing on the site on any given day sees the most globally-relevant game first. |
...
| INFRA-6 | Sentry Crons monitoring for Railway jobs | 1h | ✅ Done 2026-05-05 | 🟡 This week | `sentry-sdk>=2.0.0` added to requirements.txt. `_init_sentry()` in scheduler.py. `_run_job()` now wraps each job with `sentry_sdk.monitor(monitor_slug=...)`. 10 monitor slugs registered (morning_pipeline, settlement, email_digest, match_previews, news_checker, hist_backfill, betting_refresh, odds_refresh, enrichment_refresh, settle_ready). **Needs**: (1) `SENTRY_DSN` set in Railway env vars; (2) cron monitors registered in Sentry dashboard at sentry.io → Crons. |
...
| INFRA-8 | Resend webhook → email open/click tracking | 2h | ✅ Done 2026-05-05 | 🟡 This week | Migration 041 adds `last_email_opened_at` + `last_email_clicked_at` to profiles. `/api/resend-webhook` route handles `email.opened` (updates profile + digest log status) and `email.clicked`. Svix signature verification. **Needs**: (1) Create webhook in Resend dashboard → Webhooks → `https://www.oddsintel.app/api/resend-webhook`, events: opened+clicked; (2) Copy signing secret to `RESEND_WEBHOOK_SECRET` in Vercel + local .env.local. |
