...
| BETFAIR-DEV-START | P1 | 1-2h | Scaffold Betfair client + integration code. **No account needed.** Do now, independent of registration. |
| BETFAIR-REGISTER | P1 | 30min manual | **Gate task** — try betfair.com from Estonian IP/LTE hotspot. If blocked → skip to MATCHBOOK-REGISTER |
| BETFAIR-LIVE-KEY | P2 | 1h + £299-499 | Activate live app key after successful registration. Blocked on BETFAIR-REGISTER ✅ |
| BETFAIR-API-INTEGRATION | P2 | 1-2 days | Build `betfairlightweight` + Stream API into inplay_bot.py. Blocked on BETFAIR-LIVE-KEY ✅ |
| MATCHBOOK-REGISTER | P2 | 30min manual | Fallback only if Betfair fails geo-check. Verify Estonian access + get API key |
...
| PER-BOT-EDGE-THRESHOLD-APPLY | P1 | 2h | Apply threshold sweep findings to BOTS_CONFIG. Deferred to **~2026-06-15 (Batch 2)** — re-run sweep then with live data added and apply. Key findings from 2026-05-19 sweep (25K bets / 22 bots): strong signal on bot_ou35_attacking (14% +40%), bot_btts_all (12% +5.8%), bot_btts_conservative (8% +3.6%), bot_aggressive (15% +9%). DC bots negative at all thresholds — investigate separately. |
...
| SLICE-LIVE-VALIDATE | P2 | 30min | **Re-run `scripts/slice_live_validate.py` when any bot reaches 50+ settled live bets per slice.** Baseline stored at `dev/active/backtest-slice-baseline.csv` (185 slice rows across 18 bots). Script compares backtest ROI vs live ROI per (bot × slice_type × label) and flags confirmations/contradictions. Run command: `python scripts/slice_live_validate.py`. **Current state (2026-05-18, 1,149 live bets):** 2 confirmations (bot_aggressive:2.50-3.00 -12.6%→-19.0%, btts_conservative:T1 -9.8%→-11.9%), 6 contradictions (all home/away Poisson artifacts corrected by v14). Key lesson: backtest is unreliable for 1X2 home/away slices; trust it more for odds-bucket slices. Re-run target: ~**2026-06-15** (Batch 2 session) when bots accumulating 2+ months of live data. |
| BACKTEST-HISTORICAL-CSV-INGEST | P2 | 1-2 days | **Ingest football-data.co.uk closing odds CSVs into `odds_snapshots` to expand backtest pool from 18 days to multi-year.** Background: `backtest_pre_match_bots.py` is bounded by 18 days because that's the `odds_snapshots` + `predictions` overlap. football-data.co.uk has historical closing odds (Pinnacle, Bet365) for 2005-2025 across all popular leagues — already used by `bot_high_roi_global` and `bot_proven_leagues` configs for the "mega backtest 354K matches 2005-15" reference. **Scope:** (1) download football-data CSVs for top European + South American leagues (E0/E1/E2, SP1/SP2, D1/D2, I1/I2, F1/F2, N1, P1, B1, T1, G1, etc., 2018+); (2) build mapping from football-data team names to our `teams` table (fuzzy + manual overrides for known mismatches); (3) write `scripts/ingest_football_data_csvs.py` that converts each CSV row into `odds_snapshots` rows for `bookmaker='Pinnacle'` (B365 column maps to bookmaker='Bet365') with `is_closing = TRUE`, market='1x2' / 'over_under_25' / 'btts'; (4) skip rows already covered by live `odds_snapshots` (idempotent). (5) Re-run `backtest_pre_match_bots.py` once ingested — should jump from 28K matches in scope to 200K+. **Caveats:** (a) only 1x2 + OU 2.5 + BTTS in football-data; no AH or DC. (b) Closing-line only, no in-game movement. (c) Team-name fuzzy matching will have edge cases — log unmatched rows. (d) Lookahead bias on the model is moderate: current model was retrained on data overlapping these matches. For honest backtest, would need walk-forward retraining (much bigger project — defer). For now treat results as "directional, not pristine." Once shipped, every bot's backtest sample grows ~10x — enough to actually trust per-bot ROI estimates. |
| COOLBET-SCRAPE-FEASIBILITY | P2 | 2-3h research | **Research what it would take to scrape Coolbet odds.** Inspect public site structure (network tab → JSON XHR endpoints), `robots.txt`, terms of service, rate-limit signals. Output: a `dev/active/coolbet-scrape-feasibility.md` covering: viable endpoints, polling frequency safety margin, risk profile (account flagging, TOS), engineering effort estimate. No actual scraping yet — feasibility research only. If green, opens up `COOLBET-SCRAPE-BUILD` (proper integration). |
...
| TIER-C-EXPAND-ALIASES | ⬜ | 30-60m | **Follow-up cleanup to TIER-C-EXPAND-ODDS** — extend `TEAM_ALIASES` in `ingest_football_data_csvs.py` for ~30-50 fixable team-name mismatches (out of 159 unmatched). High-yield candidates: Poland (`Rakow`→`Raków`, `Gornik Z.`→`Górnik Zabrze`, `Ruch`→`Ruch Chorzów`, `LKS Lodz`→`ŁKS Łódź`, `Polonia Warszawa`→`Polonia Warsaw`), Mexico (`Atl. San Luis`→`Atlético San Luis`), Russia (`M. Saransk`, `R. Volgograd`, `SKA Khabarovsk`, `Pari NN`), Argentina (`Atl. Tucuman`→`Atlético Tucumán`, `Ind. Rivadavia`→`Independiente Rivadavia`, `Dep. Riestra`→`Deportivo Riestra`), China (`Guangzhou Evergrande`→`Guangzhou FC`, `Cangzhou`→`Cangzhou Mighty Lions`, etc.), Brazil (`Athletico-PR`→`Athletico Paranaense`), Austria (`A. Klagenfurt`→`Austria Klagenfurt`, `A. Lustenau`→`Austria Lustenau`). The other ~110 unmatched are defunct/relegated clubs (Chivas USA, Atlante, Amkar, Mjondalen, Sogndal) — irreducible. After aliases added, re-run `ingest_football_data_extras_odds.py` (idempotent — only new aliased rows inserted). Expected delta: +1-2K matched matches across the batch. Pre-step: build `scripts/audit_unmatched_extras.py` that dumps unmatched names with frequency to prioritize aliases. |
...
| BETFAIR-DEV-START | **[INPLAY AUTOMATION] Scaffold Betfair client — no account needed, do this now.** Install `betfairlightweight` (PyPI). Create `workers/api_clients/betfair_client.py` with: (1) `BetfairClient` class — session auth via username/password/app_key, session token management; (2) `find_market_id(match_name, market_type)` — `listMarketCatalogue` lookup by event name + market type ("MATCH_ODDS" for 1x2, "OVER_UNDER_25" for OU2.5); (3) `get_best_price(market_id, selection_id)` — `listMarketBook` for current back/lay prices; (4) `place_bet(market_id, selection_id, side, size, price, market_version)` — `placeOrders` LIMIT order with `marketVersion` guard against post-event fills; (5) `BetfairClientMock` — same interface, logs calls instead of hitting API. Wire mock into `inplay_bot.py` behind an env flag `BETFAIR_ENABLED=false` so the code path is exercised on every Railway cycle without needing real credentials. Add smoke test `BETFAIR-DEV-START` that imports the client and confirms mock fires without error. Everything here works with zero Betfair account — swap in real credentials when BETFAIR-LIVE-KEY is done. | 1-2h | ⬜ | ✅ Ready — no account needed |
| BETFAIR-REGISTER | **[INPLAY AUTOMATION] Gate task — manual registration test.** Try to register at betfair.com from an Estonian IP (home broadband first; if blocked, retry on LTE hotspot which may route via different DNS). Complete KYC with real Estonian ID + address — Betfair accepts Estonian customers under MGA license. Confirm: (1) site loads without VPN; (2) Estonian ID + address accepted for KYC; (3) Exchange tab accessible (not just Sportsbook); (4) Delayed app key can be generated. If site is ISP-blocked on broadband but LTE works: note this — ongoing API calls come from Railway anyway, so one-time registration via hotspot is acceptable. If KYC fails or geo-block is hard: drop Betfair path entirely, move to MATCHBOOK-REGISTER. Do not build anything until this confirms. | 30min manual | ⬜ | ✅ Ready | **Gate for entire Betfair automation path.** Fail here → stop, go to Matchbook. |
| BETFAIR-LIVE-KEY | **[INPLAY AUTOMATION] Activate Betfair Live App Key.** After BETFAIR-REGISTER confirms account + Exchange access. Steps: (1) fund account with £10-20; (2) place at least one manual bet on Exchange; (3) apply for Live App Key via developer.betfair.com — one-off £299 (personal trader) to £499 (see which applies); (4) confirm `placeOrders` works from a non-EE IP (test from Railway or VPS). The Live Key is required for real-time Stream API data — without it, stream is 3-min batched and useless for inplay. Delayed key is fine for development/testing phase before going live. | 1h + £299-499 fee | ⬜ | ⏳ Waiting BETFAIR-REGISTER | |
| BETFAIR-API-INTEGRATION | **[INPLAY AUTOMATION] Wire Betfair bet placement into inplay_bot.py.** Use `betfairlightweight` (PyPI). Architecture: (1) add `BetfairClient` class in `workers/api_clients/betfair_client.py` — session auth, `placeOrders` wrapper, `listMarketBook` for pre-fire price confirmation; (2) on inplay signal fire, look up Betfair market ID for the match + market (1x2 / O2.5U); (3) place LIMIT order at current best available price, use `marketVersion` to prevent post-event fills; (4) log placed bet to `real_bets` table with `source=betfair`; (5) Stream API subscription for live market data replaces current inplay polling for Betfair markets. Minimum stake €2 (raise inplay bot floor from €1). Commission 5% net — factor into edge threshold (need >5% edge to break even, raise minimum fire threshold accordingly). | 1-2 days | ⬜ | ⏳ Waiting BETFAIR-LIVE-KEY | |
| MATCHBOOK-REGISTER | **[INPLAY AUTOMATION — Betfair fallback only.] Register at matchbook.com if BETFAIR-REGISTER fails.** Estonia is not on Matchbook's restricted list. API at developers.matchbook.com — free tier (unlimited writes + 1M GET/month). Commission 4% for non-UK/IE. Lower liquidity than Betfair — inplay football depth is thinner, expect more partial fills in smaller leagues. If registration succeeds: get API credentials and queue a MATCHBOOK-API-INTEGRATION task. Only pursue if Betfair path is dead. | 30min manual | ⬜ | ⏳ Waiting BETFAIR-REGISTER failure | |
...
| META-BOT-PORTFOLIO | **Post-Phase-3: combine 2-3 best-performing bots into a personal meta-bot with portfolio Kelly staking.** Problem: running bots independently sizes each bet against the full bankroll, ignoring correlations and conflicting picks (two bots on opposite sides of the same market). **Research complete (2026-05-14).** Key findings: (1) **No closed-form solution** for simultaneous Kelly with N>1 bets — requires `scipy.optimize` over joint outcome space (2^N scenarios, trivially fast for small N). Best reference implementation: `BettingIsCool/real_kelly-independent_concurrent_outcomes` on GitHub. (2) **Conflict resolution:** net opposing fractions — if bot A says home 4% and bot B says away 2%, net result is home 2%; if net ≤ 0, don't bet. Never place both sides at same book. (3) **Correlated same-match bets** (e.g. 1X2 home + AH home on same match): apply 0.5–0.6 discount to both fractions, or just take the higher-Kelly pick. (4) **Fractional Kelly:** multiply all simultaneous Kelly fractions by 0.5 (half-Kelly). Uhrín et al. 2021 (arxiv:2107.08827) validates adaptive fractional Kelly is best in practice across soccer. (5) **Hard bankroll cap:** if sum of all active bet fractions × bankroll > 20%, scale all down proportionally. **Planned implementation:** (a) user config selects 2-3 bot IDs as the meta-portfolio; (b) conflict-netting pass; (c) correlation-discount pass (same match = 0.5× both); (d) `scipy`-based simultaneous Kelly sizing; (e) half-Kelly + 20% cap; (f) `/admin/place` shows meta-bot unified list instead of raw per-bot candidates. Personal-use only, not per-user. | 1-2 days | ⬜ | ⏳ Waiting — blocked on Phase 4 pivot decision (~2026-06-30). Start after 200-bet cohort report identifies which bots to include. |
