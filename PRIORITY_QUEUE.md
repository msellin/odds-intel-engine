...
| TIER-C-EXPAND-ODDS | 🔄 In Progress | 1h | **Closing-odds ingest for the 14 TIER-C-EXPAND countries — script ready, pending real-run.** `scripts/ingest_football_data_extras_odds.py` joins football-data.co.uk `/new/<code>.csv` closing odds (PSCH/D/A, B365CH/D/A, etc.) to the ~13K historical matches already in DB for those countries (backfill_historical.py pulled them in May but odds_done=0 across all rows — historical odds were never wired up). Reuses `resolve_team`/`extract_odds_rows`/`existing_snapshot_keys` from `ingest_football_data_csvs.py` so team aliasing and dedupe behave identically. **Critical optimization**: initial draft did one `find_match` DB query per CSV row → 9 min per league on EU pooler; replaced with `load_all_matches_for_leagues()` (single bulk SELECT) + `find_match_in_memory()` (dict lookup) → 5 sec per league, 68 sec for the full batch. **Dry-run scope: 8,612 matches matched, ~26,500 odds rows ready to insert.** Per-league highlights: USA 1641, Mexico 902, Norway 747, Japan 684, Sweden 655, Poland 505, Russia 364, Argentina 1,164 (Liga+Primera). 159 unmatched teams across the batch (mostly relegated/defunct: Honefoss, Mjondalen, Chivas USA, Amkar, etc.). Once landed, ~8K matches qualify for `predict_historical_matches.py` → backtest sample +60%, Sunday Platt retrain gets first-ever training data from these leagues. Smoke: TIER-C-EXPAND-ODDS. Run command: `python3 scripts/ingest_football_data_extras_odds.py`. |
