...
| CAL-PLATT-UPGRADE | Replace single-input Platt with 2-feature logistic: `X = [shrunk_prob, log(odds)]` | half day | ✅ Done 2026-05-12 | O/U deployed. 1X2 pending (~300 bets needed, ~2 weeks). | O/U: `fit_platt.py` fetches `simulated_bets (market='O/U')`, fits sklearn LogisticRegression `[calibrated_prob, log(odds)]`, stores `platt_c=w1`. `apply_platt(prob, market, odds=)` uses 2-feature path when `platt_c` non-null. Migration 100. Next: run `python scripts/fit_platt.py --model-version v14` to populate O/U params. 1X2 upgrade: re-run this task when 1X2 bets ≥ 300. |
