...
| SHRINKAGE-ALPHA-SQL-BUG | `load_shrinkage_alphas()` in `workers/model/improvements.py` uses `WHERE market LIKE 'shrinkage_alpha_%'` — the trailing `%` is treated as a positional parameter placeholder by psycopg2 (expects `params` tuple, gets `[]`) → `IndexError` → function always returns `{}` silently → hardcoded `CALIBRATION_ALPHA = {1: 0.20, 2: 0.30, 3: 0.50, 4: 0.65}` used instead of learned DB values (DB has `shrinkage_alpha_t1_1x2: 0.0008`, `shrinkage_alpha_t2_1x2: 0.0000`). **Fix:** escape the `%` as `%%` in the psycopg2 query string. Impact: switching to learned values will change calibrated probabilities — T1 alpha 0.20 → 0.0008 means almost no Pinnacle shrinkage for T1 1X2; T2 0.30 → 0.0000 means zero shrinkage. Validate edge distribution before deploying. | 15m | ⬜ | ✅ Ready | Low blast-radius fix but confirm the learned alpha values in model_params are still current before switching. |
