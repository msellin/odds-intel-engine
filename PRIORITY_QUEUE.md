...
> Last updated: 2026-05-07 — AF-VENUES done (venue surface signal, migration 065, fetch_venues enrichment, venue_surface_artificial in signals). AF-ODDS-MAPPING closed (already solved by bulk /odds?date=). 4-AI synthesis: AF-PLAYER-RATINGS and AF-SIDELINED queued for meta-model milestone (~May 17); AF-TRANSFERS skipped (universal consensus across all 4 models).
...
| AF-SIDELINED | Player career injury history via AF `/sidelined?player={id}`. Different from `/injuries` (fixture-specific) — full career sidelined history. Derived signals: `injury_prone_count_home/away` (players with ≥3 career injuries currently injured), `chronic_injury_risk` (weighted by career rate). 2–5 calls/day cost. 4-AI verdict: implement alongside AF-PLAYER-RATINGS — marginal signal alone, becomes useful with player infrastructure in place. | 3h | ⬜ | ⏳ Implement alongside AF-PLAYER-RATINGS (~May 17) | AF `/sidelined` endpoint. Extends player data layer. Filter `type != 'suspension'` for clean injury-only signal. |
...
| AF-TRANSFERS | ~~Mid-season transfer signal via AF `/transfers?team={id}`.~~ SKIPPED — universal consensus across all 4 AI models: 300–560 calls/day cost, signal is sparse (most teams have 0 transfers in any 30-day window during season), heavily overlaps with `news_impact_score` (Gemini already processes transfer news), and market prices major transfers immediately. | — | ⬜ | ⏳ Skip — revisit only if player-level model is mature | 4/4 AI models ranked this last. |
