...
| PER-BOT-SLICE-TIGHTEN | P2 | 3-4h | After above two. For each bot, run slice analysis on its backtest bets (by selection / odds bucket / league tier) and find loss-buckets (mirroring how AGGRESSIVE-V2 found "draws + OU under 2.5 + odds 3.3+" as v1's losers). Add `selection_filter` / `odds_range` / etc. accordingly. Likely produces a v2 of multiple bots like AGGRESSIVE-V2 did. |
