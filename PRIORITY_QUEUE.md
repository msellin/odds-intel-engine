...
> Last updated: 2026-05-09 — AF quota exhaustion incident (74,746/75K on end-of-season Saturday). Upgraded to Mega (150K/day, $39/mo). Added hard quota floor to `_get()`. AF-QUOTA-AUDIT task added with full spec and acceptance criteria. Inplay bots I (Favourite Stall), J (Goal Debt Over 1.5), L (Goal Contagion) added (9-AI consensus, Bayesian Poisson, no new AF calls). New tasks: INPLAY-NEXT-10-MIN-MARKET, INPLAY-EQUALIZER-MAGNET, INPLAY-LATE-FAV-PUSH, INPLAY-HT-REPRICING, INPLAY-CALIBRATION-IJL, INPLAY-BAYESIAN-ENGINE, INPLAY-LAYER-ARCH.
...
| ID | Task | Effort | ☑ | Ready? | Notes |
|----|------|--------|----|--------|-------|
| BULK-STORE-MATCHES | `store_match` is called per-fixture in 4 callers (fetch_fixtures, daily_pipeline_v2 ×2, backfill_historical) and does ~5 round-trips per call (ensure_team×2, dedup SELECT, INSERT/UPDATE). 1500 fixtures × 5 RTT × ~150ms EU pooler = ~15 min real time. Build `bulk_store_matches(list_of_dicts)`: one SELECT for existing-match-id-by-(home,away,date), one `execute_values` INSERT for new, one bulk UPDATE for matches needing backfilled fields (api_football_id, venue, referee, etc.). Migrate the 4 callers. Cuts fetch_fixtures step from ~15min → ~5sec. | 2-3h | ⬜ | ✅ Ready (do first tomorrow) | Risk: dedup logic is intricate (matches keyed by `(home_id, away_id, date_prefix)`, with conditional UPDATE for only-NULL fields). Smoke test must verify same end-state as current per-row code on a 50-fixture sample. Don't merge while live scheduler is busy — fixtures path runs in morning_pipeline. |
| OBS-LOG-ALL-JOBS | 11 of ~25 scheduled jobs don't log to `pipeline_runs` (`morning_pipeline`, `betting_refresh`, `news_checker`, `match_previews`, `email_digest`, `weekly_digest`, `watchlist_alerts`, `settle_ready`, `backfill_coaches`, `backfill_transfers`, `live_tracker`, `budget_sync`). The ops dashboard reads `pipeline_runs` so half the system is invisible — this is why "everything looks failing" when scheduler is restarting. Lift logging into `_run_job()` in `workers/scheduler.py` so every wrapped job auto-logs. Sub-step logging inside complex jobs (settlement chain, morning_pipeline) stays intact. | 30m | ⬜ | ✅ Ready (do second) | Adds 3 lines to `_run_job` calling `log_pipeline_start/complete/failed`. Need to handle the case where the job's body itself ALSO logs (settlement_pipeline does) — to avoid duplicate rows, check if the body already wrote a row for this run id before writing the wrapper one. Or: keep the wrapper logging only for jobs that don't already log; mark them via a flag. |
...
| AUDIT-SILENT-EXCEPT | Awareness — 64 `except: pass` cases. Most are intentional in ML feature builders (tolerant of missing data). 3 worth fixing: `scheduler.py:186/196/205` (logging errors swallowed), `live_poller.py:308` (events store errors swallowed). Add a one-line `console.print` warning when these fire. | 15m | ⬜ | ✅ Ready (low priority) | Don't blanket-fix the rest — each is a judgment call. |
...
| FETCH-ODDS-CONCURRENT | **Tomorrow.** AF /odds page size is hardcoded at 10 entries/page (probed both `?size=`, `?per_page=`, `?limit=`, `?count=` — all rejected with "field do not exist" errors). On a busy day that's ~56 sequential pages × ~340ms = ~19s of pure AF wait time per odds run. Since odds runs every 30 min (07-22 UTC = 30 runs/day), that's ~10 min/day of cron time spent waiting on serial pagination. Implement concurrent pagination in `get_odds_by_date`: fetch page 1 to learn `total_pages`, then fan out pages 2..N via `ThreadPoolExecutor(max_workers=8)`. AF Ultra rate limit (~450/min) is well above 8 concurrent. Expected step 2 AF time: ~19s → ~3s. | 30m | ⬜ | ✅ Ready (do alongside AUDIT-AF-ENDPOINTS) | Smoke test: same fixture set + record count vs sequential on yesterday's date. Risk: AF may rate-limit a burst — start with max_workers=5 if 8 trips it. |
...
| AUDIT-AF-ENDPOINTS | **Tomorrow.** Same kind of "is there a single-call form?" audit for the rest of the AF endpoints we use sequentially. The injuries win was 47×; want to know which others are similarly waste-of-time. Investigate per endpoint: (1) does AF support a date / multi-id / season-wide form? (2) is the data we actually need recoverable from one call instead of N? Candidates ranked by current cost: **(a) /standings** — currently 1 call per league × ~50 leagues; needs `league` param so likely no bulk form, but verify if comma-separated `league=A-B-C` works (analogous to `injuries?ids=`); **(b) /sidelined** — 1 call per injured player (~50-100/run); test `?players=A-B-C` form; **(c) /transfers** — 1 call per team (~100/run, 30d-cached so usually mostly cached); test `?team=A-B` form; **(d) /coachs** — 1 call per team (~50/run, 48h-cached); test `?team=A-B`; **(e) /teams/statistics** — 1 call per (team, league, season); separate problem (cache-by-last-played-match, not endpoint shape — already discussed in INJURIES-BY-DATE conversation thread); **(f) /fixtures/headtohead** — 1 call per Tier 1 pair (~50/run, same-day cached); probably no bulk form. **Method:** for each, do the same A/B I did for injuries — 1) probe endpoint with hypothesised batch syntax; 2) compare records returned vs current per-id loop on a real day; 3) document any divergences (like the "stale cross-day records" we saw on injuries); 4) if win is real, swap call sites + add smoke test + mark done. **Priority order:** standings > sidelined > transfers > coaches. **Stop criteria:** if 3 endpoints in a row show no bulk form, conclude AF only built it for /injuries and stop. **Already-known constraints:** /standings rejects `?season=` alone (errors `League field is required`) — bulk attempt confirmed needs `?league=` parameter; test comma-separated next. | 2-4h | ⬜ | ✅ Ready (tomorrow morning) | Build on INJURIES-BY-DATE pattern: live test with real data, A/B record-level diff, deprecate the per-id helper to a fallback in `api_football.py`, swap pipeline call sites, smoke-test guard against revert. Each successful endpoint conversion saves 10–60s per enrichment run. Combined potential: enrichment from ~10 min → ~3-4 min. |
...
| INPLAY-FIX-E-FALLBACK | Strategy E shot_proxy mode uses `expected_shots = (pm_xg_total / 0.10) * (minute/90)` — but 0.10 xG/shot only holds for shots on target; off-target shots average ~0.03-0.05. Expected shot denominator is inflated, `pace_ratio` comes out falsely low, and the model fires Under bets on games that aren't actually dead. **Confirmed with real data 2026-05-09:** 182 shot_proxy bets placed, 90W/92L, −8.49 pnl (-4.7% ROI). Real xG bets (11) are +0.55 pnl. Fix: (a) immediate — add `if not is_real: return None` in `_check_strategy_e` to disable shot_proxy entirely; (b) proper fix — replace proxy formula with `expected_shots_on_target = (pm_xg_total / 0.10) * (minute/90)` and compare against `shots_on_target` not total shots; (c) best fix — use league-median goals/game from `team_season_stats` as denominator. **Also: void the 182 existing shot_proxy bets** so they don't corrupt the performance page (`UPDATE simulated_bets SET result='voided' WHERE xg_source='shot_proxy' AND bot_id = (SELECT id FROM bots WHERE name='inplay_e')`). | 2h | ⬜ | ✅ Ready | Do (a) immediately to stop the bleeding. Do (b) or (c) as the proper fix in the same commit. Void the bad bets separately so the performance chart shows a clean baseline from the real-xG era. |
...
| INPLAY-BOT-RETIREMENT | Add `retired_at TIMESTAMP` column to the `bots` table (migration). Set it on `inplay_a2`, `inplay_c_home`, `inplay_f` (merged/dropped bots with no active strategy). **Performance page** (`/performance`) filters `WHERE retired_at IS NULL` — public visitors only see active bots. **Admin bot dashboard** (`/admin/bots`) shows all bots but adds a "Show retired" toggle (default off) so you can still see historical records. Prevents clutter from bots that were merged into others or dropped showing as flat-line zero-bet rows. | 1h | ⬜ | ✅ Ready | Migration: `ALTER TABLE bots ADD COLUMN retired_at TIMESTAMPTZ`. Set via `UPDATE bots SET retired_at = NOW() WHERE name IN ('inplay_a2', 'inplay_c_home', 'inplay_f')`. Frontend: `src/lib/engine-data.ts` + `src/app/(app)/performance/` + `src/app/(app)/admin/bots/`. |
...
| INPLAY-EQUALIZER-MAGNET | New strategy M: **Equalizer Magnet**. 1-0 at min 30-60, BTTS prematch ≥ 0.48, live OU25 ≥ 3.0 (market pricing high-probability draw/equalizer). Bet BTTS Yes or Over 2.5. **9-AI round-2 consensus.** Uses existing `live_btts_yes` and `live_ou_25_over` columns — zero new AF calls. | 2h | ⬜ | ✅ Ready | Edge: 1-0 markets are systematically mispriced for BTTS because sharp books focus on 1X2; soft books price BTTS independently and overreact to scoreline. Verify with `live_btts_yes IS NOT NULL` gate (low coverage ~7% — accepts sparse volume). |
| INPLAY-LATE-FAV-PUSH | New strategy N: **Late Favourite Push**. 0-0 or 1-1 at min 72-80, prematch home_win_prob ≥ 0.65, live home win odds ≥ 2.20 (drifted from ~1.5). Bivariate Poisson remaining lambda → bet Home Win if edge ≥ 3%. **9-AI round-2 consensus.** Zero new AF calls — uses `live_1x2_home`, score, and `prematch_win_prob` already in candidates. | 2h | ⬜ | ✅ Ready | Tight minute window (72-80) is intentional — before min 72 the market is still efficient; after 80 the upside is priced in. Gate on `live_1x2_home IS NOT NULL` (~16% coverage). |
...
| INPLAY-FUNNEL-LOGGING | Wire the `_funnel` dict (already added in inplay_bot.py) to heartbeat output: per-cycle counts of skips at each stage (no_prematch, league_xg_gate, no_strategy_trigger, odds_stale, score_changed, store_bet_error). Lets us see where the funnel collapses next time a strategy goes silent. | 1h | ⬜ | ✅ Ready | Defensive — pays for itself the next time a strategy goes silent. |
| INPLAY-NEXT-10-MIN-MARKET | Capture **Next 10 Minutes Total** (AF market id=65) from the existing `/odds/live` payload — goal OU 0.5 for the next ~10-min window. Already present in the live odds response — zero new AF calls required. Store as `live_next10_over` / `live_next10_under` in `live_match_snapshots`. Enables a future strategy: 0-0 min 70-80, high-λ match, bet Over 0.5 in next 10 min. **Highest-leverage free-data capture per all 4 second-round AI replies.** | 1h | ⬜ | ✅ Ready | Parse step: in `parse_live_odds()` (api_football.py), add `market_name == "Next 10 Minutes"` → `bet_name == "Over" → live_next10_over`, `"Under" → live_next10_under`. Add DB columns via migration. No strategy logic yet — just capture the market. Validate with a probe: print live_odds payload for a live match and confirm market_id=65 appears. |
| INPLAY-BAYESIAN-ENGINE | Generalise the Bayesian remaining-goals Poisson formula shared by strategies J and L into a single reusable helper `_remaining_goals_prob(pm_xg_total, minute, total_goals, threshold, h2_uplift)`. Current code duplicates the lambda calculation in `_check_strategy_j` and `_check_strategy_l`. Reduces future strategy code to 2-3 lines each. **9-AI architecture consensus.** | 1h | ⬜ | ✅ Ready | Extract after M/N/O are added — otherwise still too few callers to justify the abstraction. Also document `h2_uplift` rationale (2nd-half pace ~1.05× from Dixon & Robinson data) in the helper docstring. |
