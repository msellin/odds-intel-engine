...
> Last updated: 2026-05-07 — MGR-CHANGE done (manager signal via /coachs, migration 064, coaches enrichment component). 5 new tasks from AF docs scan: AF-PLAYER-RATINGS, AF-VENUES, AF-SIDELINED, AF-ODDS-MAPPING, AF-TRANSFERS.
...
| AH-SIGNALS | Asian Handicap line monitoring signals. Add `pinnacle_ah_line` (current AH line, e.g. -0.5), `pinnacle_ah_drift` (line shift since open), and `ah_bookmaker_disagreement` (spread of AH lines across books) to morning signals. AF odds endpoint already returns AH with `handicap` field (same pattern as O/U already parsed). AH line movement is a cleaner sharp signal than 1X2: ~2-3% lower vig = less noise, harder for retail to manipulate. Multiple AI reviewers flagged this as the highest-leverage missing signal. Wire into `batch_write_morning_signals()` and eventually meta-model features. | half day | ⬜ | ✅ Ready | `workers/api_clients/supabase_client.py` (batch_write_morning_signals) + `workers/api_clients/api_football.py` (parse_odds — handicap already parsed for O/U, same pattern for AH 1X2). |
| AF-PLAYER-RATINGS | Player ratings + per-fixture stats via AF `/players?fixture={id}`. Each played player gets a Sofascore-style float rating (6.0-10.0), minutes, goals, assists, shots, key passes, dribbles, tackles, cards. Strongest untapped AF endpoint for the model. Use case: (1) track key player form trends (rolling 5-game rating) as a pre-match signal; (2) post-match: did the "expected" key players actually play?; (3) data source for player-level injury weighting (P3.3). Cache in a `player_fixture_stats` table. Fetch post-settlement (fixture=settled_af_id). AF updates these within 30min of FT. | 1 day | ⬜ | ⏳ After backfill is complete (~May 10) | AF `/players` endpoint. Enrichment job post-settlement or standalone script. `player_fixture_stats` table (migration NNN). Could replace news_impact_score for quantifying key-player absence. Discovered via docs scan 2026-05-07. |
| AF-VENUES | Venue surface + capacity signal via AF `/venues?id={venue_id}`. Venue ID is already in the fixtures response (`fixture.venue.id`). Surface (grass vs artificial turf) is a known model variable — turf leagues have higher scoring rates and different injury patterns. Capacity as proxy for match pressure. 1 call per unique venue; cache in `venue_info` table or add columns to `leagues`. Most venues don't change surface — fetch once and cache long-term. | 2h | ⬜ | ✅ Ready | AF `/venues` endpoint. Add `venue_id` + `surface` + `capacity` to matches table (or lookup). `batch_write_morning_signals()` block: `venue_surface_artificial` (1.0/0.0). Discovered via docs scan 2026-05-07. |
| AF-SIDELINED | Player career injury history via AF `/sidelined?player={id}` (or `players` for batch up to 20). Different from `/injuries` (fixture-specific) — this is a player's full sidelined history across career. Use case: injury-proneness signal ("this player has 8 injuries in 3 years"). Batch call for all players in today's injury list. Could improve injury weighting beyond just "count of injured players". Updated several times/week. | 3h | ⬜ | ⏳ After AF-PLAYER-RATINGS (need player IDs in our DB first) | AF `/sidelined` endpoint. Extend injuries enrichment block. Discovered via docs scan 2026-05-07. |
| AF-ODDS-MAPPING | Use AF `/odds/mapping` to replace blind fixture-by-fixture polling in the odds job. `/odds/mapping` returns exactly which fixture IDs have odds available (paginated, 100/page). Currently `fetch_odds.py` polls all today's fixtures regardless — many return empty. `/odds/mapping` tells us exactly which ones to query, eliminating wasted calls. Could save 20-40% of daily odds API budget. | 1h | ⬜ | ✅ Ready | AF `/odds/mapping` endpoint. `workers/jobs/fetch_odds.py`. Add `get_odds_mapping()` to `api_football.py`. Discovered via docs scan 2026-05-07. |
| AF-TRANSFERS | Mid-season transfer signal via AF `/transfers?team={id}`. Fetches transfer history (player, from club, to club, date, type: free/loan/fee). Signal: did either team make a significant signing/departure in the last 30 days? Especially relevant for winter window (January) and summer (July-August). Cache in `team_transfers` table (already exists from prior work — check if populated). `manager_change_home_days` analogue for squad disruption. | 2h | ⬜ | ⏳ Low priority (transfers are slow signals, window effects) | AF `/transfers` endpoint. Discovered via docs scan 2026-05-07. |
