...
> Last updated: 2026-05-07 — Added 8 new tasks from 5-AI external review: VIG-REMOVE (Pinnacle vig not being stripped — confirmed bug), NEWS-IMPACT-DIR (directional signals already in Gemini output, not stored), MGR-CHANGE (new manager signal), DRAW-PER-LEAGUE (per-league draw inflation), PIPE-ALERT (proactive anomaly alerting), BM-FILTER (bookmaker filter on value bets), BOT-PUBLIC-PERF (public performance page), AH-SIGNALS (Asian Handicap line monitoring).
