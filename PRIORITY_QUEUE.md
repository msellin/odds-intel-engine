...
| OBS-HEARTBEAT | External healthchecks.io ping on `/health` every 5min. Alert when `ops_snapshot` >2h old, pool >80%, or `/health` 5xx for 2 consecutive checks. ✅ All 4 reviewers — would have caught today's outage in 5 min vs 11h. | 1h | 🔄 In Progress | ⏳ Waiting: user must create healthchecks.io check + set `HEALTHCHECKS_IO_PING_URL` env var on Railway | Code done: `job_healthcheck_ping()` in scheduler pings the URL every 5 min (no-op if env var unset). **User action: go to healthchecks.io → New Check → Period 5min, Grace 10min → copy the ping URL → set as `HEALTHCHECKS_IO_PING_URL` in Railway service env vars.** |
