...
> Last updated: 2026-05-10 — P0 + P1 + 5 P2 strategies shipped. P0/P1 (earlier today): E proxy disabled (179 bad bets voided), `_funnel` heartbeat, `_remaining_goals_prob` helper, `retired_at` bot retirement, `live_next10_*` capture. P2 batch (now): strategies M (Equalizer Magnet) and N (Late Favourite Push) shipped, strategy H refined with O2.5/O1.5 dual-line ladder, new strategy Q (Red Card Overreaction Over 2.5) shipped. Strategies G (Corner Cluster) and the LOOSEN-THRESHOLDS task were already implemented 2026-05-08 — queue status corrected. Full inplay replay across 11-day snapshot window (live odds only flow from 2026-05-07): 46 candidate bets, 32W/14L, +69.6% win rate, +378% ROI dominated by M. Q + refined H both fired 0 in replay (rare entry setups + narrow live-odds window). Treat M's ROI as upper bound — 1.3+1.3 xG fallback inflates edge in low-data leagues. Migrations 079, 080, 081, 082, 084 apply via GH Actions on push.
