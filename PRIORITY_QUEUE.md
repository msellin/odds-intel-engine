...
| ID | Task | Effort | ☑ | Ready? | Notes |
|----|------|--------|----|--------|-------|
| CAL-DIAG-1 | SQL diagnostic on 77 settled home bets: avg Poisson vs XGB prob, sharp_consensus direction, pre-Platt vs post-Platt comparison | 1h | ⬜ | ✅ Ready | Run 3 queries: (1) `avg(poisson_home_prob), avg(xgb_home_prob), avg(model_probability), avg(calibrated_prob), avg(1/odds_at_pick)` for settled home bets — tells you which component is inflated; (2) `avg(sharp_consensus_home), avg(pinnacle_implied_home), avg(1/odds_at_pick)` — if sharps agreed home was unlikely, the 31 losses were betting against sharp money; (3) how many of 31 had `sharp_consensus_home < -0.02`. Results directly inform which of CAL-PIN-SHRINK / CAL-ALPHA-ODDS / CAL-SHARP-GATE has highest ROI. Source: 4-AI Calibration Review (2026-05-06) |
| CAL-PIN-SHRINK | Switch shrinkage anchor from market avg → Pinnacle (with soft-book fallback when Pinnacle unavailable) | 30min | ⬜ | ✅ Ready | Change in betting/improvements.py: `shrunk = (1 - alpha) * model_prob + alpha * (pinnacle_implied or market_implied)`. Pinnacle vig 2-3% vs soft books 5-8% — implied probs are closer to true probability. Soft books price for liability management, not probability. `pinnacle_implied_home` already stored in match_signals (PIN-1 done). Source: 4-AI Calibration Review (2026-05-06) |
| CAL-ALPHA-ODDS | Odds-conditional α boost: `if odds > 3.0: alpha_effective = min(alpha_tier + 0.20, 0.85)` | 30min | ⬜ | ✅ Ready | Directly targets the 0.30-0.40 reliability bin (23 bets, 13% actual win rate vs 35.5% predicted). High-odds bets pulled harder toward Pinnacle → smaller apparent edge → fewer of these bets placed, which is the goal. Do after CAL-PIN-SHRINK. Source: 4-AI Calibration Review (2026-05-06) |
| CAL-SHARP-GATE | Skip 1X2 bets when `sharp_consensus < -0.02` for the selection (sharps say it's less likely than softs) | 1h | ⬜ | ✅ Ready | `sharp_consensus_home` already in match_signals (P5.1 done). Logic: `if selection='home' AND sharp_consensus_home < -0.02 → skip`. Retroactively check how many of 31 losing home bets had negative sharp_consensus before deploying (CAL-DIAG-1). Extends to away/draw via the relevant signal field. Source: 4-AI Calibration Review (2026-05-06) |
| CAL-DRAW-INFLATE | Add draw inflation factor to Poisson convolution: `adjusted_draw = raw_draw_prob × 1.08`, renormalize home/away | 1h | ⬜ | ✅ Ready | Poisson assumes scoring independence; game-state effects (parking the bus, protecting leads) inflate real draw rate. Dixon-Coles rho only patches (0,0)-(1,1) corner — higher-scoring draws (2-2, 3-3) remain underestimated. Apply after convolution, before normalization. Start at 1.08 (range 1.05-1.15), fit on backtest data via Brier score minimization. Unlocks draw market betting. Source: 4-AI Calibration Review (2026-05-06) |
...
| B-ML3 | First meta-model: 8-feature logistic regression, target=pseudo_clv>0 | 1 day | ⬜ | ⏳ ~May 10 (need 3K CLV rows, have 1,679 as of 2026-05-05) | Train after 3000+ pseudo-CLV rows. Features per META-2. See MODEL_ANALYSIS.md Stage 4. **Feature notes from 4-AI calibration review:** (1) Add `model_prob - pinnacle_implied` — likely the strongest predictor; (2) `odds_drift` and `overnight_line_move` are 0.7+ correlated — keep `odds_drift`, drop or combine `overnight_line_move`; (3) validate `news_impact_score` has AUC > 0.52 in isolation before including (LLM scores add noise unless calibrated); (4) add `odds_at_pick` (raw) — 5% edge at 1.5 odds behaves very differently from 5% edge at 5.0; (5) add `time_to_kickoff` (hours). CLV (`calibrated_prob - closing_implied`) is the strongest long-run EV validator — track as primary performance metric on settled bets, not as a placement-time feature. Source: 4-AI Calibration Review (2026-05-06) |
...
| ID | Task | Effort | ☑ | Ready? | Notes |
|----|------|--------|----|--------|-------|
| PIN-2 | Extend Pinnacle signals to all bet markets | 1h | ⬜ | ✅ Ready | Add `pinnacle_implied_draw`, `_away`, `_over25`, `_under25` to `batch_write_morning_signals()` in supabase_client.py. Same bulk query pattern as PIN-1. Unlocks PIN-3, PIN-4, PIN-6. Pinnacle covers 59% of our matches. |
| PIN-3 | Extend disagreement veto to draw/away/O/U markets | 1-2h | ⬜ | ⏳ After PIN-2 | Apply same veto logic from PIN-VETO to draw, away, over 2.5, under 2.5. Threshold may differ per market — start at 0.12, tune once 50+ settled bets per market. |
| PIN-4 | Pinnacle line movement signal | 1-2h | ⬜ | ✅ Ready | `pinnacle_line_move_home` = Pinnacle opening implied − current implied. Avg 8.7 Pinnacle snapshots/match in 30 days. Positive = home shortened (sharp money). Purer than generic `odds_drift` (Pinnacle lines only move on sharp money). Add draw + away variants. Store in `match_signals`. |
| PIN-5 | Pinnacle-anchored CLV | 2h | ⬜ | ✅ Ready | Add `clv_pinnacle` to `simulated_bets`. Compute in `settlement.py`: `(odds_at_pick / pinnacle_closing_odds) - 1`. Pinnacle CLV is the industry standard metric — consistently positive = genuinely finding edge before sharp money. Needs `odds_snapshots WHERE bookmaker='Pinnacle' AND is_closing=TRUE`. |
| PIN-6 | Pinnacle-anchored calibration shrinkage | 2-3h | ⬜ | ⏳ After PIN-2 + 200+ settled bets | Replace market avg in shrinkage with Pinnacle implied: `shrunk = (1-α)*model_prob + α*pinnacle_implied` (fallback to market avg when Pinnacle absent). Validate ECE before/after. CAL-PIN-SHRINK (Tier 1) is the quick home-only version; this is the full version for all markets. |
...
| ALN-1 | Dynamic alignment thresholds | 2h | ⬜ | ⏳ ~May 9-10 (need 300 settled bets) | Validates alignment filter on real settled bot bets. Pseudo-CLV does NOT substitute. |
