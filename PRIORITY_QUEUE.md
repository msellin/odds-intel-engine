...
| CAL-PLATT-WIDER-INPUT | P1 | 2-4h | Extend Platt calibration to use the 167K historical (prediction, outcome) pairs from BACKTEST-HISTORICAL-PREDICT-FORWARD instead of just bot bets. Currently the Wed/Sun weekly Platt refit sees only ~500 settled bot bets — with the expanded set it sees ~330x more training examples. Should produce sharper `shrinkage_alpha` per tier × per market. Directly improves the filter stack for ALL bots → better edge detection → better live ROI. Highest leverage task. Process: review `workers/jobs/settlement.py`'s Platt fit, layer historical predictions into input pool, sanity-check new alphas vs old (magnitude + direction), smoke test, deploy. |
